...
declare variable $genderInfo:queryF :=
	"SELECT DISTINCT ?ms ?temporal ?repo ?person ?role ?occupation ?birth ?death ?bondType ?related
WHERE {
?annotation a ?role ;
    oa:hasBody ?person ;
    oa:hasTarget ?ms .
...
declare variable $genderInfo:queryM :=
	"SELECT DISTINCT ?ms ?temporal ?repo ?person ?role ?occupation ?birth ?death ?bondType ?related
WHERE {
?annotation a ?role ;
    oa:hasBody ?person ;
...
declare function genderInfo:runquery($q) {
	...
	return fusekisparql:query("betamasaheft", $query)
};
...
declare function genderInfo:TempBarChartData($sparql) {
	...
	group by $r := $temp
	let $rname := if (contains($r, "t5z3")) then
		"-0999–0000 (South-Arabian Pre-Aksumite and Proto-Aksumite)"
	else if (contains($r, "tdn7")) then
		"0001–0300 (Early Aksumite)"
	else if (contains($r, "4qvv")) then
		"0300–0700 (Aksumite)"
	else if (contains($r, "rjvk")) then
		"1200–1433 (Postaksumite I)"
	else if (contains($r, "vm7f")) then
		"1434–1632 (Postaksumite II)"
	else if (contains($r, "dh3k")) then
		"1632–1769 (Gondarine)"
	else if (contains($r, "vtwm")) then
		"1769–1855 (Zamana Masāfǝnt)"
	else if (contains($r, "fc3r")) then
		"1855–1974 (Modern Period)"
	else
		"Not a valid PeriodO URI."
	let $countF := genderInfo:filter($res, "female")
	let $countM := genderInfo:filter($res, "male")
	order by $rname
	return "['" || $rname || "'," || count($countF) || "," || count($countM) || "]"
};
...
declare function genderInfo:GenBarChartData($sparql) {
	...
	let $countM := genderInfo:filter($res, "male")
	return "['" ||
		substring-after(replace($r, "\s", ""), $config:BMurl) ||
		"'," ||
		count($countF) ||
		"," ||
		count($countM) ||
...
declare %rest:GET %rest:path("BetMasWeb/gender/rels") %output:method("json") function genderInfo:relNodes() {
	let $nodes :=
		...
		return map {"id": ("https://betamasaheft.eu/" || $i), "label": $title, "group": $type}
	let $edges :=
		for $n in $genderInfo:sparqlquery//sr:result
		return (
			(: manuscript - repo :)
			map {
				"from": $n//sr:binding[@name eq "ms"]/sr:uri/text(),
			...
			map {
				"from": $n//sr:binding[@name eq "person"]/sr:uri/text(),
				"to": $n//sr:binding[@name eq "ms"]/sr:uri/text(),
				"label": substring-after($n//sr:binding[@name eq "role"]/sr:uri/text(), "https://betamasaheft.eu/"),
				"value": 1,
				"font": map {"align": "top"}
			},
			...
			if ($n//sr:binding[@name eq "related"]/sr:uri/text()) then
				map {
					...
					"label":
						substring-after(
							$n//sr:binding[@name eq "bondType"]/sr:uri/text(),
	...
	return (: returns the title and id of the entities referring to this entity or entity referring to those pointing to the entity :) (
		map {"nodes": $nodes, "edges": $edges, "cN": count($nodes), "cE": count($edges)}
	)
};
			...
			<body id="body">
				...
				<div class="w3-container w3-margin w3-padding-64" id="content">
					<div class="w3-container">
						...
						<p
						>The visualizations linked from this page 
                            represent the results of a query to the SPARQL endpoint for <b
							>persons who we
							...
							>breakdowns</a> and <a
								href="/gender/graph"
								target="_blank"
							>graphs</a> are produced on the basis of this data.
                            This is the SPARQL query producing these results:
                            <code
							>{ normalize-space($genderInfo:query) }</code>
						</p>
						<p
						>You can copy, edit it, paste it and use it from any 
                            application supporting
                            import from a SPARQL Endpoint, for example Palladio (endpoint is https://betamasaheft.eu/api/SPARQL/json), or in <a
								href="/sparql"
								target="_blank"
							>our SPARQL Endpoint</a>.</p>
						<p>At the moment the data contains some information 
                             about {
								count(config:distinct-values($sparql//sr:binding[@name eq "person"]))
							}, 
                             <a href="/gender/table/female">
								{ count(genderInfo:filter($sparql, "female")) } women </a>
                             and <a
								href="/gender/table/male"
							>{ count(genderInfo:filter($sparql, "male")) } men</a>. </p>
					</div>
				</div>
				{ nav:footerNew() }
	...
	return (
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<body id="body">
				...
				<div class="w3-container w3-margin w3-padding-64" id="content">
					<div class="w3-container">
						{ transform:transform($sparql, "xmldb:exist:///db/apps/BetMasWeb/rdfxslt/sparqltable.xsl", ()) }
					</div>
				</div>
				{ nav:footerNew() }
				<script src="resources/js/w3.js" type="text/javascript" />
...
function drawMultSeries() {
      ...
      data.addRows([" ||
										string-join(genderInfo:GenBarChartData($sparql), ",") ||
										"]);
...
function drawMultSeries() {
      ...
      data.addRows([" ||
										string-join(genderInfo:TempBarChartData($sparql), ",") ||
										"]);
      ...
      chart.draw(data, options);
						...
						<div class="w3-container">
							...
							<div id="timeline" style='height:400px;' />
      ...
      function drawChart() {
        ...
        dataTable.addRows([" ||
										string-join(genderInfo:timeline($sparql), ",") ||
										"]);
					...
					<div class="w3-container">
						...
						<div class="w3-container">
                          ...
                          https://observablehq.com/@d3/chord-diagram
							...
							<div class="w3-padding" id="BetMasRel">
