...
function workmap:workmap($worksid as xs:string*, $type as xs:string*) {
	...
	let $w := if (contains($worksid, ",")) then
		...
		return $workmap:collection-rootW/id($work)
	else
		$workmap:collection-rootW/id($worksid)
	let $baseuris :=
		for $bu in $w
		return base-uri($bu)
	let $Cmap := map {"type": "item", "name": $worksid, "path": string-join($baseuris)}
	let $kmlparam :=
		for $work at $p in $w/@xml:id
		return "kml" || $p || "=https://betamasaheft.eu/workmap/KML/" || string($work) || "?type=" || $type
	let $worktitles :=
				...
				<div class="w3-container w3-padding-64 w3-margin" id="content">
					<div class="w3-container">
						...
						>
							...
							<div class="w3-bar">
								<button class="w3-bar-item w3-button w3-red" type="submit"> Show on map
                </button>
								<a
						...
						<div class="w3-container">
							...
							<p>Map of the manuscripts of { string-join($worktitles, "; ") }
								{
									if ($type = "repo") then
										" at their current location"
									else
...
function workmap:kml($workid as xs:string, $type as xs:string*) {
	...
	return workmap:kmlfile($mss, $worktitle, $type)
};
	...
	<kml>
		{
			...
			let $place := if ($type = "repo") then
				root($ms)//t:repository[1]
			else (
				if (root($ms)//t:origPlace[t:placeName]) then
					root($ms)//t:origPlace[1]/t:placeName[1]
				else
					root($ms)//t:repository[1]
			)
			let $id := string(root($ms)/t:TEI/@xml:id)
			let $date := root($ms)//t:origDate
			let $when := if (contains($date/@when, "-")) then
				substring-before($date/@when, "-")
			else
				$date/@when
			let $getcoor := if (exists($place[1]/@ref)) then
				try {
					if (string-length(coord:invertCoord(coord:getCoords($place[1]/@ref))) lt 3) then
						"0.000,0.000"
					else
						coord:getCoords($place[1]/@ref)
				} catch * { util:log("info", $err:description) }
			else
				"0.000,0.000"
			let $reponame := exptit:printTitle($place[1]/@ref)
			return (: if($pRec//t:coord) then :) <Placemark>
				<address>
					{ $reponame } ({
						if (not($type = "repo") and root($ms)//t:origPlace[t:placeName]) then
							"place of origin"
						else
							"repository"
					})</address>
				...
				<Point><coordinates>{ coord:invertCoord($getcoor) }</coordinates></Point>
				{
					let $all := ($when, $date/@notBefore, $date/@notAfter)
					return <TimeSpan><begin>{ min($all) }</begin><end>{ max($all) }</end></TimeSpan>
				}
			</Placemark>
		}
	</kml>
