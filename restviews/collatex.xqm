...
module namespace collatex = "https://www.betamasaheft.uni-hamburg.de/BetMasWeb/collatex";
...
declare variable $collatex:meta := <meta
	...
	content="Collation Interface using Collatex https://collatex.net/"
	name="description" />;
...
declare function collatex:collatex($request as map(*)) {
	...
	let $urns :=
		...
		return $u
	return if (count($urns) le 1) then (
		<info>please provide at least 2 dts URIs separated with comma</info>
	) else (
		let $body := if ($nU != "") then
			collatex:getnarrUnitWittnesses($nU)
		else
			collatex:getCollatexBody($urns)
		let $test0 := console:log($body)
		let $req := <http:request href="{ xs:anyURI($config:collatexUrl) }" http-version="1.1" method="POST">
			<http:header name="Content-Type" value="application/json" />
			<http:header name="Access-Control-Allow-Origin" value="*" />
			<http:header name="Accept" value="application/{ $format }" />
			<http:body indent="yes" media-type="text" method="text">{ $body }</http:body>
		</http:request>
		let $post := hc:send-request($req)[2]
		(: let $test :=console:log($post) :)
		let $decoded := if (contains($format, "xml")) then
			$post
		else
			try { util:base64-decode($post) } catch * { console:log($err:description), $post }
		return $decoded
	)
};
	...
	return (
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				...
				<meta
					xmlns="http://www.w3.org/1999/xhtml"
					content="Collation of { replace($list, ",", ", ") }"
				...
				<div class="w3-container w3-margin w3-padding-64" id="content">
					<div class="w3-container">
						<div class="w3-quarter">
							<p>In this page you can collate transcriptions of manuscripts using Collatex <a
									href="https://collatex.net/"
								>https://collatex.net/</a>) from our server installation.</p>
							<p
							>You cannot enter your own text, but you can collate any text which is in the database. To add your transcription, see the guidelines <a
									href="/Guidelines/?id=howto"
								>here</a>.</p>
							<p
							>In the form below you have to provide the two DTS URNs of the passages you want to compare, then hit the collate button and you will get a visualization of the TEI apparatus output from Collatex.</p>
							<p>The format of your URNs has to be the following</p>
							<ul>
								<li>
									<pre
									>BLorient718.1</pre> will point to all the folio 1 of BL Orient 718 (recto and verso) and take all what is in there</li>
								<li>
									<pre
									>BLorient718.1r</pre> will point to all the folio 1 recto of BL Orient 718 and take all what is on that page</li>
								<li>
									<pre
									>BLorient718.1ra</pre> will point to  the folio 1 recto, column a  of BL Orient 718 and take all what is in that column</li>
								<li>
									<pre
									>BLorient718.1ra1</pre> will point to  the folio 1 recto, column a  of BL Orient 718 and take all what is in that column, line 1</li>
								<li>
									<pre
									>BLorient718.1ra1-1ra3</pre>will point to  the folio 1 recto, column a  of BL Orient 718 and take all what is in that column, line 1 to 3</li>
								<li>
									<pre
									>BLorient718.1.1-1.3</pre> will point to  the folio 1 recto, column a  of BL Orient 718 and take all what is in that column, line 1 to 3</li>
								<li>
									<pre
									>BLorient718.1ra@ወወልድ</pre>  will point to  the folio 1 recto, column a  of BL Orient 718 and take all what is in that column starting from the first occurrence of the word ወወልድ</li>
								<li>
									<pre
									>BLorient718.1ra@ወወልድ[1]</pre> will point to  the folio 1 recto, column a  of BL Orient 718 and take all what is in that column starting from the first occurrence of the word ወወልድ, but note the specification of the occurrence, it could also be [2]</li>
								<li>
									<pre
									>BLorient718.1ra@ወወልድ[1]-1ra@ቅዱስ[1]</pre> will point to the folio 1 recto, column a  of BL Orient 718 and take all what is in that column starting from the first occurrence of the word ወወልድ, and ending at the first occurrence of the word ቅዱስ</li>
								<li>
									<pre
									>BLorient718.1ra@ወወልድ[1]-3va</pre> will point to the folio 1 recto, column a  of BL Orient 718 and take all what is in that column starting from the first occurrence of the word ወወልድ, and everything including in the transcription up to the end of folio 3va</li>
								<li>
									<pre
									>EMIP01859.month1.day1</pre> is also a valid reference and will fetch the element in the manuscript transcription associated with the specified structure of type and names</li>
								<li>
									<pre
									>EMIP01859.1.1.1</pre> is also a valid reference and will fetch the nested structured marked with n</li>
								<li>
									<pre
								...
								<li>
									<pre
									>EMIP01859.NAR0019SBarkisos</pre> is also a valid reference and will fetch the element in the manuscript transcription associated with the specified narrative unit. If you want to fetch passages connected to a narrative unit without specifiying the manuscript, add the narrative unit id instead</li>
							</ul>
							<p
							>Of course, if there is no transcription, there will be no text to collate and the quality of the transcription depends on external factors.</p>
							<p
							>The level of precision possible, depends from the encoding. If you have encoded page and column breaks you can use them, otherways hopefully there are at least folia divisions.</p>
							<p
							>The collated text is cleaned from all markup and all punctuation and uses the default collation algorithm of Collatex.</p>
						</div>
						<div class="w3-threequarter">
							<div id="dtsURNs">
								<input
									class="dts w3-input w3-margin w3-border"
									placeholder="{{manuscriptid}}.{{passage}}-{{passage}}"
									type="text" />
								<input
									class="dts w3-input w3-margin w3-border"
									placeholder="{{manuscriptid}}.{{passage}}-{{passage}}"
							...
							<p
							>Alternatively, you can specify a narrative unit and we will pick any manuscript transcription which contains a reference to that and collate it.</p>
							<div id="narrativeUnit">
								<input class="narrUnit w3-input w3-margin w3-border" placeholder="{{narrativeunitid}}" type="text" />
							</div>
...
declare function collatex:getCollatexWitnessText($dtsURN) {
	...
	let $splitref := if (contains($ref, "-")) then
		tokenize($ref, "-")
	else
		$ref
	let $cleanref :=
	...
	let $tokenizetext := <ts>
		{
			for $t in tokenize($cleantext, "\s+")
			return <t>{ $t }</t>
		}
	</ts>
	(: let $t := console:log($tokenizetext) :)
	let $l1 := if (count($delimiters) = 1) then
		$delimiters
	else
		$delimiters[1]
	(: let $t := console:log($l1) :)
	let $l2 := if (count($delimiters) = 2) then
		$delimiters[2]
	else
		"n/a"
	(: let $t := console:log($l2) :)
	let $firstlimit := if ($l1 = "n/a") then
		1
	else (
		count($tokenizetext//*:t[contains(., $l1)]/preceding-sibling::*:t) + 1
	)
	let $secondlimit := if ($l2 = "n/a") then
		count($tokenizetext//*:t)
	else (
		count($tokenizetext//*:t[contains(., $l2)]/preceding-sibling::*:t) + 1
	)
	(: let $t := console:log(($firstlimit||'-'||$secondlimit)) :)
	let $delimited :=
		for $r in $firstlimit to $secondlimit
		return $tokenizetext//*:t[$r]
...
declare function collatex:getCollatexBody($urns) {
	let $matchingmss :=
		...
		return $witness
	return '{"witnesses" : [' || string-join($matchingmss, ",") || "]}"
};
...
declare function collatex:getnarrUnitWittnesses($nU) {
	let $matchingmss :=
		for $ms in ($collatex:expanded//t:div[ends-with(@corresp, $nU)], $collatex:expanded//t:div[@corresp = $nU])
		...
		return if ($ID = "") then (
			...
			return '{"id" : "' || $ID || '", "content" : "' || $cleantext || '"}'
	return '{"witnesses" : [' || string-join($matchingmss, ",") || "]}"
};
...
declare function collatex:cleanforcollatex($text) {
	...
	return normalize-space($cleantext)
};
