...
(:
 ...
 : zero-length string - analyze-string() (dtslib:parseDTSid) forbids that
 ...
 :)
declare variable $dtslib:regexID :=
...
declare function dtslib:fileingitCommits($id, $bmID, $apitype) {
	...
	let $permapath := replace(
		dtslib:capitalize-first(substring-after(base-uri($file), "/db/apps/BetMasData/")),
		$collection,
		""
	)
...
declare %private function dtslib:apparatus2string($apparatus as node()*) {
	...
	return typeswitch ($node)
		case element(cx:apparatus) return
			<div>{ dtslib:apparatus2string($node/node()) }</div>
		case element(t:app) return
			<div class="row">{ dtslib:apparatus2string($node/node()) }</div>
		case element(t:rdg) return
			<div>{ string($node/@wit) }{ dtslib:apparatus2string($node/node()) }</div>
		case text() return
...
did not work, emailed exist db, Magdalena Turska very kindly provided this alternative approach.
 :)
		...
		dtslib:TranscriptionPassageNodes($text, $selector/@n, "")
			...
			let $links := if ($ref = "") then (
				...
				<http:header
					...
					value="&lt;/api/dts/document?id={ $id }&amp;ref={
						dtslib:PrevNextRef($text, $start, "prev")
				...
				<http:header
					name="Link"
					value="&lt;/api/dts/document?id={ $id }&amp;ref={
						dtslib:PrevNextRef($text, $ref, "prev")
					}&gt; ; rel='prev', &lt;/api/dts/document/?id={ $id }&amp;ref={
...
declare %private function dtslib:fragment($file, $edition, $ref, $start, $end, $text) {
  ...
  return :)
		...
		if (starts-with($ref, "NAR") or starts-with($ref, "https://betasamaheft.eu/NAR")) then (
			...
			return <TEI xmlns="http://www.tei-c.org/ns/1.0">
				<dts:fragment xmlns:dts="https://w3id.org/dts/api#">{ $msCorr }</dts:fragment>
			</TEI>
			...
			return (: util:log("info", "breakpoint") :) <TEI xmlns="http://www.tei-c.org/ns/1.0">
 ...
 :)(: let $test := console:log(string-join($possibleRefs, ' - ')) :)
		...
		let $selectors :=
			...
			return <s>
				...
				<path>{ dtslib:selectorRef($l, $text, $cleanMSrefs[$r], "no") }</path>
			</s>
		let $nodes :=
...
(:~
 ...
 :)
declare function dtslib:parseRef($ref) {
...
month1.day30.NAR0069Gabreel ((@subtype,@n).(@subtype,@n).(@corresp))
 :)
	...
	else if ($parseRef//s:non-match) then (
		if (matches($ref, "\.")) then
			for $mp at $pp in tokenize($ref, "\.")
			return <ref l="{ $pp }" type='subtypeNorXMLid'><option type="xmlid">{ $mp }</option></ref>
		else
			<ref l="1" type='subtypeNorXMLid'><option type="xmlid">{ $ref }</option></ref>
	) else
...
declare function dtslib:refname($n) {
   ...
   has a valid position in the text structure :)
	...
	return if (starts-with($this, "ms_") or $n/name() = "pb" or $n/name() = "cb" or $n/name() = "lb") then
		$this
	else
		string-join($all, ".")
};
...
declare function dtslib:rn($n) {
	...
	) else if (matches($n/@corresp, "#[mp]")) then
		substring-after($n/@corresp, "#")
	else if ($n/@n) then
		string($n/@n)
...
declare function dtslib:reftitle($n) {
	if ($n/@corresp) then (
		if (starts-with($n/@corresp, "#")) then
			<title>{ normalize-space(exptit:printSubtitle($n, substring-after($n/@corresp, "#"))) }</title>
		else if (starts-with($n/@corresp, "LIT")) then
			<title>{ normalize-space(exptit:printTitleID($n/@corresp)) }</title>
		else if (starts-with($n/@corresp, "NAR")) then
			<title>{ normalize-space(exptit:printTitleID($n/@corresp)) }</title>
		else
			<title>{ exptit:printTitle($n/@corresp) }</title>
...
declare function dtslib:wits($mydoc, $BMid) {
	let $computedWitnesses := if ($mydoc/@type eq "mss") then (
		...
		for $witness in $dtslib:collection-rootMS//t:title[starts-with(@ref, $BMid)]
		let $root := root($witness)/t:TEI/@xml:id
		group by $groupkey := $root
		return string($groupkey)
	)
...
declare %private function dtslib:pas($n, $fallback, $type, $manifest, $BMid) {
	<p>
		{ dtslib:refname($n) }
		{ dtslib:typename($n, $fallback) }
		{ dtslib:reftitle($n) }
		{ dtslib:passageIIIFrange($n, $manifest, $type, $BMid) }
	</p>
};
...
is built from pb and cb or from subtype and n. :)
			let $partpath := (
				switch ($ty)
					case "corresp" return
						"/t:div[ends-with(@corresp,'" || $r/text() || "')]"
					case "nar" return
...
of pbs and cbs available within it.   :)
					...
					case "subtypeNorXMLid" return
						"/(t:div[@subtype='" ||
							...
							")"
					default return
						"/(t:div|t:cb|t:pb|t:lb|t:l)[(@xml:id='" ||
							$r/text() ||
							"') or (@n='" ||
							$r/text() ||
							"') or contains(@corresp,'" ||
...
declare function dtslib:mainColl($collURN, $count, $items, $page, $nav) {
	...
	let $members :=
		...
		let $edition := ""
		return dtslib:member($collURN, $edition, $document, "no")
	return map {
		"@context": $dtslib:context,
		"@id": $collURN,
		"@type": "Collection",
...
declare %private function dtslib:nestedDivs($edition as node()*) {
	...
	let $citStr := if ($node/child::t:div) then
		let $subType := dtslib:nestedDivs($node/child::t:div)
		return map:put($citType, "dtslib:citeStructure", $subType)
	else if ($node/t:ab/t:l) then
		let $subType := dtslib:nestedDivs($node)
		return map:put($citType, "dtslib:citeStructure", $subType)
...
declare %private function dtslib:membercontent($document, $edition, $vers, $nosparql) {
	...
	let $teirefdecl := if ($doc/@type eq "mss" and not($doc//t:objectDesc/@form = "Inscription")) then
		[
			map {
				"dts:citeType": "folio",
				"dtslib:citeStructure": [map {"dts:citeType": "page", "dts:citeStructure": [map {"dts:citeType": "column"}]}]
			}
		]
	else if ($doc/@type eq "nar") then (
	) else
	...
	let $ext := if (count($parts) ge 1) then
		map:put($all, "dtslib:extensions", $parts)
	else
		$all
	let $pass := if ($c le 1) then
		$ext
	else
		map:put($ext, "dtslib:passage", $dtsPass)
	let $nav := if ($c le 1) then
		$pass
	else
		map:put($pass, "dtslib:references", $dtsNav)
	return $nav
};
...
declare %private function dtslib:membercontent($document, $edition, $vers) {
	...
	let $manifest := dtslib:manifest($doc, $id)
	let $addmanifest := if ((count($ext) ge 1) and (count($manifest) ge 1)) then
...
declare function dtslib:dublinCore($id) {
	...
	let $title := if (starts-with($id, "LIT")) then
		dtslib:DCsparqls($id, "title")
	else
		map {"@value": exptit:printTitleID($id), "@lang": "en"}
	let $relation := dtslib:DCsparqls($id, "relation")
	let $listChange :=
		for $change in $exptit:col/id($id)//t:change
...
declare %private function dtslib:callfuseki($querytext) {
...
(:~
 ...
 :)
declare %private function dtslib:mapconstructor(
	$id,
	$currentmap as map(*)?,
	$candidateproperty,
	$index as xs:integer,
	...
	if ($index = count($listofcandidateproperties)) then
		...
		return if (count($candidatevalue) = 0 and count($inversestatement) = 0) then
			dtslib:mapconstructor($id, $currentmap, $next, $index + 1, $listofcandidateproperties)
		else
...
declare function dtslib:WebAnn($id, $indexEntries, $page) {
	...
	let $dtslinks := if (starts-with($r, "#")) then
		map {"dtslib:passage": $doc}
	else
		map {"dtslib:passage": $doc, "dtslib:references": $nav}
	let $separator := if (starts-with($r, "#")) then (
	) else
		"."
	let $tit := if (starts-with($r, "#")) then
		exptit:printSubtitle($closestreference, $r)
	else
		dtslib:reftitle($closestreference)
...
declare function dtslib:AnnoEntriesAttestations($indexName, $indexEntries, $page) {
	...
	let $c := $r//*:count/text()
	let $i := $r//*:id/text()
	return dtslib:refannocol($i, $c, $indexName)
};
...
declare function dtslib:AnnoEntriesAttestationsItem($BMid, $title, $indexName, $indexEntries, $page) {
	...
	let $refs :=
		...
		let $tit := if (contains($ref, "urn:")) then
			...
			try { exptit:printTitleID($ref) } catch * { console:log($err:description) }
		order by $c descending
	...
	return dtslib:refannocolItem($BMid, $title, $i, $c, $indexName, $entityorlocus)
};
...
declare function dtslib:ItemAnnotationsEntries($name) {
	switch ($name)
		case "mss" return
			count(
				$dtslib:collection-rootMS//t:TEI[descendant::t:persName[@ref[. != "PRS00000" and . != "PRS0000"]] or
					descendant::t:placeName[@ref] or
					descendant::t:title[@ref] or
					descendant::t:term[@key][not(parent::t:keywords)] or
					descendant::t:ref[@cRef]]
		...
		case "works" return
			count(
				$dtslib:collection-rootW//t:TEI[descendant::t:persName[@ref] or
					descendant::t:placeName[@ref] or
					descendant::t:title[@ref] or
					descendant::t:term[@key][not(parent::t:keywords)] or
					descendant::t:ref[@cRef]]
		...
		case "nar" return
			count(
				$dtslib:collection-rootN//t:TEI[descendant::t:persName[@ref] or
					descendant::t:placeName[@ref] or
					descendant::t:title[@ref] or
					descendant::t:term[@key][not(parent::t:keywords)] or
					descendant::t:ref[@cRef]]
		...
		default return
			count(
				(
					$dtslib:collection-rootMS, $dtslib:collection-rootW, $dtslib:collection-rootN
				)//t:TEI[descendant::t:persName[@ref] or
					descendant::t:placeName[@ref] or
					descendant::t:title[@ref] or
					descendant::t:term[@key][not(parent::t:keywords)] or
					descendant::t:ref[@cRef]]
...
declare %private function dtslib:ItemAnnoCount($id) {
	count(
		$exptit:col//id($id)[descendant::t:persName[@ref[. != "PRS00000" and . != "PRS0000"]] or
			descendant::t:placeName[@ref] or
			descendant::t:title[@ref] or
			descendant::t:term[@key][not(parent::t:keywords)] or
			descendant::t:ref[@cRef]]
...
declare function dtslib:switchContext($context) {
	switch ($context)
		case "mss" return
			$dtslib:collection-rootMS
		case "works" return
			$dtslib:collection-rootW
		case "nar" return
			$dtslib:collection-rootN
...
declare function dtslib:CollAnno($context, $indexes) {
	...
	return if ($count = 0) then (
		...
		dtslib:MainAnnotationCollections($context, $index, $count)
};
...
declare function dtslib:sortingkey($input) {
