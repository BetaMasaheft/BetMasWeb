...
declare function coord:coordType($placenameref as xs:string) {
	...
	let $seq :=
		for $point in tokenize($data//coord, " ")
		return $point
	return if ((count($seq) gt 1) and ($seq[last()] eq $seq[1])) then (
		"polygon"
	) else (
		"point"
	)
...
declare function coord:invertCoord($coords as xs:string*) {
	if (count($coords) gt 1) then (
	) else if (coord:coordType($coords, "string") = "polygon") then (
	) else if ($coords = " ") then (
	) else if ($coords = ",") then (
	) else
		let $invert := substring-after($coords, ",") || "," || substring-before($coords, ",")
		return replace($invert, " ", "")
};
...
declare function coord:GNorWD($placeexternalid as xs:string) {
	...
	else if (starts-with($placeexternalid, "pleiades:")) then
...
declare function coord:getWikiDataCoord($Qid as xs:string) {
   ...
   SERVICE wikibase:label { 
	...
	return normalize-space(replace($removetrailing, " ", ","))
};
