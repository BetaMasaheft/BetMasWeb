...
declare function expand:create-collections($uri as xs:string) {
	...
	let $parts :=
		for $part at $p in tokenize($collection-uri, "/")
		return <part n="{ $p }">{ $part }</part>
	...
	return if (xmldb:collection-available($current-path) or $current-path = "//") then (
	) else (
		xmldb:create-collection($parent-collection, $collection),
		let $createdcol := xs:anyURI($parent-collection || $collection)
		return (sm:chgrp($createdcol, "Cataloguers"), sm:chmod($createdcol, "rwxrwxr-x"))
	)
};
...
declare function expand:id($id) {
	...
	if (starts-with($id, "http")) then
		$id
	else if (matches($id, "betmas:")) then
		$id
...
declare function expand:tei2fulltei($nodes as node()*, $bibliography) {
	...
	return typeswitch ($node)
							...
							<listRelation>
								{
									...
									return expand:tei2fulltei($relations/node(), $bibliography)
								}
							</listRelation>
						</standOff>
					else (
		...
		case element(t:listRelation) return
			()
		case element(t:listPrefixDef) return
			()
		case element(t:publicationStmt) return
			element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
				($node/@*, expand:tei2fulltei($node/node(), $bibliography), expand:dateidno($node))
			}
		case element(t:encodingDesc) return
			element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
				...
				<p xmlns="http://www.tei-c.org/ns/1.0">Encoded according
					...
					>Beta maṣāḥǝft Guidelines</ref>.
                        These Guidelines detail the TEI format ruled by
                        the <ref
						target="https://betamasaheft.eu/Guidelines/?id=schemaView"
					>Beta maṣāḥǝft Schema</ref>.
                        The present TEI file is enriched with an
                        <ref
						target="https://github.com/BetaMasaheft/BetMas/blob/master/db/apps/BetMasWeb/modules/expand.xqm"
					>Xquery transformation</ref>
                        taking advantage of the <ref
						target="https://betamasaheft.eu"
					>exist-db database instance</ref> where
		...
		case element(t:relation) return
			<relation xmlns="http://www.tei-c.org/ns/1.0" name="{ $node/@name }" ref="{ expand:id($node/@name) }">
				{
					for $attribute in ($node/@active | $node/@passive | $node/@mutual)
					return attribute {$attribute/name()} { expand:token(normalize-space($attribute)) },
					expand:tei2fulltei($node/node(), $bibliography)
				}
			</relation>
		case element(t:titleStmt) return
			<titleStmt xmlns="http://www.tei-c.org/ns/1.0">
				<title type="full">
					{
						try { expand:printTitleMainID(string($node/ancestor-or-self::t:TEI/@xml:id)) } catch * {
							util:log("INFO", concat("no full title added for ", string($node/ancestor-or-self::t:TEI/@xml:id)))
						}
					}
				</title>
				{ expand:tei2fulltei($node/node(), $bibliography) }
				{
					...
					return <respStmt corresp="https://betamasaheft.eu/team.html#{ $resp }" xml:id="ged-{ $resp }">
				...
				}
				{
					...
					return <respStmt
						...
						xml:id="{
							if ($role) then
								concat(substring($role, 1, 3), "-", $resp)
							else
								concat("ed-", $resp)
						}"
					>
				...
				}
				{
					...
					return <respStmt corresp="https://betamasaheft.eu/team.html#{ $resp }" xml:id="{ $resp }">
		...
		case element(t:revisionDesc) return
			element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} { expand:tei2fulltei($node/node(), $bibliography) }
		case element(t:change) return
			<change xmlns="http://www.tei-c.org/ns/1.0" when="{ $node/@when }" who="#{ $node/@who }">
				<!--{let $resp := $node/@who return $expand:editorslist//t:item[@xml:id = string($resp)]/text()}: -->
				{ $node/text() }
			</change>
		case element(t:profileDesc) return
			element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
				(
					expand:tei2fulltei($node/node(), $bibliography),
		...
		case element(t:repository) return
			expand:refel($node, $bibliography)
		case element(t:msIdentifier) return
			expand:refel($node, $bibliography)
		...
		case element(t:editor) return
			let $k := $node/@key
			let $role := $node/@role
			return element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
				(
					$node/@*,
					attribute corresp { concat("https://betamasaheft.eu/team.html#", $k) },
					attribute {"xml:id"} {
						if ($role) then
							concat(substring($role, 1, 2), "-", $k)
						else
							$k
					},
		...
		case element(t:bibl) return
			...
			return element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
				(
					...
					if ($node/@corresp or $matchingbibl/@type) then
						<seg xmlns="http://www.tei-c.org/ns/1.0">
							{
								if ($node/@corresp) then
									attribute corresp { $node/@corresp }
								else (
								),
								if ($matchingbibl/@type) then
									attribute type { $matchingbibl/@type }
								else (
								)
							}
					...
					if ($node/@corresp) then
						expand:biblCorresp($node/@corresp, $node)
					else (
					),
		...
		case element(t:handNote) return
			...
			element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
				(
					...
					if ($node/@facs) then
						...
						return attribute corresp {
							if (
								starts-with($mainFacs, "http") or
									(starts-with($mainFacs, "#")) or
									contains($mainFacs, "vatlib") or
									contains($mainFacs, "gallica")
							) then
								$mainFacs
							else
								concat(
									$expand:BMurl,
									"api/iiif/",
									$node/ancestor-or-self::t:TEI/@xml:id,
									"/manifest",
									(
										if ($node/parent::t:altIdentifier/@xml:id) then
											"?alt=" || string($node/parent::t:altIdentifier/@xml:id)
										else if ($node/parent::t:altIdentifier) then
											"?alt=alt"
										else (
										)
									)
		...
		case element(t:material) return
			element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
				(
					$node/@*,
					if (empty($node)) then
...
declare function expand:citeStructBypass($nodes) {
	...
	return expand:citeStructure($sublevel)
};
...
declare function expand:dateidno($node) {
	...
	return (
		<date xmlns="http://www.tei-c.org/ns/1.0" type="expanded">
			{ format-dateTime(current-dateTime(), "[D1].[M1].[Y1] at [H01]:[m01]:[s01]") }
		</date>,
		...
		return (
			...
			let $col := switch2:col($node/ancestor-or-self::t:TEI/@type)
			return (
...
declare function expand:attributes($node, $bibliography) {
	element {fn:QName("http://www.tei-c.org/ns/1.0", name($node))} {
		(
			$node/@*[not(name() = "corresp")][not(name() = "resp")][not(name() = "who")][not(name() = "ref")][not(
				name() = "sameAs"
			)][not(name() = "calendar")],
			(
			...
			if ($node/@corresp and $node[not(@type = "external")]) then
				attribute corresp {
					$expand:BMurl ||
						(
							if (starts-with($node/@corresp, "#")) then
								string($node/ancestor-or-self::t:TEI/@xml:id) || string($node/@corresp)
							else
								string($node/@corresp)
						)
				}
			else if ($node/@corresp and $node[@type = "external"]) then
				attribute corresp {
					(
						if (starts-with($node/@corresp, "#")) then
							string($node/ancestor-or-self::t:TEI/@xml:id) || string($node/@corresp)
		...
		if ($node/name() = "witness" and $node[not(@type = "external")] and not($node/text())) then (
			...
			return ($file//t:msIdentifier/t:idno)
		) else (
		)
	}
};
...
declare function expand:file($filepath) {
	...
	let $zotero :=
		...
		let $z := if ($expand:zotero//t:biblStruct[t:note[@type = "tags"]/t:note[@type = "tag"] = $ptr]) then
			$expand:zotero//t:biblStruct[t:note[@type = "tags"]/t:note[@type = "tag"] = $ptr][1]
		else
			try {
				doc(concat("https://api.zotero.org/groups/358366/items?tag=", $ptr, "&amp;format=tei"))//t:biblStruct
			} catch * { console:log($err:description) }
		return <bibl xmlns="http://www.tei-c.org/ns/1.0">
			{
				if ($z/@corresp) then
					attribute corresp { $z/@corresp }
				else (
				)
			}
			{
				if ($z/@type) then
					attribute type { $z/@type }
				else (
				)
			}
			<ptr target="{ $ptr }" />
			{
				for $t in $z//t:title
				return <title>{ $t/@* }{ $t/node() }</title>
			}
			{
				for $a in $z//t:author
				return <author>{ $a/@* }{ $a/node() }</author>
			}
			{
				for $a in $z//t:editor
				return <editor>{ $a/@* }{ $a/node() }</editor>
			}
			{
				for $a in $z//t:pubPlace
				return <pubPlace>{ $a/@* }{ $a/node() }</pubPlace>
			}
			{
				for $a in $z//t:publisher
				return <publisher>{ $a/@* }{ $a/node() }</publisher>
			}
			{
				for $a in $z//t:date
				return <date>{ $a/@* }{ $a/node() }</date>
			}
			{
				for $a in $z//t:series
				return <series>{ $a/@* }{ $a/node() }</series>
			}
			{
				for $a in $z//t:biblScope
				return <biblScope>{ $a/@* }{ $a/node() }</biblScope>
			}
			{
				for $a in $z//t:note
...
declare function expand:biblCorrTok($corresp, $node) {
	let $c := if (starts-with($corresp, "#")) then
		concat($node/ancestor-or-self::t:TEI/@xml:id, $corresp)
	else
		string($corresp)
	let $anchor := if (contains($corresp, "#")) then
	...
	let $prefix := if ($listWit ge 1) then
		$listWit
	else if (string-length($anchornode/name()) ge 1) then (
		" ", $anchornode/name(), " "
	) else (
	)
	let $lang := if (string-length($anchornode/@xml:lang) ge 1) then
		concat(" [", $node//t:language[@ident = $anchornode/@xml:lang], "]")
	else (
	)
	let $wit := if (
		$anchornode/name() = "witness" and $anchornode[not(@type = "external")] and not($anchornode/text())
	) then (
	...
	let $text := if ($anchornode//text()) then
		$anchornode//text()
	else if ($wit) then
		string($wit)
	else
	...
	return (
		...
		<ref
			...
			target="https://betamasaheft.eu/{ $node/ancestor-or-self::t:TEI/@xml:id }#{ $anchor }"
		>{ $text }</ref>
	)
};

declare function expand:biblCorresp($corresp, $node) {
	let $string := normalize-space($corresp)
	return <note xmlns="http://www.tei-c.org/ns/1.0" type='about'>about {
			if (contains($string, " ")) then
				for $corr at $p in tokenize($string, " ")
				return (
					expand:biblCorrTok($corr, $node),
					if ($p = count(tokenize($string, " "))) then (
					) else
						", "
				)
			else
				expand:biblCorrTok($corresp, $node)
		}
	</note>
};
