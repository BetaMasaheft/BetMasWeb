	...
	return typeswitch ($node)
		...
		case element(t:titleStmt) return
			<titleStmt xmlns="http://www.tei-c.org/ns/1.0">
				<title type="full">
					{
						try { titles:printTitleMainID(string($node/ancestor-or-self::t:TEI/@xml:id)) } catch * {
		...
		case element(t:change) return
			<change xmlns="http://www.tei-c.org/ns/1.0" when="{ $node/@when }" who="#{ $node/@who }">
				{
					let $resp := $node/@who
					return $expand:editorslist//t:item[@xml:id = string($resp)]/text()
				}: { $node/text() }
			</change>
				...
				(
					...
					if ($node[not(text())] and $node/@corresp and $node/@type[not(. = "authFile")]) then
						...
						return if ($corrnode) then
							let $buildID := titles:printTitleMainID($node/ancestor-or-self::t:TEI/@xml:id) ||
								" element " ||
								$corrnode/name() ||
								" with id " ||
								string($node/@corresp)
					...
					if ($node[not(text())]) then (
						let $valandcomma := (
							...
							let $fFornot := if (($node/preceding-sibling::element())[1]/name() = "locus") then
								"x"
							else if ($node/ancestor-or-self::t:TEI//t:extent/t:measure[@unit = "page"]) then
								"pp"
							else if (($node/following-sibling::element())[1]/name() = "locus") then (
								$fF || "ols"
							) else (
								$fF || "ol"
							)
							...
							let $value := (
								if ($node/@from and $node/@to) then
									...
									let $cutlistoftargets := if (count($targets) ge 3) then (
										...
										$targets
									return string-join($cutlistoftargets, ", ")
								else
									$node/node()
							)
							return $value,
						...
						return replace(string-join($valandcomma), " , ", ", ")
					) else (
					),
					expand:tei2fulltei($node/node(), $bibliography)
				)
				...
				(
					...
					if ($node/@facs) then
						...
						return attribute corresp {
							if (
								starts-with($mainFacs, "http") or contains($mainFacs, "vatlib") or contains($mainFacs, "gallica")
							) then
								$mainFacs
...
declare function expand:biblCorrTok($corresp, $node) {
	let $c := if (starts-with($corresp, "#")) then
		...
		string($corresp)
	let $anchor := if (starts-with($corresp, "#")) then
	...
	return ($prefix, <ref xmlns="http://www.tei-c.org/ns/1.0" target="/{ $c }">{ titles:printTitleID($c) }</ref>)
};

declare function expand:biblCorresp($corresp, $node) {
	<note xmlns="http://www.tei-c.org/ns/1.0" type='about'>(about: {
			if (contains($corresp, "\s")) then
				for $corr in tokenize($corresp, "\s")
				return expand:biblCorrTok($corr, $node)
			else
