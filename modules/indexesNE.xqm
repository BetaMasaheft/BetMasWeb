...
import module namespace switch = "https://www.betamasaheft.uni-hamburg.de/BetMasWeb/switch2" at "xmldb:exist:///db/apps/BetMasWeb/modules/switch2.xqm";

declare
	%templates:default("collection", "") %templates:default("entity", "") %templates:default("pointer", "")
function indexesNE:placeNames(
	$node as node(),
	$model as map(*),
	...
	return map {"hits": $placeIDS, "type": "indexes"}
};

declare
	%templates:default("collection", "") %templates:default("entity", "") %templates:default("pointer", "")
function indexesNE:persNames(
	$node as node(),
	$model as map(*),
	$collection as xs:string,
	$entity as xs:string,
	...
	return map {"hits": $persIDS[. != "PRS00000"][. != "PRS0000"], "type": "indexes"}
};
...
declare function indexesNE:placeNameForm($node as node(), $model as map(*)) {
	<form xmlns="http://www.w3.org/1999/xhtml" action="" class="w3-container">
		<div class="w3-container w3-margin-bottom">
			...
			<br />
			<input class="w3-input w3-border" name="pointer" placeholder="LOC / INS / pleiades: / wd:" />
		</div>
		<div class="w3-container w3-margin-bottom">
...
declare function indexesNE:persNameForm($node as node(), $model as map(*)) {
	<form xmlns="http://www.w3.org/1999/xhtml" action="" class="w3-container">
		<div class="w3-container  w3-margin-bottom">
			<small>enter a Beta maṣāḥǝft or Wikidata identifier</small>
			<br />
			<input class="w3-input w3-border" name="pointer" placeholder="PRS / wd:" />
		</div>
		<div class="w3-container w3-margin-bottom">
			<small>Select one collections</small>
			<br />
			<select class="w3-select w3-border" name="collection">
				<option value="all">all</option>
				<option value="mss">Manuscripts</option>
...
function indexesNE:placeNamesRes($node as node(), $model as map(*), $start as xs:integer, $per-page as xs:integer) {
	...
	let $ptrs := $exptit:col//t:placeName[@ref eq $target]
					...
					<ul class="nodot">
						{
							...
							return <li style="margin-bottom:20pt">in <a href="{ $root }">{ exptit:printTitle($root) }</a> as
     <ol>
									{
				...
				<div class="w3-half w3-padding">{ charts:pieAttestations($target, "placeName") }</div>
			</div>
		</div>
	</div>
};
...
function indexesNE:persNamesRes($node as node(), $model as map(*), $start as xs:integer, $per-page as xs:integer) {
	for $target at $p in subsequence($model("hits"), $start, $per-page)
	let $ptrs := $exptit:col//t:persName[@ref eq $target]
	let $count := count($ptrs)
    ...
    has been marked up <span class="badge">
     ...
     <ol>
				...
				<div class="w3-half w3-padding">{ charts:pieAttestations($target, "persName") }</div>
			</div>
		</div>
	</div>
};
