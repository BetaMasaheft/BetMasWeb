...
(:~
 ...
 :)
declare
	%test:arg("type", "work") %test:assertEquals("works") %test:arg("type", "corpus") %test:assertEquals("corpora")
...
declare
	%test:arg("type", "works")
	%test:assertEquals("'collection($config:data-rootW)")
	%test:arg("type", "xxxx")
	%test:assertEquals("'collection($config:data-root)")
function switch2:collection($type) {
	switch ($type)
		case "works" return
			"collection($config:data-rootW)"
		case "narratives" return
			"collection($config:data-rootN)"
		case "studies" return
			"collection($config:data-rootS)"
		case "persons" return
			"collection($config:data-rootPr)"
		case "places" return
			"collection($config:data-rootPl)"
		case "institutions" return
			"collection($config:data-rootIn)"
		case "authority-files" return
			"collection($config:data-rootA)"
...
declare function switch2:collectionVarValTit($type) {
	switch ($type)
		case "all" return
			"$exptit:col"
		case "mss" return
			'$exptit:col//t:TEI[@type="mss"]'
		case "work" return
			'$exptit:col//t:TEI[@type="work"]'
		case "nar" return
			'$exptit:col//t:TEI[@type="nar"]'
		case "studies" return
			'$exptit:col//t:TEI[@type="studies"]'
		case "auth" return
			'$exptit:col//t:TEI[@type="auth"]'
		case "pers" return
			'$exptit:col//t:TEI[@type="pers"]'
		case "place" return
			'$exptit:col//t:TEI[@type="place"]'
		case "ins" return
			'$exptit:col//t:TEI[@type="ins"]'
		default return
			"$exptit:col"
};
