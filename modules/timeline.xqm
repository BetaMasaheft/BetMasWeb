...
declare function tl:RestEntityTimeLine($this, $collection) {
	...
	let $data :=
		let $dateManuscripts :=
			...
			let $datesofcitingMss :=
				for $citingms in distinct-values($whatpointshere[ancestor::t:TEI[@type eq "mss"]]/ancestor::t:TEI/@xml:id)
				return doc(($config:data-rootMS || "/" || string($citingms) || ".xml"))//t:origDate[@when or
					(@notBefore or @notAfter)]
			for $date in ($dateofThisManuscript, $datesofcitingMss)
			return tl:date($date, "obj")

		let $dateInManuscripts :=
			let $dateinthisms := $this//t:date[@when or @notBefore or @notAfter]
			let $datesIncitingMss :=
				for $Incitingms in
					config:distinct-values($whatpointshere[ancestor::t:TEI[@type eq "mss"]]/ancestor::t:TEI/@xml:id)
				return doc(($config:data-rootMS || "/" || string($Incitingms) || ".xml"))//t:date[@when or
					@notBefore or
					@notAfter]
		...
		let $datePersons :=
			let $datesOfThisPerson := $this//t:person[t:birth[@evidence eq "internal"][@when or @notBefore or @notAfter] or
				t:death[@evidence eq "internal"][@when or @notBefore or @notAfter] or
				t:floruit[@evidence eq "internal"][@when or @notBefore or @notAfter]]
			let $datesofRelatedPersons :=
				for $ref in config:distinct-values($this//@ref[contains(., "PRS")])
				return doc(
					($config:data-rootPr || "/" || substring-after($ref, $config:BMurl) || ".xml")
				)//t:person[t:birth[@evidence eq "internal"][@when or @notBefore or @notAfter] or
					t:death[@evidence eq "internal"][@when or @notBefore or @notAfter] or
					t:floruit[@evidence eq "internal"][@when or @notBefore or @notAfter]]
			let $datesIncitingPrs :=
				for $citingpr in
					config:distinct-values($whatpointshere[ancestor::t:TEI[@type eq "pers"]]/ancestor::t:TEI/@xml:id)
				return doc(($config:data-rootPr || "/" || string($citingpr) || ".xml"))//t:person[t:birth[@evidence eq
					"internal"][@when or @notBefore or @notAfter] or
					t:death[@evidence eq "internal"][@when or @notBefore or @notAfter] or
					t:floruit[@evidence eq "internal"][@when or @notBefore or @notAfter]]
			...
			<links>
				{
					...
					return tl:link($dateManuscript, "obj", "e")
				}
				{
					for $datePerson in $datePersons
					return tl:link($datePerson, "prs", "e")
				}
			</links>
			<dates>
				{
					for $dateManuscript in ($dateManuscripts, $dateInManuscripts)
					return tl:dateObj($dateManuscript, "obj")
				}
				{
					for $datePerson in $datePersons
					return tl:dateObj($datePerson, "prs")
				}
			</dates>
		</all>

	return tl:js($data, "entity")
};
		...
		default return
			(
				'{id:"' ||
					...
					$date/location ||
					"_" ||
					substring(replace(tl:resp($date/resp), " ", ""), 1, 5) ||
					'",  group: "manuscripts", content: ' ||
					$date/item ||
					"_" ||
					$date/location ||
					"_" ||
...
declare function tl:link($date as node(), $mode as xs:string, $context as xs:string) {
	let $label := switch ($context)
		case "g" return
			('<a href="' || $date/item || "#" || $date/location || '">' || $date/name || "</a>")
		default return
			(
				'<a href="' ||
					$date/item ||
					...
					$date/name ||
					(
						if ($date/location) then
							", " || $date/location
						else (
						)
					) ||
					(
						if ($date/resp/text()) then
							" according to " || tl:resp($date/resp)
						else (
						)
					) ||
					(
						if ($date/evidence/text()) then
							" based on " || $date/evidence
						else (
						)
					) ||
					"</a>"
	...
	return switch ($mode)
		case "prs" return
			("var " || $date/item || "= '" || '<a href="' || $date/item || '">' || $date/name || "</a>'; ")
		case "work" return
			(
				"var " ||
					$date/item ||
...
declare function tl:date($date as node(), $mode as xs:string) {
	...
	let $tree := <date>
		<item>{ $rid }</item>
		<name>{ exptit:printTitleID($rid) }</name>
		<location>{ string($date/ancestor::t:*[@xml:id][1]/@xml:id) }</location>
		<evidence>{ string($date/@evidence) }</evidence>
		{
			if ($date/@resp) then
				for $d in tokenize($date/@resp, " ")
				return <resp>{ string($d) }</resp>
			else
				for $d in $date//t:bibl/t:ptr
				return <resp>{ replace(string($d/@target), ":", "_") }</resp>
		}
		{
			switch ($mode)
				case "prs" return
					if ($date[t:birth[@when or @notBefore or @notAfter] and t:death[@when or @notBefore or @notAfter]]) then
						<range>
							<from>
								{
									if ($date/t:birth/@notBefore) then
										string($date/t:birth/@notBefore)
									else if ($date/t:birth/@when) then
							...
							<to>
								{
									if ($date/t:death/@notAfter) then
										string($date/t:death/@notAfter)
									else if ($date/t:death/@when) then
						...
						<range>
							<from>
								{
									if ($date/t:death/@notBefore) then
						...
						<range>
							<from>
								{
									if ($date/t:floruit/@notBefore) then
							...
							<to>
								{
									if ($date/t:floruit/@notAfter) then
						...
						<range><from>{ string($date/t:floruit/@when) }</from></range>

					else (
					)
				default return
					if ($date[@notBefore and @notAfter]) then
						<range><from>{ string($date/@notBefore) }</from><to>{ string($date/@notAfter) }</to></range>
					else if ($date[@notBefore and not(@notAfter)]) then
						<from>{ string($date/@notBefore) }</from>
					else if ($date[@notAfter and not(@notBefore)]) then
						<from>{ string($date/@notAfter) }</from>
					else if ($date[@when]) then
						<from>{ string($date/@when) }</from>
