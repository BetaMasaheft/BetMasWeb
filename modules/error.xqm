	...
	switch ($name("type"))
		case "user" return
			<html xmlns="http://www.w3.org/1999/xhtml">
				<head />
				<body>
					<h1>Either you are not requesting your personal account or the account {
							$name("name")
						} is not an enabled account or you are not authenticated.</h1>
				</body>
			</html>
		case "xmlitem" return
			<html xmlns="http://www.w3.org/1999/xhtml">
				<head />
				<body><h1>{ $name("name") } is not an available item.</h1></body>
			</html>
		default return
			<html xmlns="http://www.w3.org/1999/xhtml">
				<head>
					...
					<div xmlns="http://www.w3.org/1999/xhtml" class="w3-container w3-card-4 w3-red w3-margin w3-padding-64">
						{
							switch ($name("type"))
										...
										<ul>
											{
												...
												return <li><a href="/{ $c }/list">{ $c }</a></li>
											}
										</ul>,
								...
								case "repo" return
									(
										<h1>{ $name("name") } is not the id af any available repository.</h1>,
										<p class="w3-large">Available repositories are listed <a href="/institutions/list">here</a>.</p>
									)
								case "catalogue" return
									(
										<h1>{ $name("name") } is not the id af any available catalogue.</h1>,
										<p class="w3-large">Available catalogues are the following:</p>,
										<ul>
											{
												for $catalogue in
