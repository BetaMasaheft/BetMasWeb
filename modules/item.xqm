...
declare function item2:printTitle($id) {
...
declare function item2:persList($item) {
	...
	return if (count($persons) le 0) then
		<p>No persons related to this manuscripts are known.</p>
	else
		<ul class=" w3-ul w3-hoverable">
			{
				for $pers in $item//t:persName[@role]
...
declare function item2:witList($item) {
	...
	<ul class=" w3-ul w3-hoverable">
		{
			for $wit in $item//t:witness[not(@type)]
			return <li class="nodot" id="{ string($wit/@xml:id) }">
				<a href="{ $wit/@corresp }" target="_blank">
					{
						if ($wit/@xml:id) then (
							<b class="lead">{ string($wit/@xml:id) }</b>, ":"
						) else (
						)
					}
					{ $wit/t:idno/text(), ", ", $wit/t:title/text() }
				</a>
			</li>
		}
		{
			for $wit in $item//t:witness[@type eq "external"]
			return <li class="nodot" id="{ string($wit/@xml:id) }">
				<a href="{ $wit/@facs }" target="_blank">
					{
						if ($wit/@xml:id) then (
							<b class="lead">{ string($wit/@xml:id) }</b>, ":"
						) else (
...
declare function item2:witnesses($id) {
	...
	return if ($item/@type eq "mss") then
		<div class="w3-bar-item" id="textWitnesses"><h5>Transcription of the manuscript</h5></div>
	else (
		<div class="w3-bar-item" id="textWitnesses"><h5>Witnesses of the edition</h5></div>,
		item2:witList($item),
		let $versions := $exptit:col//t:relation[@name eq "saws:isVersionOf"][contains(@passive, $id)]
		return if ($versions) then (
			<h5 class="w3-bar-item">Other versions</h5>,
			<ul class="w3-bar-item nodot">
				{
					for $parallel in $versions
					let $p := $parallel/@active
					return <li><a href="{ $p }">{ exptit:printTitle($p) }</a></li>
				}
			</ul>
		) else (
		),
		let $versionsO := $exptit:col//t:relation[@name eq "isVersionInAnotherLanguageOf"][contains(@passive, $id)]
		return if ($versionsO) then (
...
declare function item2:RestViewOptions($this, $collection) {
	...
	return <div xmlns="http://www.w3.org/1999/xhtml" class="w3-bar w3-small w3-padding w3-margin-top" id="options">
		<!--
 ...
 Item Navigation</a>
		...
		<div class="w3-bar-item w3-tooltip">
			<a
				...
				href="{ $config:appUrl }/{ $collection }/{ $id }/main"
				id="mainEntryLink"
			>Main view</a>
 ...
 Edit on GitHub
		...
		<div class="w3-bar-item w3-tooltip">
			...
			<span
				class="w3-text w3-tag itemoptiontooltip"
			>Click here to hide or show again the little arrows and small left pointing hands in this page.</span>
		</div>
		<!--
<div class="w3-bar-item w3-tooltip" >
...
</div>-->
		{
			if (($collection = "manuscripts") and ($document//t:relation[contains(@name, "sdc")])) then
				<div class="w3-bar-item w3-tooltip">
					<a
						class="w3-button w3-padding-small w3-gray"
						href="{ $config:appUrl }/{ $collection }/{ $id }/graph"
						id="GraphViewLink"
						target="_blank"
					><i>Syntaxe</i></a>
					<span
						class="w3-text w3-tag itemoptiontooltip"
					>See graphs of the information available. If the manuscript contains relevant information,
		...
		}
		{
			if (($document//t:placeName)) then
				<div class="w3-bar-item">
					<a
						class="w3-button w3-padding-small w3-gray"
						href="{ $config:appUrl }/IndexPlaces?entity={ string($document/@xml:id) }"
					>Places Index</a>
				</div>
			else (
			)
		}
		{
			if (($document//t:persName)) then
				<div class="w3-bar-item">
					<a
						class="w3-button w3-padding-small w3-gray"
						href="{ $config:appUrl }/IndexPersons?entity={ string($document/@xml:id) }"
					>Persons Index</a>
				</div>
			else (
			)
...
<span class="w3-text w3-tag itemoptiontooltip">Further visualization of relational information</span></div>
  -->
		<div class="w3-bar-item w3-container">
			...
			>Relations</a>
			<div
				class="w3-modal w3-card w3-display-container w3-margin-top"
				id="relations"
				style="vertical-align: top;max-height:600px;width:80%;"
			>
    ...
    <header class="w3-container w3-red">
      ...
      <h4>Attestations</h4>
...
</div>-->
		{
			if (
				(
					$collection = "works" or $collection = "narratives" or $collection = "studies" or $collection = "manuscripts"
				) and
					($document//t:body[t:div[@type = "edition"][t:ab or t:div[@type = "textpart"]]])
			) then (
				<div class="w3-bar-item w3-tooltip">
					<a
						class="w3-button w3-padding-small w3-red"
						href="{ $config:appUrl }{ ("/" || $collection || "/" || $id || "/text") }"
						target="_blank"
					>Text</a>
				...
				<div class="w3-bar-item w3-tooltip">
					<a
						class="w3-button w3-padding-small w3-gray"
						href="{ $config:appUrl }{ ("/" || $collection || "/" || $id || "/geoBrowser") }"
						target="_blank"
					>Map of places</a>
					<span
						class="w3-text w3-tag itemoptiontooltip"
					>See places marked up in the text using the Dariah-DE Geo-Browser</span>
				</div>
			) else (
			)
		}
		{
			if (
				($collection = "works" or $collection = "narratives" or $collection = "studies") and
					($document//t:body[t:div[@type = "translation"][t:ab or t:div[@type = "textpart"]]])
			) then (
				<div class="w3-bar-item w3-tooltip">
					<a
						class="w3-button w3-padding-small w3-red"
		...
		{
			if (
				$collection = "manuscripts" and
					($this//t:msIdentifier//t:idno[@facs][@n] or $this//t:msIdentifier//t:idno[starts-with(@facs, "http")])
			) then
				<div class="w3-bar-item w3-tooltip">
					<a
						class="w3-button w3-padding-small w3-gray"
		...
		}
		{
			if (
				$collection = "manuscripts" and
					($this//t:msIdentifier/t:idno[not(@facs)]) and
					$this//t:collection[. eq "Ethio-SPaRe"]
			) then
				<div class="w3-bar-item w3-tooltip">
					<a
						class="w3-button w3-padding-small w3-gray"
						href="{
							(
								"mailto:denis.nosnitsin@uni-hamburg.de?Subject=Request%20for%20images%20of%20Ethio-SPaRe%20Manuscript%20" ||
									$id
							)
						}"
						target="_blank"
					>Request Images from Ethio-SPaRe</a>
					<span
						class="w3-text w3-tag itemoptiontooltip"
					>Send an email to Ethio-SPaRe Project leader to request to make the images of this manuscript available here.</span>
				</div>
			else (
			)
		}
		{
			if ($collection = "manuscripts" and $this//t:facsimile/t:graphic) then
				...
				return <div class="w3-bar-item w3-tooltip">
					<a
						class="w3-button w3-padding-small w3-gray"
						href="{ $g/@url }"
						target="_blank"
...
declare function item2:RestItemHeader($this, $collection) {
	...
	let $id := string($this/@xml:id)
	let $repoids := if (
		$document//t:msIdentifier/t:repository/text() = "Lost" or $document//t:msIdentifier/t:repository/text() = "In situ"
	) then (
		$document//t:msIdentifier/t:repository/text()
	) else if ($document//t:msIdentifier/t:repository/@ref) then
		config:distinct-values($document//t:msIdentifier/t:repository/@ref)
	else
		"No Repository Specified"
	let $key :=
		for $ed in $document//t:titleStmt/t:editor[not(@role eq "generalEditor")]
		return editors:editorKey(string($ed/@key)) ||
			(
				if ($ed/@role) then
					" (" || string($ed/@role) || ")"
				else (
	...
	return <div xmlns="http://www.w3.org/1999/xhtml" class="ItemHeader w3-container">
		<div xmlns="http://www.w3.org/1999/xhtml" class="w3-twothird">
			<h3 id="headtitle">{ exptit:printTitleID($id) }</h3>
			{
				...
				return (
					if ($formerly) then
						<p>This record was formerly also listed as { string-join($formerly/@passive, ", ") }.</p>
					else (
					),
					if ($same) then
						for $s in $same
						return <p>This record is the same as <a href="{ string($s/@passive) }" target="_blank">
								{ exptit:printTitleID($s/@passive) }
							</a>.</p>
					else (
					)
				)
			...
			<p id="mainEditor"><i>{ string-join($key, ", ") }</i></p>
			{
				if ($collection = "manuscripts") then
					<p>
						{
							if ($this//t:additional//t:source/t:listBibl[@type eq "catalogue"]) then (
								"This manuscript description is based on ",
								<a href="#additionals">the catalogues listed in the Catalogue Bibliography</a>,
								if (
									$this//t:additional//t:source/t:listBibl[@type eq "catalogue"]/t:bibl/t:ptr[@target eq "bm:BmWebsite"]
								) then (
									" and has been corrected or enriched by the Bm project team; please check our ",
									<a href="#footer">Revision history for more information</a>
								) else (
								)
							) else if ($this//t:collection = "EMIP") then
								string-join($this//t:collection//text(), ", ")
							else if (contains($this//t:funder, "IslHornAfr")) then (
								"Newly catalogued in IslHornAfr, see also ",
								<a
									href="http://islhornafr.tors.sc.ku.dk/backend/manuscripts/{
										format-number(number(replace($id, "IHA", "")), "####")
									}"
								>IslHornAfr manuscript { format-number(number(replace($id, "IHA", "")), "####") }</a>
							) else if (contains($this//t:idno, "EMML")) then
								string:tei2string($this//t:editionStmt)
							else
		...
		<div xmlns="http://www.w3.org/1999/xhtml" class="w3-third">
			<div class="w3-container" id="general">
				<div class="w3-row">
					{
						if ($document//t:change[not(@who eq "PL")]) then
							if (count($document//t:change[not(@who eq "PL")]) eq 1) then
								...
								<span class="w3-tag w3-gray w3-small">Last edit: {
										let $last := max(
											for $date in $document//t:change/@when
											return xs:date($date)
										)
										return format-date($last, "[MNn] [D], [Y]", "en", (), ())
									}
								</span>
						else
							<span class="w3-tag w3-red">Stub</span>
				...
				<div class="w3-row w3-hide-small">
					<span class="w3-tag w3-gray w3-small" style="word-break:break-all;">{ $config:appUrl || "/" || $id }</span>
				</div>
				{
					switch ($collection)
						case "manuscripts" return
							if ($document//t:repository/text() = "Lost" or $document//t:repository/text() = "In situ") then
								<div class="w3-row">
									<span class="w3-tag w3-gray w3-large w3-margin-top">{ $document//t:repository/text() }</span>
									<p class="w3-large">Collection:  { string-join($document//t:msIdentifier/t:collection, ", ") }</p>
									{
										if ($document//t:altIdentifier) then
											<p>Other identifiers: {
													...
													return if ($altId/@type eq "TM") then
														<a
															content="{ $altId }"
															href="https://www.trismegistos.org/text/{ $altId/text() }"
															property="http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by"
														>TM{ $altId/text() }
															{
																if ($altId[$p = count($document//t:msIdentifier/t:altIdentifier/t:idno/text())]) then
																	" "
																else
																	", "
															}
								...
								<div>
									{
										...
										let $repoplace := if ($repodoc//t:settlement[1]/@ref) then
											exptit:printTitleID($repodoc//t:settlement[1]/@ref)
										else if ($repodoc//t:settlement[1]/text()) then
											$repodoc//t:settlement[1]/text()
										else if ($repodoc//t:country[1]/@ref) then
											exptit:printTitleID($repodoc//t:country[1]/@ref)
										else (
										)
										return (
											<a
												class="w3-tag w3-gray w3-large w3-margin-top"
												href="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;reporef={
													if (starts-with($repo, $config:BMurl)) then
														substring-after($repo, $config:BMurl)
													else
														$repo
												}"
											...
											>
												...
												{ string($this//t:msIdentifier/t:repository/text()) }
											</a>,
											<a
												href="/{
													if (starts-with($repo, $config:BMurl)) then
											...
											<p>Other identifiers: {
													...
													return if ($altId/@type eq "TM") then
														...
														<span
															...
															property="http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by"
														>
															{ concat(string($altId/ancestor::t:altIdentifier[1]/t:repository), " ", $altId/text()) }
															{
																if ($altId[$p = count($document//t:msIdentifier/t:altIdentifier/t:idno/text())]) then
																	" "
																else
						...
						case "persons" return
							if (starts-with($document//t:person/@sameAs, "wd:")) then
								wiki:wikitable(substring-after($document//t:person/@sameAs, "wd:"))
							else (
								string($document//t:person/@sameAs)
							)
						case "works" return
							apptable:clavisIds(root($document))
						case "institutions" return
							<div class="w3-row">
								<a class="w3-tag w3-red" href="{ $config:appUrl }/institutions/" role="label">Institution</a>
								{
									if ($document//t:place/@type) then
										let $type := data($document//t:place/@type)
										let $list := if (contains($type, " ")) then
											tokenize(normalize-space($type), " ")
										else
											string($type)
										return (
											<div>
												{
													...
													return <a
														class="w3-tag w3-red"
														href="{ $config:appUrl }/places/list?placetype={ $t }"
														target="_blank"
													>{ $t }</a>
												}
											</div>,
											if (starts-with($document//t:place/@sameAs, "wd:")) then
												wiki:wikitable(substring-after($document//t:place/@sameAs, "wd:"))
											else (
												string($document//t:place/@sameAs)
											)
										)
									else (
									)
            ...
            w3-margin-top"
                  ...
                  Manuscripts in { exptit:printTitleID($id) }</a>
							...
							if ($document//t:personGrp) then
								<div class="w3-row">
									<span class="w3-tag w3-red">
										{
											if ($document//t:personGrp[@role eq "ethnic"]) then
												"Ethnic/Linguistic"
											else (
											)
										}
						...
						case "work" return
							if ($document//t:titleStmt/t:author) then
								<div class="w3-row">
									<p class="w3-large">
										<a href="{ $document//t:titleStmt/t:author[1]/@ref }">{ $document//t:titleStmt/t:author[1] }</a>
									</p>
								</div>
							else (
							)
...
declare function item2:AdminLocTable($adminLoc as element()*) {
	...
	return <tr>
		<td>
			{
				if ($s/@type) then
					exptit:printTitleID($s/@type/data())
				else
					$s/local-name()
			}
		</td>
		<td>
			{
				if ($s/@ref) then (
					<a
						href="{ $config:appUrl }/{
							if (starts-with($s/@ref, $config:BMurl)) then
								substring-after($s/@ref, $config:BMurl)
							else
								$s/@ref
						}"
						target="_blank"
...
declare function item2:RestPersRole($file, $collection) {
	...
	return if ($collection = "persons") then (
		<div class="w3-panel w3-margin w3-card-4">
			{
				...
				return if ($persrole) then
					...
					return <div>
						{ exptit:printTitle($id) } is  { string($r) }  of the following items
             <ul
								...
								return <li>
									<a href="{ $thisid }">{ exptit:printTitle($thisid) }</a>
									{
										if ((count($root//t:origDate[@evidence = "internal-date"]) ge 1)) then
											let $dates :=
												for $date in $root//t:origDate[@evidence = "internal-date"]
												return string:tei2string($date)
											return " which contains the following internal dates: " || string-join($dates, ", ") || "."
										else (
										)
									}
									{
										if ((count($root//t:msDesc//t:date) ge 1)) then
											let $dates :=
												for $date in $root//t:msDesc//t:date
												return (string:tei2string($date) || " in a " || $date/parent::t:*/name() || " element")
											return " The description of the manuscript also contains the following dates " ||
												string-join($dates, ", ") ||
												"."
										else (
										)
		...
		let $notnull := $file//t:persName[@ref != "PRS000000" and
			@ref != "PRS00000" and
			@ref != "PRS0000" and
			@ref != "PRS000"]
		let $pers := $notnull[@role]
		return for $p in $pers

			group by $ID := $p/@ref

			return <div class="w3-panel w3-margin w3-gray w3-card-4">
				<a href="{ $ID }">{ exptit:printTitle($ID) }</a>
    is <span class="w3-tag w3-red">
					{
						for $role in config:distinct-values($p/@role)
						return string($role) || " "
					}
				</span>
				{ " of this manuscript" }.

    {
					...
					return <ul>and is also <span
							class="w3-red w3-tag role"
							onclick="document.getElementById('{ $r }list').style.display='block'"
						>{ string($r) }</span>
						<div>
							<div class="w3-modal " id="{ $r }list">
								<div class="w3-modal-content">
									<button
										class="w3-button w3-red"
										onclick="document.getElementById('{ $r }list').style.display='none'"
										type="button"
									>Close</button>
									<header><h4 class="w3-margin" id="{ $r }listcount">There are other</h4></header>
									<div class="w3-container"><ul class="w3-ul w3-hoverable" id="{ $r }listitems" /></div>
								</div>
							</div>
						</div> of :
                ...
                {
							for $root in $role/ancestor::t:TEI[@xml:id != $id]
							return <li><a href="{ string($root/@xml:id) }">{ exptit:printTitle(string($root/@xml:id)) }</a></li>
						}
					</ul>
				}
			</div>
...
declare function item2:RestAdditions($id) {
	...
	let $sameKey :=
		...
		return $corr
	return if ($sameKey) then
		<div class="w3-panel w3-card-4 w3-margin w3-gray" id="InAdditions">
			<h4 class="modal-title">
				{ count($sameKey) } Addition{
					if (count($sameKey) gt 1) then
						"s"
					else (
					)
				} name{
					if (count($sameKey) gt 1) then (
						...
						"s"
				} person <span>{ exptit:printTitle($id) }</span>
			</h4>
			<div id="InAdditions{ $id }"><ul>{ apprest:referencesList($id, $sameKey, "name") }</ul></div>
		</div>
	else (
	)
};
	...
	return if ($sameKey) then
		<div class="w3-panel w3-card-4 w3-margin w3-gray" id="InMiniatures">
			...
			<div id="InMiniatures{ $id }">
				<ul class="w3-ul w3-hoverable">{ apprest:referencesList($id, $sameKey, "name") }</ul>
			</div>
		</div>
	else (
	)
...
declare function item2:RestMiniaturesKeys($id) {
	let $adds := $apprest:collection-rootMS//t:decoNote[@type eq "miniature"][descendant::t:ref[@type eq
		"authFile"][@corresp eq $id]]
	...
	return <ul class="w3-ul w3-hoverable">
		{
			...
			return <li class="nodot">
				<a
					...
					href="{ $config:appUrl }/authority-files/list?keyword={ $K }"
					role="button"
					target="_blank"
				>
					{ exptit:printTitleID($K) }
				</a>
                                               in
                                               <ul>
					{
						...
						return <li>
							<a href="{ $config:appUrl }/{ $containerID }" target="_blank">{ exptit:printTitleID($containerID) }</a>
						</li>
					}
				</ul>
			</li>
			...
			<h4 class="modal-title">
				{ count($sameKey) } place record{
					...
					if (count($sameKey) gt 1) then (
						...
						"s"
				} person <span>{ exptit:printTitle($id) }</span> as a tabot</h4>
			<div id="Tabot{ $id }"><ul class="w3-ul w3-hoverable">{ apprest:referencesList($id, $sameKey, "name") }</ul></div>
		</div>
	else (
	)
};
...
declare function item2:RestMss($id) {
	...
	let $distinctMssIds :=
		for $s in ($sameKey, $sameKeyAdd)
		return string($s/ancestor::t:TEI/@xml:id)
	let $countDistMss := count(config:distinct-values($distinctMssIds))
	return <div class="w3-panel w3-margin w3-red w3-card-4" id="computedWitnesses" style="word-break:break-all;">
		<span class="scrollwrap nodot w3-small">
			<h5 class="openInDialog">This unit, or parts of it, is contained in { $countDistMss } manuscript records {
					$count
				} time{
					if ($count gt 1) then
						"s"
			...
			<p>
				<a
					href="{
						$config:appUrl
					}/newSearch.html?searchType=text&amp;mode=any&amp;work-types=mss&amp;titletext=https://betamasaheft.eu/{
						$id
					}"
					target="_blank"
				>See these {
						$countDistMss
					} manuscripts in the list view.</a>
Scrolling in this box will also show you a summary of all the occurences.  <a
					href="{ $config:appUrl }/compare?workid={ $id }"
					target="_blank"
				>See also compare view.</a>
			</p>
			<div class="w3-small" id="Samekeyword{ $string }">
				{
						...
						<ul class="nodot w3-padding">
							{
								...
								return <li class="w3-bar w3-card-2 list-group">
									{
									...
									<a class="w3-bar-item" href="{ $config:appUrl }/manuscripts/{ $groupkey }/main">
										{ $tit } ({ string($groupkey) }) </a>
									<div class="w3-bar-item">
										<!--  <span class="WordCount w3-tag" data-msID="{$groupkey}" data-wID="{$string}"/> -->
										<ul class="w3-padding">
											{
												for $h in $hit
												let $msitem := $h/parent::t:msItem
												return <li>content item with id <b>{ string($msitem/@xml:id) }</b>
													{
														if ($h/text()) then (
															", ", <i>{ $h/text() }</i>
														) else (
						...
						<ul class="nodot  w3-padding ">
							{
								...
								order by $tit[1]
									...
									(: order by root($hit)/t:TEI/@xml:id :)
								return <li class="w3-bar w3-card-2 list-group">
									<a href="{ $config:appUrl }/manuscripts/{ $groupkey }/main">{ $tit } ({ string($groupkey) }) </a>
									<br />
									...
									<ul class="w3-padding">
										{
											...
											let $position := " [" ||
												count($item/preceding-sibling::t:item) + 1 ||
												"/" ||
												count($item/parent::t:*/child::t:item) ||
												"]"
											return <li>addition with id <b>{ string($item/@xml:id) }</b>
												{
													if ($h/text()) then (
														", ", <i>{ $h/text() }</i>
													) else (
				...
				}
				{
					if (starts-with($id, "NAR")) then (
						...
						return (
							<p>As additional content associated with the keyword <b>
									<a href="{ $config:appUrl }/authority-files/list?keyword={ string($broad) }">
										{ exptit:printTitleID($broad) }
									</a>
								</b> this unit
  is present an additional { count($usedasType) } time{
									if (count($usedasType) gt 1) then
										"s"
									else (
									)
								}
			...
			</div>
			{
				...
				return if (count($corresps) ge 1) then (
					for $c in $corresps
					let $workid := string(root($c)/t:TEI/@xml:id)
					let $witnesses := $apprest:collection-rootMS//t:title[contains(@ref, $workid)][parent::t:msItem or
						ancestor::t:additions]
					let $countwitnesses :=
					...
					return (
						<div class="w3-small scrollwrap" id="Samepart{ $string }">
							<p>This textual unit is also part of <a href="{ $config:appUrl }/{ $workid }" target="_blank">
									{ $tit }
								</a>, which is contained in the following { count($countwitnesses) } manuscripts {
									count($witnesses)
								} times:</p>,
<ul class=" w3-padding nodot">
								{
									...
									return <li><a href="{ $config:appUrl }/{ $id }" target="_blank">{ $wtit }</a></li>
								}
							</ul>
						</div>
					)
			...
			}
			{
				...
				let $relformspart := $exptit:col//t:relation[(@name eq "saws:formsPartOf") or
					(@name eq "ecrm:CLP46i_may_form_part_of")][starts-with(@passive, $id)]/@active
				let $all := ($parts, $rels, $relformspart)
				let $ids := distinct-values($all)
				return if (count($ids) ge 1) then (
					<p>This textual unit has also {
							count($ids)
						} parts which are also independent Textual Units. 
 Below is a list of witnesses for each of them.</p>,
					for $c in $ids
					let $witnesses := $apprest:collection-rootMS//t:title[contains(@ref, $c)][parent::t:msItem or
						ancestor::t:additions]
					let $countwitnesses :=
						for $wit in $witnesses
						let $wid := string(root($wit)/t:TEI/@xml:id)
					...
					let $cid := if (starts-with($c, $config:BMurl)) then
						substring-after($c, $config:BMurl)
					else
						$c
					let $tit := exptit:printTitleID($cid)
					let $subids :=
						for $subid in $work//t:div[@type eq "textpart"][@corresp eq $c]/@xml:id
						return $id || "#" || string($subid)
					let $stringsubids := string-join($subids, ",")

					return if (count($witnesses) ge 1) then (
						<div class="w3-small scrollwrap" id="Samepart2{ $string }">
							<p>
								{ $stringsubids } is listed as <a href="{ $c }" target="_blank">{ $c }</a> in <a
									href="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=mss&amp;{ $c }"
									target="_blank"
								>{ count($countwitnesses) } manuscripts { count($witnesses) } times</a>:</p>
							<ul class="nodot w3-padding">
								{
									for $wit in $witnesses
									let $wid := string(root($wit)/t:TEI/@xml:id)
									...
									let $wtit := exptit:printTitleID($id)
									return <li class="w3-bar w3-card-2 list-group">
										<a href="{ $config:appUrl }/{ $id }" target="_blank">{ $wtit }</a>
									</li>
								}
							</ul>
							{
								if ($work//t:div[@type eq "textpart"][@corresp = $c]) then
									let $subids :=
										for $subid in $work//t:div[@type eq "textpart"][@corresp eq $c]/@xml:id
										return $id || "#" || string($subid)
									let $stringsubids := string-join($subids, ",")
									return <p>
										<a
											href="{ $config:appUrl }/compare?workid={ $stringsubids },{ $c }"
											target="_blank"
										>Click to compare manuscripts of both { $stringsubids } and { $c }.</a>
									</p>
								else (
								)
							}
						...
						<p>
							<a href="{ $config:appUrl }/{ $cid }" target="_blank">{ $tit }</a> is listed also as {
								$c
							}, but not recorded with this reference in any manuscript at the moment.</p>
				) else (
				)
			}
		</span>
	...
	return <div class="w3-third w3-padding" id="seeAlsoForm">
			...
			if (count($options) ge 1) then (
				...
				<form action="" class="w3-container">
					<div class="w3-container">
						<select class="w3-select w3-border" id="seealsoSelector" name="seealso">
							<option>select...</option>
							{ $options }
						</select>
					</div>
				</form>,
				<div class="w3-container w3-margin w3-gray" id="SeeAlsoResults">No keyword selected.</div>
			) else
		...
		}
		{
			if ($collection = "institutions") then
				...
				return if (count($mss) ge 1) then
					<div
						class="w3-panel w3-card w3-red"
						style="word-break:break-all;max-height:  350px;overflow-y: scroll;"
					>
                The dataset contains { count($mss) }
						<a
							href="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;reporef={ $id }"
						> records for manuscripts at this repository</a>.
                 {
							if (count($mss) lt 50) then
								...
								<span>These are too many to be shown here, please follow the <a
										href="/newSearch.html?searchType=text&amp;mode=any&amp;reporef={ $id }"
									> link</a> to see the full list.</span>
						}
					</div>
				else (
				)
		...
		}
		{
			if ($collection = "places" or $collection = "institutions") then
				<div>
					<div class="w3-container" data-id="{ $id }" id="pelagiosrelateditems">
						{
							if ($file//t:place/@sameAs) then
								attribute data-sameAs { string($file//t:place/@sameAs) }
							else (
							)
						}
					...
					<script src="resources/js/gnisci.js" type="text/javascript" />
				</div>
			else (
			)
		}
		{
			if ($collection = "authority-files" and $file//t:relation[starts-with(@passive, "ic")]) then
				<div class="w3-container" id="EuropeanaMatches">
					{
						for $iconclass in $file//t:relation[starts-with(@passive, "ic")]
						let $icID := substring-after($iconclass/@passive, "ic:")
						let $europeanalink := (
							"http://sparql.europeana.eu/?default-graph-uri=http%3A%2F%2Fdata.europeana.eu%2F&amp;query=SELECT+%3FProvidedCHO%0D%0AWHERE+%7B%0D%0A++%3FProxy+%3Fproperty+%3Chttp%3A%2F%2Ficonclass.org%2F" ||
								$icID ||
								"%3E+%3B%0D%0A+++++++++ore%3AproxyIn+%3FAggregation+.+%0D%0A++%3FAggregation+edm%3AaggregatedCHO+%3FProvidedCHO%0D%0A%7D&amp;format=text%2Fhtml&amp;timeout=0&amp;debug=on"
						)
						return <div data-value="{ string($iconclass/@passive) }">
   Items  linked to <a
								href="http://iconclass.org/{ $icID }"
							>Iconclass concept { $icID }</a>:
   <ul>
								<li><a href="{ $europeanalink }">Click to see items in Europeana</a></li>
								<li>
									<a
										href="https://corpusvitrearum.de/id/{ $icID }/about.html"
									>Click to see items in Corpus Vitrearum Medii Aevi </a>
								</li>
							</ul>
							<script src="resources/js/europeanaSparql.js" type="application/javascript" />
						</div>
					}
				</div>
			else (
