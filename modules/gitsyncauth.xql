...
(:~
 ...
 :
 :
 : slightly modified to serve only authority-files repo for BetaMasaheft
 :
...
return if (not(empty($post-data))) then
	...
	return try {
		if (matches(request:get-header("User-Agent"), "^GitHub-Hookshot/")) then
			if (request:get-header("X-GitHub-Event") = "push") then
				...
				let $private-key := environment-variable("GIT_SECRETa")
				let $actual-result := <actual-result>
				...
				<response status="fail"><message>Invalid trigger.</message></response>
		else
			<response status="fail"><message>This is not a GitHub request.</message></response>
	} catch * {
	...
	<response status="fail"><message>No post data recieved</message></response>
