		...
		if (starts-with($node/@target, "#")) then
			...
			let $title := switch ($nodeName)
				case "persName" return
					if ($r//t:persName[@type eq "normalized"][contains(@corresp, $anchor)]) then
						string-join($r//t:persName[@type eq "normalized"][contains(@corresp, $anchor)]//text(), "")
					else
				...
				case "div" return
					if ($item[@type eq "edition"]) then (
						"edition " ||
							(
								if ($item[@resp]) then (
									let $respsource := string($item/@resp)
									let $resp := if (starts-with($respsource, "#")) then (
										$anchor
									) else
										"by " || editors:editorKey($respsource)
									return $resp
								) else
									$anchor
							)
				...
				case "title" return
					"title" || ($item/@xml:lang || $item)
				case "handNote" return
					"hand " || $anchor
				case "decoNote" return
					"decoration " || $anchor
				case "layout" return
					"layout note " || $anchor
				default return
...
declare function string:tei2string($nodes as node()*) {
	...
	return typeswitch ($node)
		(: case text()
			...
			<a href="{ string($node/@href) }">{ $node/text() }</a>
		case element(i) return
			<i>{ $node/text() }</i>
		case element(t:listBibl) return
			<div><h4>Bibliography</h4>{ string:tei2string($node/node()) }</div>
