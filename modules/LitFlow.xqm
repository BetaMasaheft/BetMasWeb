...
declare function LitFlow:compareGroups($groups, $g1 as xs:string*, $g2 as xs:string*) {
	...
	for $kG1 in
		$groups//local:group[@key eq $g1]/local:keysGroup
...
(nk2*sk) - (nk1*sk). a positive difference is an increase in diversity, 0 is the exact same, a negative difference is a descrease in diversity :)
		...
		return <pair xmlns="http://local.local">
			<name>{ $kG2Name }</name>
			<tot>{ string($t2) }</tot>
...
declare function LitFlow:Sankey($filter, $type) {
	let $AdditionsTypes := doc(concat($config:data-rootA, "/taxonomy.xml"))//t:category[t:desc eq
		"Additiones"]//t:category/t:catDesc
	...
	let $Subjects := doc(concat($config:data-rootA, "/taxonomy.xml"))//t:category[t:desc eq
		"Subjects"]//t:category/t:catDesc

	let $Periods := doc(concat($config:data-rootA, "/taxonomy.xml"))//t:category[t:desc eq
		"Periods"]//t:category/t:catDesc
	...
	let $works :=
		...
		return <work mss="{ $root }">{ $key/@key }{ $WorkInDMS }</work>
	...
	let $GuestText :=
		...
		return <work>{ $key/@key }{ $WorkInDMS }</work>

	let $datedWorks := if ($type = "works") then
		$DW
	else (
		$works, $GuestText
	)
	...
	let $groups := <groups>
		{
			...
			group by $period
			return <group xmlns="http://local.local" total="{ count($datedW) }">
				{ $period }
				{
					for $W in $datedW
					let $id := string($W/@ref)

					let $root := if ($type = "works") then
						root($W)[descendant::t:term[@key eq $filter]]
					else
						collection($config:data-rootW)/id($id)[descendant::t:term[@key eq $filter]]

					let $keywords :=
						for $k in $root//t:term
						where $k/@key = $Subjects
						order by $k/@key descending
						return string($k/@key)
					let $keywordSequence := replace(string-join($keywords, "-"), "\s+", "")
					group by $keywordSequence
					return if (string-length($keywordSequence) le 0) then (
					) else
						<keysGroup total="{ count($W) }">
							{ $keywordSequence }
							{
								if (contains($keywordSequence, "-")) then
									...
									<key>{ $keywordSequence }</key>
							}
							{
								for $work in $W
								let $id := if ($type = "works") then
									string(root($work)/t:TEI/@xml:id)
								else
									string($work/@ref)
								let $ms := string($work/@mss)
								return <work><id>{ $id }</id><mss>{ $ms }</mss></work>
							}
						</keysGroup>
				}
			</group>
	...
	let $edges := (
		LitFlow:compareGroups($groups, "Aks", "Paks1"),
...
"
		...
		<div class="w3-panel  w3-red w3-card-2 w3-margin">
			{
				if ($type = "works") then
					<p class="w3-large">Work records grouped by period and set of keywords.</p>
				else
					<p
						class="w3-large"
					>Selection of works linked from first level msItems in manuscripts records.
		...
		<div class="w3-container">
			{
				...
				let $order := switch ($per)
					...
					default return
						0
				order by $order
				return <div class="w3-col" style="width:10%;max-height:600pt; overflow:auto">
					<h2>{ $per }</h2>
					{
								...
								<tbody>
									{
										...
										return <tr>
											<td>{ $p }</td>
											<td>
												<a href="/{ $work/local:id/text() }" target="_blank">
													{ exptit:printTitle($work/local:id/text()) }
												</a>
												{
													if ($type = "works") then (
													) else (
														" in ",
														<a href="/{ $work/local:mss/text() }" target="_blank">
															{ exptit:printTitle($work/local:mss/text()) }
														</a>
													)
												}
											</td>
