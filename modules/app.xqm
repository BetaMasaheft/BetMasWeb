...
declare function app:showFacets($node as node()*, $model as map(*)) {
	...
	return <form action="" class="w3-container w3-center" id="facetsSearch">
		<div class="w3-row w3-left-align">
			<button class="w3-button w3-block w3-left-align w3-red" type="submit">refine search results <i
					class="fa fa-search" />
			</button>
			<input hidden="hidden" name="query" value="{ request:get-parameter("query", ()) }" />
		</div>
...
declare function app:facetGroup($group, $groupname, $subsequence) {
	<div>
		<div class="w3-row w3-left-align w3-margin-top"><b>{ $groupname }</b></div>
		{
			...
			order by $facetTitle
			return app:facetDiv($f, $facets, $facetTitle)
		}
	</div>
};
						...
						function ($label, $count) {
							<span>
									...
									value="{ $label }" />
								{
									...
									else if ($f = "changeWho") then
										editors:editorKey($label)
									else if ($f = "languages") then
										$app:languages//t:item[@xml:id eq $label]/text()
									else
										$label
								}
...
declare function app:sortingkey($input) {
