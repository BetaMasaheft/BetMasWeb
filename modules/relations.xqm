...
module namespace rels = "https://www.betamasaheft.uni-hamburg.de/BetMasWeb/rels";
...
declare function rels:EntityRels($node as node(), $model as map(*)) {
	...
	let $all :=
		...
		let $secondaryrelations :=
			for $ref in
				(
					$entity//@ref[. != $localId][not(parent::t:repository)],
					$entity//@active[. != $localId],
					$entity//@passive[. != $localId]
				)
		...
		let $data := (: collects all available ids :) (: the first entry makes sure there is the id of the current item :)
		<allIDS>
			<id type="entry">{ string($localId) }</id>
			{
				...
				let $refid := if ($pointerRoot/@ref) then
					$pointerRoot/@ref
				else if ($pointerRoot/@corresp) then
					$pointerRoot/@corresp
				else if ($pointerRoot/@passive) then
					$pointerRoot/@passive
				else if ($pointerRoot/@mutual) then
					$pointerRoot/@mutual
				else
					$pointerRoot/@active

				return (: first return the root of the referring entity and the id in the corresp, active, passive, mutual, etc. there. :) <data
				>
					<ids type="wph">
			...
			}
			{
				...
				let $container := if ($msItem//ancestor::t:msPart[1]) then
					$msItem//ancestor::t:msPart[1]
				else
					$msItem//ancestor::t:TEI
				let $titleId := if (contains($msItem/@ref, "#")) then
					substring-before($msItem/@ref, "#")
				else
					$msItem/@ref
				return <data>
					<ids type="title">
						<id>
							{ string($localId) }
							{
								if ($container/name() = "msPart") then (
									"#" || string($container/@xml:id)
								) else (
								)
							}
						</id>
						<id>{ string($msItem/@ref) }</id>
					</ids>
					<relations>
						<edge>
							{
			...
			}
			{
				...
				return <data>
					<ids type="div">
						...
						{
							if ($parent) then
								<id type="parent">{ string($localId) }#{ string($parent/@xml:id) }</id>
							else (
							)
						}
					</ids>
						...
						<edge>
							{
								(
									'{from:"' ||
										...
										'", to:"' ||
										(
											if ($singlePersWithRole/@corresp) then (
												string($singlePersWithRole/@corresp)
											) else
												$localId
										) ||
										'", label:"' ||
										("betmas:" || string($singlePersWithRole/@role)) ||
										""", font: {align: 'top'}},"
								)
							}
						</edge>
					...
					<relations>
						{
							...
							let $edges := if (contains(normalize-space($mutual), " ")) then
								...
								return <edge>
									{
										(
											'{from:"' ||
												...
												($eachID[2]) ||
												'", label:"' ||
												substring-after(string($relation/@name), ":") ||
												""", font: {align: 'top'}},"
										)
									}
								</edge>
							else if (contains(normalize-space($active), " ")) then (
								for $eachID in tokenize(normalize-space($active), " ")
								return <edge>
									{
										(
							...
							) else if (contains(normalize-space($passive), " ")) then (
								for $eachID in tokenize(normalize-space($passive), " ")
								return <edge>
									{
										(
		...
		let $nodes := <ids>
			{
				...
				let $collection := switch (substring($distinctId, 0, 4))
					case "LOC" return
						"places"
					case "LIT" return
						"works"
				...
				let $filetitle := try {
					let $title := if (contains($distinctId, "bm")) then (
						rels:getZoteroTextData($distinctId)
					(: could be made to look at the title in zotero :)
					) else if (starts-with($distinctId, "gn:")) then (
						exptit:getGeoNames($distinctId)
...
(\:                          consider if there is an id, and if this has a title which can be used as label :\)
						...
						normalize-space(
							exptit:printTitle(
								$exptit:col/id(
									if (contains($distinctId, "#")) then
										substring-before($distinctId, "#")
									else
										$distinctId
								)
							)
							...
							(
								if (contains($distinctId, "#")) then
									substring-after($distinctId, "#")
								else (
								)
							)
					)

					let $substring := if (string-length(replace($title, '"', "")) gt 20) then
						concat(substring(replace($title, '"', ""), 0, 20), "...")
					else
						replace($title, '"', "")
					return $substring
				} catch * { $distinctId }
		...
		return <all><nodes>{ $nodes//node/text() }</nodes><edges>{ $data//edge/text() }</edges></all>
...
declare function rels:EntityRelsTable($node as node(), $model as map(*)) {
	<table xmlns="http://www.w3.org/1999/xhtml" class="table table-hover" width="100%">
		...
		<tbody>
			{
				...
				return <tr>
					<th>
						{
							...
							for $id in $list//id
							return <a href="{ concat("https://betamasaheft.eu/", $id) }">{ exptit:printTitle($id) }</a>
						}
					</th>
					<th>{ data($relation/@name) }</th>
					<th>
						{
					...
					<th>{ viewItem:relations($relation/t:desc) }</th>
				</tr>
			}
		</tbody>
	</table>
