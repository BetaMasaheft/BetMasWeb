...
declare function gitsync:rdf($collection-uri, $file-name) {
	...
	let $shortCollName := if (contains($collectionName, "works") or contains($rdffilename, "LIT")) then
		"works"
	else if (contains($collectionName, "studies") or contains($rdffilename, "STU")) then
		"studies"
	else if (contains($collectionName, "narratives") or contains($rdffilename, "NAR")) then
		"narratives"
	else if (contains($collectionName, "persons") or contains($rdffilename, "PRS")) then
		"persons"
	else if (contains($collectionName, "places") or contains($rdffilename, "LOC")) then
		"places"
	else if (contains($collectionName, "institutions") or contains($rdffilename, "INS")) then
		"institutions"
	else if (contains($collectionName, "manuscripts") or contains(doc($file-name)//t:TEI/@type, "mss")) then
		"manuscripts"
	else if (contains($collectionName, "authority") or contains(doc($file-name)//t:TEI/@type, "auth")) then
		"authority-files"
	else (
	)
	...
	return "stored RDF/XML and updated fuseki"
};
...
declare function gitsync:updateinstitutionsMOD($file-name) {
	...
	return "updated institutions.xml static list"
};
...
declare function gitsync:updateinstitutionsDEL($file-name) {
	...
	return "removed value from list in institutions.xml"
};
...
declare function gitsync:updateinstitutionsADD($file-name) {
	...
	return "added value at the end of the list in institutions.xml"
};
...
declare function gitsync:updatepersonsMOD($file-name) {
	...
	return "updated persNamesLabels.xml static list with " || $t || " for item with id = " || $id || ". "
};
...
declare function gitsync:updatepersonsDEL($file-name) {
	...
	return "removed value from list in persNamesLabels.xml "
};
...
declare function gitsync:updateplacesMOD($file-name) {
	...
	return "updated placeNamesLabels.xml static list"
};
...
declare function gitsync:updateplacesDEL($file-name) {
	...
	return "removed value from list in placeNamesLabels.xml "
};
...
declare function gitsync:updatetextpartsMOD($file-name) {
	...
	let $update := if ($textslist/t:item[@corresp eq $id]) then
		...
		update insert <item xmlns="http://www.tei-c.org/ns/1.0" change="addedAt{ current-dateTime() }" corresp="{ $id }">
			{ $newtitleSelector }
		</item> into $textslist
...
declare function gitsync:do-update($commits, $contents-url as xs:string?, $data-collection) {
	...
	return for $modified in $commits?*?modified?*
		...
		let $req := <http:request href="{ xs:anyURI($file-path) }" http-version="1.1" method="GET">
		...
		return (
			try {
				...
				util:log("INFO", concat("updated BetMasData ", $file-name))
			} catch * {
				(
			...
			try {
				gitsync:updateExpanded($collection-uri, $file-name), util:log("INFO", concat("updated expanded ", $file-name))
			} catch * {
				(
					util:log(
						"INFO",
						concat("Failed to update expanded resource ", $file-name, ": ", $err:code, ": ", $err:description)
					)
				)
			},
...
:  because the app needs to remain in sync with the GIT repo. Yes, invalid data has to be allowed in...
			...
			try {
				gitsync:updateLists($data-collection, $file-name), util:log("INFO", concat("updated lists ", $file-name))
			} catch * {
				(util:log("INFO", concat("Failed to update lists ", $file-name, ": ", $err:code, ": ", $err:description)))
			},
			(: then update the RDF repository :)
			try { gitsync:rdf($collection-uri, $file-name), util:log("INFO", concat("updated rdf ", $file-name)) } catch * {
				(util:log("INFO", concat("Failed to update rdf ", $file-name, ": ", $err:code, ": ", $err:description)))
			},
			(: and finally check the ids for wrong anchors :)
			try {
				gitsync:checkAnchors($data-collection, $committerEmail, $collection-uri, $file-name),
				util:log("INFO", concat("check anchors ", $file-name))
			} catch * {
				(util:log("INFO", concat("Failed to check anchors ", $file-name, ": ", $err:code, ": ", $err:description)))
			}
...
declare function gitsync:do-add($commits, $contents-url as xs:string?, $data-collection) {
	let $committerEmail := $commits?1?pusher?email
	return for $modified in $commits?*?added?*
		let $file-path := concat($contents-url, $modified)
		let $gitToken := environment-variable("GITTOKEN")
		(: environment-variable('GIT_TOKEN') :)
		let $req := <http:request href="{ xs:anyURI($file-path) }" http-version="1.1" method="GET">
		...
		return try {
			(
				...
				(: gitsync:updateBibl($collection-uri, $file-name) , :)
				(: then    update the  expanded collection :)
				try { gitsync:updateExpanded($collection-uri, $file-name) } catch * {
					(
						util:log(
							"INFO",
							concat("Failed to update expanded resource ", $file-name, ": ", $err:code, ": ", $err:description)
				...
				let $stored-file := doc($collection-uri || "/" || $file-name)
				return (
					try { gitsync:validateAndConfirm($stored-file, $committerEmail, "updated") } catch * {
						(util:log("INFO", concat("validateAndConfirm ", $stored-file, ": ", $err:code, ": ", $err:description)))
					},
					if (contains($data-collection, "institutions")) then (
						try { gitsync:updateinstitutionsADD($file-name) } catch * {
							(util:log("INFO", concat("could not add to the list ", $stored-file)))
						}
					) else (
					),
					...
					let $id := substring-before($file-name, ".xml")
					let $remove := if ($id = $alldeleted) then
						let $item := $deletedlist/t:item[. = $id]
						return update delete $item
					else (
					)
					return "removed value from the list in deleted.xml",
					try { gitsync:rdf($collection-uri, $file-name) } catch * {
						(util:log("INFO", concat("failed rdf ", $file-name, ": ", $err:code, ": ", $err:description)))
					},
					if (ends-with($file-name, ".xml")) then (
						...
						let $allids :=
							for $xmlid in $stored-file//@xml:id
							return string($xmlid)
						let $taxonomy :=
							for $key in $gitsync:taxonomy//t:catDesc/text()
							return lower-case($key)
						return if ($stored-fileID eq $filename) then (
							if (($allids = $taxonomy) and ($collectionName != "authority-files")) then (
								let $intersect := config:distinct-values($allids[. = $taxonomy])
								return gitsync:wrongAnchor($committerEmail, $intersect, $filename)
							) else (
							)
						) else (
...
declare function gitsync:do-delete($commits, $contents-url as xs:string?, $data-collection) {
	...
	let $rdfcoll := concat("/db/rdf/", substring-after($collection-uri, "/db/apps/BetMasData/"))
	let $rdffilename := replace($file-name, ".xml", ".rdf")
	(: Remove from all three locations :)
	let $removeBetMas :=
		for $doc in collection("/db/apps/BetMasData")
		where matches(util:document-name($doc), $file-name)
		let $coll := util:collection-name($doc)
		return try {
			util:log("INFO", "[Remove BetMasData] " || $coll || "/" || $file-name), xmldb:remove($coll, $file-name)
		} catch * {
			util:log("ERROR", "[Remove BetMasData] failed: " || $coll || "/" || $file-name || " : " || $err:description)
		}
	let $removeExpanded :=
		for $doc in collection("/db/apps/expanded")
		where matches(util:document-name($doc), $file-name)
		let $coll := util:collection-name($doc)
		return try {
			util:log("INFO", "[Remove expanded] " || $coll || "/" || $file-name), xmldb:remove($coll, $file-name)
		} catch * {
			util:log("ERROR", "[Remove expanded] failed: " || $coll || "/" || $file-name || " : " || $err:description)
		}
	let $removeRDF :=
		for $doc in collection("/db/apps/rdf")
		where matches(util:document-name($doc), $file-name)
		let $coll := util:collection-name($doc)
		return try {
			util:log("INFO", "[Remove expanded] " || $coll || "/" || $file-name), xmldb:remove($coll, $file-name)
	...
	let $deletedlist := $gitsync:deleted//t:list
	let $id := substring-before($file-name, ".xml")
	let $updateDeleted := update insert <item
		xmlns="http://www.tei-c.org/ns/1.0"
		change="{ current-date() }"
		source="{ concat($collection-uri, "/", $file-name) }"
	>{ $id }</item> into $deletedlist
	return ()
};
...
declare function gitsync:updateMirrorCol($collection-uri, $file-name, $file-data, $updateoradd) {
	...
	return if (xmldb:collection-available($collection-uri)) then
...
declare function gitsync:fileortax($collection-uri, $file-name, $committerEmail) {
	let $stored-file := doc($collection-uri || "/" || $file-name)
	return if ($file-name = "taxonomy.xml") then (
...
declare function gitsync:updateLists($data-collection, $file-name) {
	if (contains($data-collection, "institutions")) then (
		...
		gitsync:updatepersonsMOD($file-name)
                    ...
                    gitsync:updateplacesMOD($file-name) ) :) else (
	)
};
...
declare function gitsync:checkAnchors($data-collection, $committerEmail, $collection-uri, $file-name) {
	if (ends-with($file-name, ".xml")) then (
		...
		let $filename := substring-before($file-name, ".xml")
		let $allids :=
			for $xmlid in $Stfile//@xml:id
			return lower-case(string($xmlid))
		let $taxonomy :=
			for $key in $gitsync:taxonomy//t:catDesc/text()
			return lower-case($key)
		return if ($stored-fileID eq $filename) then (
...
declare function gitsync:updateExpanded($collection-uri, $file-name) {
	...
	return if ($storedTEI/t:TEI) then
		...
		return if (xmldb:collection-available($expanded-collection-uri)) then
			<response status="okay">
				<message>
					{
						(
		...
		return if (xmldb:collection-available($expanded-collection-uri)) then
			<response status="okay">
				<message>{ xmldb:store($expanded-collection-uri, xmldb:encode-uri($file-name), $expanded-file) }</message>
			</response>
		else
			<response status="okay">
				<message>
...
declare function gitsync:validateAndConfirm($item, $mail, $type) {
	...
	return (: if all is ok, do not bother :) if ($validation = true()) then (
	) (: if there are problems, fire notification email :) else (
		(: build the message :)
		...
		let $contributorMessage := <mail>
			...
			<message>
				<xhtml>
					<html>
						<head><title>Report on the issues encountered in your file.</title></head>
						<body>
							<p
							>It's nice to push your changes directly to the live app and let the world know immediately about them!</p>
							<p>This time however your newly { $type } file <mark>{ $id }.xml</mark>
								<b
								>did not validate</b> to the Beta maṣāḥǝft schema,
							...
							<p
							>The following is a validation report from the app, but you can validate the file with your preferred method.</p>
							<div style="background-color:#ffad99;">
								<p><b>{ $report//status/text() }</b>.</p>
								{
									for $mess in $report//message
									return <p>{ string($mess/@level) } at line { string($mess/@line) }: { $mess/text() }</p>
								}
							</div>
							<p>
								<b>Please fix these issues as soon as possible and push the data to the <b>persons</b> repo again.</b>
							</p>
							<p
							>For the future, please note that you do not need to commit everything every time, you can decide
		...
		return util:log("INFO", concat("invalid file ", $id))
	)
};
...
declare function gitsync:wrongID($mail, $storedFileID, $filename) {
	let $address := if ($mail[1] = "noreply@github.com") then
		"info@betamasaheft.eu"
	else
		$mail[1]
	let $WrongIdMessage := <mail>
		...
		<message>
			<xhtml>
				<html>
					<head><title>The file { $filename }.xml has id { $storedFileID } instead of { $filename }.</title></head>
					<body>
	...
	return util:log("INFO", concat("The file ", $filename, ".xml has id ", $storedFileID, "instead of ", $filename))
};
...
declare function gitsync:wrongAnchor($mail, $intersect, $filename) {
	let $WrongIdMessage := <mail>
		...
		<message>
			<xhtml>
				<html>
					<head><title>The file { $filename }.xml uses an invalid @xml:id.</title></head>
					<body>
						<p>The file { $filename }.xml has @xml:id {
								string-join($intersect, ", ")
							} which are not allowed as IDs, because they are values reserved for 
                                the taxonomy.
                                </p>
						<p>
                                Please fix it as soon as possible.
	...
	return util:log(
		"INFO",
		concat("The file ", $filename, ".xml has id ", string-join($intersect, ", "), "which is reserved for taxonomy.")
	)
};
...
declare function gitsync:failedCommitMessage($mail, $data-collection, $message) {
	let $failureMessage := <mail>
		...
		<message>
			<xhtml>
				<html>
					<head><title>Failed Syncing { $data-collection } with the App.</title></head>
					<body>
						<p
						>Alas, your latest push to the application failed. Do not expect to find your latest changes in the application.</p>
						<p
						>Your friendly tech lead already knows about it, as he is receiving this very same message, and will look at it as soon as possible. Below is the error message. You can read it,
	...
	return util:log("INFO", concat("Failed syncing ", $data-collection))
};
...
declare function gitsync:TaxonomyMessage() {
	let $failureMessage := <mail>
		...
		<message>
			<xhtml>
				<html>
					<head><title>taxonomy.xml has been updated. the canonicaltaxonomy.xml is thus outdated.</title></head>
					<body>
	...
	return util:log("INFO", "taxonomy.xml has been updated. the canonicaltaxonomy.xml is thus outdated")
};
	...
	return try {
		if ($json-data?ref = "refs/heads/master") then
			if (exists($json-data?commits)) then
				...
				<response status="fail"><message>This is a GitHub request, however there were no commits.</message></response>,
				util:log("INFO", concat($data-collection, " This is a GitHub request, however there were no commits."))
			)
		else (
			<response status="fail"><message>Not from the master branch.</message></response>,
			util:log("INFO", concat($data-collection, " Not from the master branch.", $json-data?ref))
		)
	} catch * {
		(
			<response status="fail"><message>{ concat($err:code, ": ", $err:description) }</message></response>,
