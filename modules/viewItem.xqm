...
declare %private function viewItem:breakdownRef($FromToTarget) {
	...
	return (
		...
		for $s in $analyse//s:match
		return <ref>
			<folio>{ $s/s:group[@nr = "1"]/text() }</folio>
			{
				if ($s/s:group[@nr = "2"]) then
					<side>{ $s/s:group[@nr = "2"]/text() }</side>
				else (
				)
			}
			{
				if ($s/s:group[@nr = "3"]) then
					<col>{ $s/s:group[@nr = "3"]/text() }</col>
				else (
				)
			}
			{
				if ($s/s:group[@nr = "4"]) then
					<line>{ $s/s:group[@nr = "4"]/text() }</line>
				else (
				)
			}
		</ref>
...
declare %private function viewItem:locus($this) {
	...
	let $lowerPrefix := matches($clean, "[^\.]$") or
		matches($clean, " ca.") or
		matches($clean, " e.g.") or
		matches($clean, " cp.") or
		matches($clean, " esp.")
	...
	let $prefix := if ($lowerPrefix) then
		concat($base, " ")
	else
		concat(upper-case(substring($base, 1, 1)), substring($base, 2), " ")
	...
	return if ((count($this/ancestor::t:msItem) gt 2) and (count($this/ancestor::t:TEI//t:msItem) gt 100)) then
		"f." ||
			string($this/@from) ||
			(
				if ($this/@to) then
					concat("-", string($this/@to))
				else (
				)
			) ||
			substring-after($this/@target, "#")
	else (
		if ($excerpt) then
			"(Excerpt from "
		else (
		...
		if ($this[not(text())]) then
			if (contains(string($this/@target), " ")) then
				...
				$prefix,
				<a href="#{ $this/@from }">{ viewItem:choosefacsorlb($this, $ancID) }{ viewItem:parseRef($this/@from) }</a>,
				"–",
				<a href="#{ $this/@to }">
			...
			if ($this/@target) then
				<a href="{ $this/@target }">{ viewItem:choosefacsorlb($this, $ancID) }{ $this/text() }</a>
			else
				<a href="#{ $this/@from }">{ viewItem:choosefacsorlb($this, $ancID) }{ $this/text() }</a>
		),
		...
		else if ($this/ancestor::t:TEI//t:div[contains(@xml:id, "ranskribus")]) then
			(: 'matches lb' :)
...
declare %private function viewItem:gallery($item) {
	...
	return for $desc in $descs
		...
		return <div>
			<button
				aria-label="View images for { exptit:printTitle($mid) }"
				class="w3-button w3-red"
				onclick="document.getElementById('{ $modalid }').style.display='block'"
				style="vertical-align: top;width:160px;height:32;"
			>Images</button>
					...
					<div class="w3-container">
						{
							...
							let $tileSources := string-join(
								for $i in 1 to xs:integer($n)
								let $img := concat($config:appUrl, "/iiif/", $facs, format-number($i, "000"), ".tif/info.json")
								return '"' || $img || '"',
								","
							)
...
declare function viewItem:matchingFacs($locus) {
	...
	return <div class="w3-modal" id="{ $modalid }">
		<div class="w3-modal-content">
			<header class="w3-container">
				<h4>Images relevant for { exptit:printTitle($idandanchor) }, from {
						string($locus/ancestor::t:TEI//t:msIdentifier/t:idno/@facs)
					}
				</h4>
				<div>
					{
			...
			<div class="w3-container">
				{
					...
					let $mid := string(
						if ($locus/parent::t:witness) then
							$locus/parent::t:witness/@corresp
						else
							$mainID
					)
					let $manifest := if (starts-with($MainFacs, "http")) then
						...
						"https://betamasaheft.eu/api/iiif/" || $mid || "/manifest"
					...
					let $fcc := if (empty($rawPage) or normalize-space($rawPage) eq "") then
						"1"
					else if (string-length(translate($rawPage, "0123456789", "")) eq 0) then
						format-number(xs:integer($rawPage), "###")
					else
						string($rawPage)
					let $calculatedCanvas := try {
						iiifut:calculate-canvas($MainFacs, $fcc, $mid, "https://betamasaheft.eu")
					...
					let $openseadragonjs := 'OpenSeadragon({
						...
						'",
                           ...
                           defaultZoomLevel:   1,' ||
						(
							if (($locus/@from and $locus/@to) or $locus/@target[contains(., " ")]) then
								" sequenceMode: true, "
							else (
							)
						) ||
...
declare %private function viewItem:matchinglb($locus) {
	...
	let $ranges :=
		for $v in $values[string-length() ge 1]
		let $FromToTarget := replace($v, "\s", "")
		return <val><pos>{ $locus/position() }</pos>{ viewItem:breakdownRef($FromToTarget) }</val>
	return <div class="w3-modal" id="{ $modalid }">
		<!-- Modal content-->
		<div class="w3-modal-content">
			<header class="w3-container">
			...
			<div class="w3-container">
				{
					...
					let $pl := $prev//*:line
					let $pfs := ($pf || $ps)
					return if (($f = $pf) and ($s = $ps)) then (
					) else
						let $url := if (
							$l and
								$file//t:lb[@n = $l/text()][starts-with(@facs, "#facs_")][preceding-sibling::t:cb[1][@n =
									$c]][preceding-sibling::t:pb[1][@n = $fs]]
						) then
							let $matchingPageBreak := $file//t:lb[@n = $l/text()][starts-with(
								@facs,
								"#facs_"
							)][preceding-sibling::t:cb[1][@n = $c]][preceding-sibling::t:pb[1][@n = $fs]]
							let $nextMatchingPageBreak := $file//t:lb[@n = $nl/text()][starts-with(
								@facs,
								"#facs_"
							)][preceding-sibling::t:cb[1][@n = $nc]][preceding-sibling::t:pb[1][@n = $nfs]]
							let $matchingLine := $file//t:zone[@rendition = "Line"][@xml:id =
								substring-after($matchingPageBreak/@facs, "#")]
							...
							return concat($iiifbase, $locationclean, "/", $filename, "/", $region, "/full/0/default.jpg")

						else if (
							$c and $file//t:cb[@n = $c][starts-with(@facs, "#facs_")][preceding-sibling::t:pb[1][@n = $fs]]
						) then
							let $matchingColumnBreak := $file//t:cb[@n = $c][starts-with(
								@facs,
								"#facs_"
							)][preceding-sibling::t:pb[1][@n = $fs]]
							...
							let $region := string-join(($regionX, $regionY, $regionW, $regionZ), ",")
							return concat($iiifbase, $locationclean, "/", $filename, "/", $region, "/full/0/default.jpg")
						else if ($s and $file//t:pb[@n = $fs][starts-with(@facs, "#facs_")]) then
							...
							return concat($iiifbase, $locationclean, "/", $filename, "/full/full/0/default.jpg")
						else (
						)

						let $FromToTarget := string-join($range//text()[not(parent::*:pos)])
						let $firstcanvas := concat(
							"https://betamasaheft.eu/manuscripts/",
							$mainID,
							"/viewer?FirstCanv=https://betamasaheft.eu/api/iiif/",
							$mainID,
							"/canvas/p",
							$f
						)
						return (
							...
							<a href="{ $firstcanvas }" target="_blank">
								<img alt="Extract from { $location } for { $FromToTarget }" src="{ $url }" style="max-width:100%" />
							</a>
						)
				}
			</div>
...
declare %private function viewItem:locusrv($att) {
	...
	return if ($digits castable as xs:integer) then
		$digits
	else (
	)
};
...
declare %private function viewItem:choosefacsorlb($locus, $ancID) {
	...
	let $explicitPageToken := if (count($locus) gt 1) then
		string($locus[2])
	else (
	)
	return if ($actualElement/@facs and empty($explicitPageToken)) then
		attribute onclick { viewItem:imagesID($actualElement, "call", $actualElement/@facs, $ancID) }
	else if ($TEI//t:div[contains(@xml:id, "ranskribus")]) then
		attribute onclick { viewItem:imagesID($actualElement, "call", $actualElement/@*, "") }
	else if ($TEI//t:idno/@facs) then
		let $pageMS := if (string-length($explicitPageToken) gt 0) then
			...
			"1"
		let $page := if (empty($pageMS) or normalize-space(string($pageMS)) eq "") then
			"1"
		else
			string($pageMS)
		...
		let $id := $TEI/@xml:id
		let $canvas := try {
			if (starts-with($MainFacs, "http")) then
				iiifut:calculate-canvas($MainFacs, $page, $id, "https://betamasaheft.eu")
			else
				"https://betamasaheft.eu/api/iiif/" || $id || "/canvas/p" || $page
		} catch * { <error>{ $err:description }</error> }
		let $viewer := concat("https://betamasaheft.eu/manuscripts/", $id, "/viewer?FirstCanv=", $canvas)
		return (attribute title { "See viewer" }, attribute {"data-viewerurl"} { $viewer }, attribute target { "_blank" })
	else (
...
declare %private function viewItem:VisColl($collation) {
	...
	let $parameters := <parameters>
		<param name="mainID" value="{ string($collation/ancestor::t:TEI/@xml:id) }" />
	...
	let $transformation := try { transform:transform($collation, $xslt, $parameters) } catch * {
...
declare %private function viewItem:date($date) {
	if (matches($date, "\d{4}-\d{2}-\d{2}")) then
		...
		let $monthname := switch ($monthnumber)
			...
			default return
				()
		return concat(replace(substring-after($date, "-"), $monthnumber, $monthname), " ", substring-before($date, "-"))
	else
		format-number($date, "####")
};
...
declare %private function viewItem:notBnotA($element) {
	let $prefix := if (not($element/@notBefore)) then
		"Before "
	else if (not($element/@notAfter)) then
		"After"
	else (
	)
...
declare %private function viewItem:correspTit($t, $id) {
	...
	return (
		viewItem:TEI2HTML($corresp),
		viewItem:sup($corresp),
		if ($p = $count) then (
		) else
			", "
	)
...
declare %private function viewItem:worktitle($t) {
	...
	return <li property="http://purl.org/dc/elements/1.1/title">
		{
			attribute {"xml:id"} { $id },
			if ($t/@type) then
			...
			if ($t/@ref) then
				<a href="{ $config:appUrl }/{ viewItem:URI2ID($t/@ref) }" target="_blank">{ $t/text() }</a>
			else
...
declare %private function viewItem:placename($name) {
	...
	return <li>
		{
			attribute {"xml:id"} { $id },
			if ($name/@type and not(contains($name/@type, "normalized"))) then
				concat(string($name/@type), ": ")
			else (
			),
			if ($name/@ref) then
				<a href="{ $config:appUrl }/{ viewItem:URI2ID($name/@ref) }" target="_blank">{ $name/text() }</a>
			else
...
declare %private function viewItem:persname($name) {
	let $id := string($name/@xml:id)
	let $parent := $name/parent::t:person | $name/parent::t:persGroup
	let $cors := $parent/t:persName[substring-after(@corresp, "#") = $id]
	let $count := count($cors)
	return <li>
		{
			attribute {"xml:id"} { $id },
...
declare %private function viewItem:makeSequence($attribute) {
	let $string := normalize-space($attribute)
	return if (contains($string, " ")) then
		tokenize($string, " ")
	else
		string($attribute)
};
...
declare %private function viewItem:workAuthorList($parentname, $p, $a) {
	...
	return (
		$parentname,
		<a class="persName" href="{ $config:appUrl }/{ $filename }">{ exptit:printTitle($p) }</a>,
		if ($a/@name = "saws:isAttributedToAuthor") then (
			" ", <span class="w3-tag w3-round-large w3-red">attributed</span>
		) else (
		),
		<a id="{ generate-id($a) }Ent{ $filename }relations"><span class="glyphicon glyphicon-hand-left" /></a>,
		".",
		if ($a/t:desc) then
...
declare %private function viewItem:workAuthLi($a, $aorp) {
	...
	return for $p in $ps
		return <li>{ viewItem:workAuthorList($parentname, $p, $a) }</li>
};
...
declare %private function viewItem:personAuthLi($a, $aorp) {
	...
	let $ps := viewItem:makeSequence($att)
	return for $p in $ps
		return <li>{ viewItem:persRelList($parentname, $p, $a) }</li>
};
...
declare %private function viewItem:persRelList($parentname, $p, $a) {
	...
	return (
		$parentname,
		<a href="{ viewItem:TEI2HTML($p) }">{ exptit:printTitle($p) }</a>,
		<a id="{ generate-id($a) }Ent{ $filename }relations"><span class="glyphicon glyphicon-hand-left" /></a>,
		".",
		if ($a/t:desc) then
			viewItem:TEI2HTML($a/t:desc)
...
declare %private function viewItem:parentLink($node) {
	if ($node/ancestor::t:div[@xml:id]) then
		let $href := "/text/" ||
			string($node/ancestor::t:TEI/@xml:id) ||
			"#" ||
			string($node/ancestor::t:div[@xml:id][1]/@xml:id)
		return (
			<a class="page-scroll" href="{ $href }" target="_blank">
...
declare %private function viewItem:headercontext($node) {
	if ($node/ancestor::t:msPart[1]) then (
		...
		<a href="#{ $node/ancestor::t:msPart[1]/@xml:id }">{ substring-after($node/ancestor::t:msPart[1]/@xml:id, "p") }</a>
	) else (
	),
	" ",
	if ($node/ancestor::t:msItem[1]) then (
		", item ",
		<a href="#{ $node/ancestor::t:msItem[1]/@xml:id }">{ substring-after($node/ancestor::t:msItem[1]/@xml:id, "i") }</a>
	) else (
	),
	" ",
	if ($node/@corresp) then
		...
		let $items :=
			...
			let $text := if ($item/text()) then
				$item/text()
			else if ($item/name() = "listWit") then
			...
			return <a href="{ $config:appUrl }/{ viewItem:URI2ID($c) }">{ $text }{ $lang }</a>
		return ("(about:", $items, ")")
	else (
	)
};
...
declare %private function viewItem:biblioHeader($listBibl) {
	...
	return concat(concat(upper-case(substring(string($t), 1, 1)), substring($t, 2), " "[not(last())]), " Bibliography"),
...
declare %private function viewItem:bibliographyHeader($listBibl) {
	...
	else if ($listBibl[not(parent::t:item) and not(ancestor::t:physDesc)]) then
	...
	<div class="w3-row">
		<div class="w3-col" style="width:85%">
			<span class="Zotero Zotero-full" data-type="{ $node/t:seg/@type }" data-value="{ $t }">
				...
				{
					let $crs :=
						for $cr in $node/t:citedRange
						return (string($cr/@unit) || " " || $cr/text())
					return " " || string-join($crs, ", ")
				}
		...
		<div class="w3-rest">
			<span class="w3-bar-block w3-hide-small w3-hide-medium">
				<a class="w3-bar-item w3-button w3-tiny" href="{ $node/@corresp }" target="_blank">Zotero</a>
				<a
					class="w3-bar-item w3-button w3-tiny"
					href="{ $config:appUrl }/bibliography?pointer={ $t }"
				>Other citations</a>
			</span>
		</div>
	</div>
...
declare %private function viewItem:bibliographyitem($node) {
	if ($node[not(@corresp) and not(t:ptr[@target])]) then
		<b style="color:red;">THIS BIBLIOGRAPHIC RECORD ({
				$node
			}) has no @corresp or t:ptr/@target. Please check the
            schema error report to fix it.</b>
	else if ($node[@corresp and not(t:ptr[@target])]) then (
		let $cors := viewItem:makeSequence($node/@corresp)
		for $c in $cors
		return <a href="{ $c }">{ exptit:printTitle($c) }</a>,
		$node/text(),
		viewItem:TEI2HTML($node/t:date),
		viewItem:TEI2HTML($node/t:note)
	) else if ($node/t:ptr/@target = "bm:EthioSpare" and $node/parent::t:listBibl[@type = "catalogue"]) then
		...
		let $crs :=
			for $cr in $node/t:citedRange
			return (string($cr/@unit) || " " || $cr/text())
		return if ($node/parent::t:surrogates) then
...
declare %private function viewItem:EthioSpareFormatter($node) {
	...
	let $cataloguer := if ($TEI//t:editor[@role = "cataloguer"]) then
		$TEI//t:editor[@role = "cataloguer"]/text()
	else if ($TEI//t:editor[not(@role = "generalEditor")]) then
		$TEI//t:editor[not(@role = "generalEditor")]/text()
	else
		let $resp := substring-after(($TEI//t:change[contains(., "created")]/@who)[1], "#")
		return string-join(
			distinct-values(($TEI//t:editor[@key = $resp]/text() | $TEI//t:respStmt[@xml:id = $resp]/t:name/text())),
			", "
		)
	let $repository := $TEI//t:repository/text()
	let $date :=
		for $d in $TEI//t:origDate
		return viewItem:dates($d)
	let $title := $TEI//t:titleStmt/t:title[1]/text()
	return (
		<a href="https://mycms-vs03.rrz.uni-hamburg.de/domlib/receive/{ $domliblist }">MS { $repository }, Ethio-SPaRe {
				$BMsignature
			}
...
declare %private function viewItem:relation($node) {
	(
		<a href="{ viewItem:reflink($node/@active) }">{ exptit:printTitle($node/@active) }</a>,
		" ",
		<a href="{ viewItem:reflink($node/@ref) }"><code>{ string($node/@name) }</code></a>,
		" ",
		<a href="{ viewItem:reflink($node/@passive) }">{ exptit:printTitle($node/@passive) }</a>,
		" ",
...
declare %private function viewItem:publicationStmt($node) {
	<div class="w3-container w3-small w3-responsive" id="publicationStmt" style="overflow-x:auto;">
		<h3>Publication Statement</h3>
		{
			for $n in $node/node()
			return <div class="w3-row">
				<div class="w3-col" style="width:80px;">{ $n/name() }</div>
				...
				<div class="w3-rest" style="overflow-x:auto; vertical-align: text-top; text-align: left;">
					{ viewItem:TEI2HTML($n) }
				</div>
			</div>
		}
...
declare %private function viewItem:certainty($certainty) {
	...
	let $statement := if ($certainty[@cert and not(@resp) and not(@assertedValue)]) then
		concat(" is ", $certainty/@cert, ".")
	else if ($certainty[@resp and @assertedValue]) then
		concat(" is ", $certainty/@assertedValue, " according to ", $resp)
	else if ($certainty[not(@resp) and @assertedValue]) then
		concat(" is low. It might alternatively be ", $certainty/@assertedValue, ".")
	else if ($certainty[@resp and not(@assertedValue)]) then
	...
	return <span class="w3-tooltip">
		<sup>[!]</sup>
		<span class="w3-text">The certainty about the { string($certainty/@locus) } of { $match }
			{ $statement }
			{ $desc }
		</span>
	</span>
...
declare %private function viewItem:footnoteptr($node) {
	...
	return <sup><a href="{ $node/@target }" id="pointer{ $t }">{ string($note/@n) }</a></sup>
};
	...
	<dl style="font-size:smaller;">
		{
			...
			return (
				<dt>
					<i><a href="#pointer{ $t }" id="{ $node/@xml:id }">{ string($node/@n) })
                        </a></i>
				</dt>,
				<dd>{ viewItem:TEI2HTML($node/node()) }</dd>
			)
		}
	</dl>
};
...
declare %private function viewItem:date-like($date) {
	if ($date/@calendar) then
		...
		viewItem:dates($date), viewItem:time($date)
	)
};
				...
				>
					{
						...
						else if (string-length($att) = 7) then (
							attribute data-year { number(substring($att, 1, 4)) },
							attribute data-month { number(substring($att, 6, 2)) }
						) else if (starts-with($att, "--")) then (
							attribute data-month { number(substring($att, 3, 2)) },
							attribute data-day { number(substring($att, 6, 2)) }
						) else (
							attribute data-year { number(substring($att, 1, 4)) },
							attribute data-month { number(substring($att, 6, 2)) },
	...
	if ($node/parent::t:decoDesc) then (
		...
		return (
			...
			<div class="summaryText" id="extracted{ $id }">
				...
				<ol class="summary">
					{
						if ($node/ancestor::t:msPart or $node/ancestor::t:msFrag) then
							for $item in $node/ancestor::t:msPart//t:msItem[not(parent::t:msItem)]
							order by $item/position()
							return viewItem:summaryitems($item)
						else
...
declare %private function viewItem:summaryitems($item) {
	<li>
		<a class="page-scroll" href="#{ $item/@xml:id }">{ string($item/@xml:id) }</a>
        ({
			viewItem:TEI2HTML($item/t:locus)
		}),
        { string-join($item/t:title//text(), " ") }
		{
			if ((count($item/ancestor::t:TEI//t:msItem) gt 100) and (count($item/ancestor::t:msItem) gt 1)) then
				<p>There are {
						count($item/ancestor::t:TEI//t:msItem)
					} items in this description, to show this page we have to stop here.</p>
			else if ($item/t:msItem) then
				<ol class="summary">
					{
						for $subitem in $item/t:msItem
						order by $subitem/position()
						return viewItem:summaryitems($subitem)
					}
				</ol>
			else (
			)
...
declare %private function viewItem:cae($entity) {
	...
	let $id := if (not(contains($mainid, "NAR") or contains($mainid, "STU") or contains($mainid, "IHA"))) then
		viewItem:URI2ID($mainid)
	else (
	)
	return if ($id) then
		"CAe " ||
...
declare %private function viewItem:namedEntityTitle($entity) {
	if ((count($entity/ancestor::t:msItem) gt 2) and (count($entity/ancestor::t:TEI//t:msItem) gt 100)) then (
		...
		if (matches($entity/@ref, "LIT")) then
			<a xmlns="http://www.w3.org/1999/xhtml" href="{ $config:appUrl }/{ viewItem:URI2ID($entity/@ref) }">({
					viewItem:cae($entity)
				})</a>
		else (
		)
	) else (
		<a xmlns="http://www.w3.org/1999/xhtml" href="{ $config:appUrl }/{ viewItem:URI2ID($entity/@ref) }" target="_blank">
...
declare %private function viewItem:namedEntityPerson($entity) {
	(
		if ($entity/@ref) then
			<a xmlns="http://www.w3.org/1999/xhtml" href="{ viewItem:reflink($entity/@ref) }">
				{ viewItem:TEI2HTML($entity/node()) }
			</a>
		else
			viewItem:TEI2HTML($entity/node()),
...
declare %private function viewItem:namedEntityPersonNoLink($entity) {
	(
		<span
			...
			resource="{ viewItem:reflink($entity/@ref) }"
		>{ viewItem:TEI2HTML($entity/node()[not(self::t:note)]) }</span>,
		if ($entity/@role) then
			string($entity/@role)
		else (
		)
...
declare function viewItem:namedEntityPlace($entity) {
	(
		...
		<a xmlns="http://www.w3.org/1999/xhtml" href="{ viewItem:reflink($entity/@ref) }">
			{
				if (count($entity/node()) ge 1) then
					viewItem:TEI2HTML($entity/node()[not(self::t:note)][not(self::t:certainty)])
				else (
				)
			}
		</a>,
		let $idref := viewItem:URI2ID($entity/@ref)
		return if (contains($idref, "pleiades")) then
			...
			return <span
				...
				data-value="{ $pleiadesid }"
			>↗</span>
		else if (contains($idref, "wd")) then
			<span
				xmlns="http://www.w3.org/1999/xhtml"
				class="pelagios popup"
		...
		" ",
		if (
			not($entity/ancestor::t:div[@type = "edition"]) and
				$entity[@when | @notBefore | @notAfter | @when-custom | @notBefore-custom | @notAfter-custom]
		) then
			if ($entity/@when) then (
				" (information recorded on: " || viewItem:date($entity/@when) || ")"
			) else if ($entity/@notBefore) then (
				" After: " || viewItem:date($entity/@notBefore)
			) else if ($entity/@notAfter) then (
		...
		if ($entity/@type = "qushat") then
			" qušat "
		else if ($entity/@type = "warada") then
			" waradā "
		else (
		)
	)
...
declare %private function viewItem:msItem($msItem) {
	...
	>
		...
		<div class="w3-hide msItemContent" id="item{ $trimid }">
			<div class="w3-container">
				<hr class="msItems w3-left-align" />
				{
...
declare %private function viewItem:figure($figure as element(t:figure)) {
	...
	return (: if the link ends with .svg :) if (matches($link, "\.svg$")) then
		<div
			id="{ $id }"
			style="height:{
				if ($height) then
		...
		>
			{ doc($link) }
			<div class="caption w3-margin-left w3-tiny">{ viewItem:TEI2HTML($figure/t:graphic/t:desc) }</div>
		</div>
	...
	else if ($figure/t:graphic/@n) then
		let $n := $figure/t:graphic/@n
		let $urls :=
			for $f in 1 to $n
			return '"' || replace($url, "/info.json", concat(format-number(., "000"), ".tif/info.json")) || '"'
		return <div id="{ $id }">
		...
		<div id="{ $id }">
			<div id="{ $id }" style="height:300px">
				<a href="{ $uri || "/full/1424,/0/default.jpg" }">
					<img src="{ $uri || "/full/356,/0/default.jpg" }" style="height:300px" />
				</a>
			</div>
			<div class="caption w3-margin-left w3-tiny">{ viewItem:TEI2HTML($figure/t:graphic/t:desc) }</div>
			<!--       <script
			...
			<p>
				{
					if ($node//t:material/@key) then
						...
						>{ concat(upper-case(substring($mk, 1, 1)), substring($mk, 2), " ") }</span>
					else (
					)
				}
				<span class="w3-tag w3-red" typeof="{ $config:appUrl }/{ $node/parent::t:objectDesc/@form }">
...
declare %private function viewItem:palaeography($node) {
	<div rel="http://purl.org/dc/terms/hasPart">
		<h3>Palaeography { viewItem:headercontext($node) }</h3>
		{
			...
			return <li class="nodot" id="{ $h/@xml:id }">
				{ <h4>Hand { substring-after($h/@xml:id, "h") }</h4> }
				{
					if ($h/@corresp) then
						<p>Hand of <a href="{ string($h/@corresp) }">{ substring-after($h/@corresp, "#") }</a></p>
					else (
					)
				}
				...
				{ viewItem:TEI2HTML($h/node()[empty(self::t:seg)][empty(self::t:list)]) }
				{
					if ($h/t:list[@type = "abbreviations"]) then (
						<h4>Abbreviations</h4>, <ul>{ viewItem:TEI2HTML($h/t:list[@type = "abbreviations"]/node()) }</ul>
					) else (
					)
				}
			</li>
...
declare %private function viewItem:layoutdimensionunit($dim) {
	(<span class="lead">{ $dim/text() }</span>, string($dim/parent::t:*/@unit))
};
	...
	return <div id="layout{ $pos }" resource="{ $config:appUrl }/{ $node/ancestor::t:TEI/@xml:id }#layout{ $pos }">
		...
		}
		{
            ...
            let $leftmargin := if ($node/t:dimensions[not(@xml:lang)][@type = 'margin'][1]/t:dim[@type = 'left'][1]/text()) then
                ...
                ('0')
            let $textwidth := $node/t:dimensions[not(@xml:lang)][not(@type)][1]/t:width[1][not(contains(text(), '-'))]
            let $heighText := $node/t:dimensions[not(@xml:lang)][not(@type)][1]/t:height[1][not(contains(text(), '-'))]
            let $totalHeight := if ($node/ancestor::t:TEI//t:dimensions[not(@xml:lang)][@type = 'outer' and @unit = 'mm'][not(contains(text(), '-'))]/t:height/text() or
            $node/ancestor::t:TEI//t:dimensions[not(@xml:lang)][@type = 'outer'][not(contains(text(), '-'))]/t:height[@unit = 'mm']/text()) then
                (max($node/ancestor::t:TEI//t:dimensions[not(@xml:lang)][@type = 'outer']/t:height))
            else
                0
            let $totalwidth := if ($node/ancestor::t:TEI//t:dimensions[not(@xml:lang)][@type = 'outer' and @unit = 'mm'][not(contains(text(), '-'))]/t:width/text() or
            $node/ancestor::t:TEI//t:dimensions[not(@xml:lang)][@type = 'outer'][not(contains(text(), '-'))]/t:width[@unit = 'mm']/text())
            then
...
declare %private function viewItem:ab($node as element(t:ab)) {
	if ($node/ancestor::t:div[@type = "edition"]) then
		<div class="w3-container">
			{
				if ($node/t:title/t:ref[1][@target]) then
					<h3><a href="{ string($node/t:title[1]/@target) }" target="_blank">{ string($node/t:title) }</a></h3>
				else (
				)
			}
			{
				if ($node/t:title/t:ref[1][@target]) then
					viewItem:TEI2HTML($node/node()[not(self::t:title)])
				else
					viewItem:TEI2HTML($node/node())
			}
		</div>
	else if ($node/ancestor::t:div[@subtype = "Psalmus"]) then
		<div class="w3-container">
			<h3><a href="{ $node/t:title/t:ref/@target }">{ viewItem:TEI2HTML($node/t:title) }</a></h3>
...
declare %private function viewItem:acquisition($node as element(t:acquisition)) {
	(<h3>Acquisition</h3>, <p>{ viewItem:TEI2HTML($node/node()) }</p>)
};
...
declare %private function viewItem:altIdentifier($node as element(t:altIdentifier)) {
	(
		...
		return <span content="{ $idno/text() }" property="http://www.cidoc-crm.org/cidoc-crm/P1_is_identified_by">
			{ $idno/text() }
		</span>,
...
declare %private function viewItem:handDesc($node) {
	<div resource="{ $config:appUrl }/{ $node/ancestor::t:TEI/@xml:id }/hand/{ $node/@xml:id }">
		{
			attribute typeof {
				if ($node/@script) then (
				...
				$config:appUrl || "/hand",
				"https://w3id.org/sdc/ontology#UniMain"
			}
		}
		<h6 id="{ $node/@xml:id }" style="scroll-margin-top: 64px;">Hand { viewItem:headercontext($node) }
			{
				if ($node/@corresp) then (
					"(",
					for $c in viewItem:makeSequence($node/@corresp)
					let $type := switch ($c)
						case starts-with(., "#q") return
							"quire"
						case starts-with(., "#a") return
							"addition"
						case starts-with(., "#i") return
							"content item"
						case starts-with(., "#b") return
							"binding"
						case starts-with(., "#d") return
							"decoration"
						default return
							$c
					return (
						<a href="{ $c }">{ $type }{ substring-after($c, "#") }</a>,
						<span
							property="http://purl.org/dc/terms/relation"
							resource="https://betamasaheft/{ $node/ancestor::t:TEI/@xml:id }/{ $type }/{
								substring-after($c, "#")
							}" />
					),
					")"
...
declare %private function viewItem:foliation($node as element(t:foliation)) {
	(<h3>Foliation { viewItem:headercontext($node) }</h3>, <p>{ viewItem:TEI2HTML($node/node()) }</p>)
};
...
declare %private function viewItem:extent($node as element(t:extent)) {
	...
	<div rel="http://purl.org/dc/terms/hasPart">
		<span content="{ $node/t:measure[1][@unit = "leaf"][not(@type)] }" property="{ $config:appUrl }/hasTotalLeaves" />
		{ viewItem:TEI2HTML($node/node()) }
	</div>
};
...
declare %private function viewItem:decoDesc($node) {
	<div id="deco" rel="http://purl.org/dc/terms/hasPart">
		<h2>
			<span class="w3-tooltip">Decoration { viewItem:headercontext($node) }
				<span class="w3-text">
                    In this unit there are in total {
						let $types :=
							for $type in distinct-values($node/t:decoNote/@type)
							let $count := count($node/t:decoNote[@type = $type])
							return (
								$count ||
									" " ||
									viewItem:categoryname($node/ancestor::t:TEI, $type) ||
									(
										if ($count gt 2) then
											"s"
										else
				...
				<ol>
					{
						...
						return viewItem:decoNoteItem($node, $r)
					}
				</ol>
			) else (
			)
		...
		{
			if (
				$node/t:decoNote[not(ancestor::t:binding)][@type != "rubrication"][@type != "miniature"][@type != "frame"]
			) then (
	...
	>
		{
			attribute typeof {
				...
				return ("http://betamasaheft.eu/ontology/" || $type),
				for $type in $r/@type
				return ("http://betamasaheft.eu/ontology/" || $type)
			}
		}
...
declare %private function viewItem:colophon($node as element(t:colophon)) {
	(
		<hr class="colophon" />,
		<h3 id="{ $node/@xml:id }" style="scroll-margin-top: 64px;">
			{
				if ($node/@type) then
					string($node/@type)
				else
		...
		<p>
			{
				viewItem:TEI2HTML(
					$node/node()[not(self::t:note)][not(self::t:foreign)][not(self::t:listBibl)][not(self::t:locus)]
				)
			}
		</p>,
		if ($node/t:foreign) then
			for $t in $node/t:foreign
			return <p lang="{ $t/@xml:lang }">
		...
		if ($node/t:note) then
			<p>{ viewItem:TEI2HTML($node/t:note) }</p>
		else (
		),
		if ($node/t:listBibl) then
			<p>{ viewItem:TEI2HTML($node/t:listBibl) }</p>
		else (
		)
	)
};
...
declare %private function viewItem:bindingDesc($node) {
	(
		...
		<p id='b1'>{ viewItem:TEI2HTML($node/t:binding/t:decoNote[@xml:id = "b1"]) }</p>,
		if ($node//t:decoNote[@type = "bindingMaterial"]) then (
			for $bo in $node/t:binding/t:decoNote[@type = "bindingMaterial"]
			return (
				<h4 id="{ $bo/@xml:id }">Binding material</h4>,
				for $m in $bo/t:material/@key
				return <p>{ string($m) }</p>,
				<p>{ viewItem:TEI2HTML($bo/node()) }</p>
			)
		) else (
		),
			...
			<h4 id="{ $b/@xml:id }">
				{
					if ($b/@type = "SewingStations") then
						...
						functx:capitalize-first(string($b/@type))
					else (
					)
				}
			</h4>,
		...
		if ($node/t:binding/t:decoNote[@type = "Other"]) then (
			...
			return <p>{ viewItem:TEI2HTML($bo/node()) }</p>
		) else (
		)
	)
};
...
declare %private function viewItem:accMat($node as element(t:accMat)) {
	(<h3>Accompanying Material</h3>, <p>{ viewItem:TEI2HTML($node/node()) }</p>)
};
...
declare %private function viewItem:additions($node) {
	<div id="additiones" rel="http://purl.org/dc/terms/hasPart">
		<h2>
			<span class="w3-tooltip">Additions { viewItem:headercontext($node) }
				<span class="w3-text">
                    In this unit there are in total {
						let $types :=
							for $type in distinct-values($node//t:item/t:desc/@type)
		...
		<ol>
			{
				...
				return viewItem:additionItem($a)
			}
		</ol>
		{
			if ($node//t:item[starts-with(@xml:id, "e")]) then (
...
declare %private function viewItem:additionItem($a) {
	...
	>
		...
		<p>
			{ viewItem:TEI2HTML($a/t:locus) }
			{
				if ($a/t:desc/@type) then (
					" (Type: ",
					<a href="{ $config:appUrl }/authority-files/list?keyword={ $a/t:desc/@type }" target="_blank">
...
declare %private function viewItem:desc($node as element(t:desc)) {
	if ($node[not(parent::t:relation)][not(parent::t:handNote)]) then
		<p>{ viewItem:TEI2HTML($node/node()) }</p>
	else
		viewItem:TEI2HTML($node/node())
};
		...
		<b>
			{
				if ($node/@type = "supplication") then
					"Supplication"
				else if ($node/@type = "subscriptio") then
					"Subscription"
				else
					"Explicit"
...
declare %private function viewItem:faith($node as element(t:faith)) {
	if ($node/text()) then
		$node/text()
	else if (count($node/ancestor::t:TEI//t:catDesc[. = string($node/@type)]) ge 1) then
...
declare %private function viewItem:filiation($node as element(t:filiation)) {
	<p><b>Filiation: </b>{ viewItem:TEI2HTML($node/node()) }</p>
};
...
declare %private function viewItem:hi($node as element(t:hi)) {
	if ($node/@rend = "ligature") then
		<span style="border-top:1px solid">{ viewItem:TEI2HTML($node/node()) }</span>
	else if ($node/@rend = "apices") then
		<sup>{ viewItem:TEI2HTML($node/node()) }</sup>
	else if ($node/@rend = "underline") then
		<u>{ viewItem:TEI2HTML($node/node()) }</u>
	else if ($node/@rend = "rubric") then
		<span class="rubric">{ viewItem:TEI2HTML($node/node()) }</span>
	else if ($node/@rend = "encircled") then
		<span class="encircled">{ viewItem:TEI2HTML($node/node()) }</span>
	else
		viewItem:TEI2HTML($node/node())
};

declare %private function viewItem:history($node as element(t:history)) {
	(<h2>Origin { viewItem:headercontext($node) }</h2>, <p>{ viewItem:TEI2HTML($node/node()) }</p>)
};
		...
		<b>
			{
				if ($node/@type = "supplication") then
					"Supplication"
				else if ($node/@type = "inscriptio") then
					"Inscriptio"
				else
					"Incipit"
			} ({ viewItem:fulllang($node/@xml:lang) }
...
declare %private function viewItem:list($node as element(t:list)) {
	if ($node[ancestor::t:abstract or ancestor::t:desc]) then
		<ol>{ viewItem:TEI2HTML($node/node()) }</ol>
	else
		<ul>{ viewItem:TEI2HTML($node/node()) }</ul>
};

declare %private function viewItem:listBibl($node as element(t:listBibl)) {
	(<h4>{ viewItem:biblioHeader($node) }</h4>, <ul class="bibliographyList">{ viewItem:TEI2HTML($node/node()) }</ul>)
};
	...
	<span class="w3-tooltip">
		...
		})
        {
			if ($node/following-sibling::t:*[1][self::t:locus]) then
				": "
			else if ($node/following-sibling::t:measure[not(@xml:lang)][@unit = "leaf"][@type = "blank"]) then
				", "
			else
				"."
		}
		<span class="w3-text w3-tag">Entered as { $node/text() }</span>
	</span>
};
...
declare %private function viewItem:msContents($node as element(t:msContents)) {
	(
		<h3>Contents</h3>,
		<div class="accordion" id="contents">{ viewItem:TEI2HTML($node/node()[not(name() = "summary")]) }</div>
	)
};
...
declare %private function viewItem:note($node as element(t:note)) {
	if ($node/parent::t:placeName) then
		<div class="w3-panel w3-gray">
			{
				if ($node/t:p or $node/ancestor::t:app) then
					viewItem:TEI2HTML($node/node())
				else if (not($node/parent::*:fragment)) then
					<p>{ viewItem:TEI2HTML($node/node()) }</p>
				else
					<p>{ viewItem:TEI2HTML($node/node()) }</p>
			}
	...
	else if ($node/parent::t:rdg) then (
		" ", <i>{ viewItem:TEI2HTML($node/node()) }</i>, " "
	) else if ($node[@xml:id][@n]) then
...
declare %private function viewItem:origplace($node as element(t:origPlace)) {
	(
		<p><b>Original Location: </b>{ viewItem:TEI2HTML($node/node()) }</p>,
		<p>{ viewItem:TEI2HTML($node/parent::t:origin/t:provenance) }</p>
	)
};

declare %private function viewItem:provenance($node as element(t:provenance)) {
	(<h3>Provenance</h3>, <p>{ viewItem:TEI2HTML($node/node()) }</p>)
};
...
declare %private function viewItem:roleName($node as element(t:roleName)) {
	if ($node[not(parent::t:persName)]) then
		<a
			...
			href="/xpath.html?xpath=%24config%3Acollection-root%2F%2Ft%3AroleName%5Bcontains%28.%2C%27{
				$node/text()
			}%27%29%5D"
		>{ concat(string($node), " ") }<sup>{ string($node/@type) }</sup></a>
	else
		<span xmlns="http://www.w3.org/1999/xhtml" class="w3-tooltip">
			{ concat(string($node), " ") }
			<span class="w3-text"><sup>role: { string($node/@type) }</sup></span>
		</span>
};

declare %private function viewItem:rs($node as element(t:rs)) {
	if ($node/@type = "inline") then
		<img alt="Region of image from { $node/t:graphic/@url }" src="{ $node/t:graphic/@url }" />
	else
		viewItem:TEI2HTML($node/node())
};

declare %private function viewItem:rubric($node as element(t:rubric)) {
	<p lang="{ $node/@xml:lang }"><b>Rubric { string($node/@xml:lang) }: </b>{ viewItem:TEI2HTML($node/node()) }</p>
};

declare %private function viewItem:sealDesc($node as element(t:sealDesc)) {
	(<h3>Seals { viewItem:headercontext($node) }</h3>, viewItem:TEI2HTML($node/node()))
};

declare %private function viewItem:seg($node as element(t:seg)) {
	if ($node/@ana) then
		<span class="{ substring-after($node/@ana, "#") }">{ viewItem:TEI2HTML($node/node()) }</span>
	else
		viewItem:TEI2HTML($node/node())
};
...
declare %private function viewItem:surrogates($node as element(t:surrogates)) {
	(<h2>Surrogates</h2>, viewItem:TEI2HTML($node/node()))
};
		...
		<span property="http://purl.org/dc/elements/1.1/language">
			{
				if ($node/@xml:lang) then
					...
					viewItem:fulllang($node/@mainLang)
			}
		</span>
		{
...
declare %private function viewItem:lb($node as element(t:lb)) {
	if ($node/parent::t:ab) then (
		if ($node/@break) then
			"|"
		else (
			" |", <sup id="{ viewItem:DTSpartID($node) }">{ $node/@n }</sup>, " "
		)
	) else if ($node[ancestor::t:ab][parent::t:placeName or parent::t:persName or parent::t:w]) then (
		if ($node/@break) then
			"|"
		else (
			" |", <sup id="{ viewItem:DTSpartID($node) }">{ $node/@n }</sup>, " "
		)
	) else if ($node[parent::t:l][not(parent::t:ab)] and $node/preceding-sibling::text()) then (
		<br />
	) else if (
		$node[not(parent::t:ab)][not(parent::t:l)][not(parent::t:w)][not(parent::t:persName)][not(parent::t:placeName)]
	) then (
		let $line := $node/@n
		return (
			<br />,
			if (number($node/@n) and $node/@n mod number(5) = 0 and not($node/@n = 0)) then (
				string($node/@n), <span style="padding-left: 5em;" />
			) else
...
declare %private function viewItem:handShift($node as element(t:handShift)) {
	<sub><a href="{ $node/@new }">{ substring-after($node/@new, "#") }</a></sub>
};
...
declare %private function viewItem:surplus($node as element(t:surplus)) {
	...
	return <span class="w3-tooltip">
		...
		{
			if ($resp) then
				<span class="w3-text w3-tag w3-small">{ $resp }</span>
			else (
			)
		}
	</span>
	...
	return <span class="w3-tooltip">
		{
			...
			) else if ($sic and $orig) then (
				"{", $orig/string(), "}"
			) else (
				viewItem:TEI2HTML($node/node())
			)
		}
		<span class="w3-text w3-tag w3-small">
			{
				if ($resp) then (
					$sicHTML, " ", viewItem:tipResp($resp, "corrected by")
				) else (
					$sicHTML, "(!)"
				)
			}
...
declare %private function viewItem:unclear($node as element(t:unclear)) {
	<span
		style="background-color:hsla(50, 20%, 50%, 0.2); opacity: 0.6; text-decoration-line: underline; text-decoration-style: wavy; text-decoration-color: gray;"
	>[{ viewItem:TEI2HTML($node/node()) }?]</span>
};
...
declare %private function viewItem:sic($node as element(t:sic)) {
	<span class="w3-tooltip">
		{ viewItem:TEI2HTML($node/node()) }(!)
			...
			{
				if ($node/@resp) then
					viewItem:tipResp($node/@resp, "sic by")
				else (
				)
			}
		</span>
...
declare %private function viewItem:del($node as element(t:del)) {
	let $content := if (empty($node)) then
	...
	let $renderers := map {
		...
		"overUnderlined":
			function ($c) {
				<span style="border-top:1px dotted #000; border-bottom:1px dotted #000; padding:1px;">{ $c }</span>
			},
		"effaced": function ($c) { <span class="w3-gray">{ $c }</span> }
	}
	let $rendKey := head(
		for $k in map:keys($renderers)
		return if (contains($node/@rend, $k)) then
			$k
		else (
		)
	)
	let $renderer := if ($rendKey) then
		$renderers($rendKey)
	else
		function ($c) { $c }
	return <span class="w3-tooltip">
		...
		<span class="w3-text w3-tag w3-small CorrResp">
			{
				if ($node/@resp) then
					viewItem:tipResp($node/@resp, "corrected by")
				else (
				)
			}
		</span>
...
declare %private function viewItem:supplied($node as element(t:supplied)) {
	...
	let $renderer := if (map:contains($renderers, $reason)) then
		$renderers($reason)
	else if ($reason) then
		function ($c) { ("(", $reason, ": ", $c, ")") }
	else
...
declare %private function viewItem:orig($node as element(t:orig)) {
	<span class="undeciphrable">{ viewItem:TEI2HTML($node/node()) }</span>
};

declare %private function viewItem:add($node as element(t:add)) {
	let $id := generate-id($node)
	return if ($node/@hand and $node/@place) then (
		<span class="w3-tooltip">
			{ viewItem:TEI2HTML($node/node()) }
			<span class="w3-text">Note added {
					if ($node/@hand) then
	...
	) else if ($node/@place = "overstrike" and $node/preceding-sibling::t:del) then (
		"{", viewItem:TEI2HTML($node/node()), "}"
	) else if ($node/@hand and not($node/@place)) then (
		"/",
		<span class="w3-tooltip">
			{ viewItem:TEI2HTML($node/node()) }
			<span class="w3-text">Note added {
		...
		"/"
	) else if ($node/@place = "margin" and $node/ancestor::t:TEI[@type = "work"]) then (
		<span class="w3-tooltip">
			{ viewItem:TEI2HTML($node/node()) }
			<span class="w3-text">Note added {
					if ($node/@hand) then
...
declare %private function viewItem:gap($node as element(t:gap)) {
	let $reason := string($node/@reason)
	let $quantity := if ($node/@quantity) then
		xs:integer($node/@quantity)
	else (
	)
			...
			function ($n) {
				if ($quantity) then
					...
					if ($extent = "unknown") then
						...
						string-join(
							for $i in 1 to xs:integer($extent)
							return "▧",
							""
						)
				else
	...
	return <span class="w3-tooltip">
		...
		{
			if ($resp) then
				<span class="w3-text w3-tag w3-small OmissionResp">{ $resp }</span>
			else (
			)
		}
	</span>
...
declare %private function viewItem:pb($node as element(t:pb)) {
	if ($node/ancestor::t:div[@type = "edition"] or $node/ancestor::dts:fragment) then (
		" |", <sup id="{ viewItem:DTSpartID($node) }">{ $node/@n }</sup>, " ", viewItem:imagelink($node)
	) else (
		<hr id="part{ $node/@n }" />, <p>{ viewItem:imagelink($node) }</p>
	)
};
...
declare %private function viewItem:l($node as element(t:l)) {
	if ($node/ancestor::t:div[@subtype = "Psalmus"]) then
		<div class="w3-container"><p><sup>{ $node/@n }</sup>{ viewItem:TEI2HTML($node) }</p></div>
	else (
		<sup>
			{
				if ($node/t:ref[1][@target]) then
					<a href="{ string($node/t:ref[1]/@target) }" target="_blank">{ string($node/@n) }</a>
				else if ($node/@n) then
					string($node/@n)
				else
					" "
...
declare %private function viewItem:app($node as element(t:app)) {
	if ($node/parent::t:lem) then (
		...
		<sup id="{ $node/ancestor::t:div[@type = "textpart"][1]/@n }appNote{ generate-id($node) }">
			<a href="#{ $node/ancestor::t:div[@type = "textpart"][1]/@n }appPointer{ generate-id($node) }">
...
declare %private function viewItem:wit($node) {
	...
	return <span>
		{
			if ($node/@resp) then (
				attribute data-resp { string($node/@resp) }, attribute class { "w3-tooltip RdgRespMs" }
			) else (
				attribute class { "w3-tooltip" }
			)
		}
		{ $trimmedid }
		<span class="w3-text">{ <a href="{ string($witness/@corresp) }">{ $witness/t:idno/text() }</a> }</span>
	</span>
};
...
declare %private function viewItem:lem($node as element(t:lem)) {
	let $resp := $node/@resp
	let $listWit := $node/ancestor::t:TEI//t:listWit
	return (viewItem:TEI2HTML($node/node()), " ", viewItem:wit($node), " ")
};
...
declare function viewItem:div($node as element(t:div)) {
	if ($node[@type = "apparatus"]) then
		<div class="row-fluid" id="apparatus">
			<hr />
			{
				for $app at $p in $node/t:app
				return (
					<a href="{ $app/@from }">{ concat(substring-after($app/@from, "#"), ", ", $app/@loc, " ") }</a>,
	...
	else if ($node[parent::t:body][not(@type = "apparatus")]) then (
		<div
			class="w3-row"
			id="{
				if ($node/@xml:id) then
		...
		<br />
	) else if ($node[@type = "textpart"] | $node[@type = "edition"][not(child::t:div)]) then
		<div id="{ viewItem:DTSpartID($node) }">
			{
			...
			}
			{
				...
				return if ($node/child::t:div[@type = "textpart"]) then (
					viewItem:titletemplate($node, $text),
					for $child in $node/node()
					return if ($child/self::t:div) then
						viewItem:div($child)
					else
						viewItem:TEI2HTML($child)
				) else (
					<div
						class="{
					...
					<div id="text-{ $node/@xml:id }">
						...
						{
							(: if($node/t:ab) then viewItem:TEI2HTML($node/node()[not(self::t:label)]) else :) viewItem:TEI2HTML(
								$node/node()
							)
						}
					</div>,
						...
						<div class="w3-third row apparata ">
							{
								...
								order by $p
								return viewItem:applisting($app, $p)
							}
							<hr />
							{
								...
								return (
									...
									for $a at $p in $app
									return viewItem:applisting($a, $p)
								)
							}
						</div>
					else (
					),
					<div id="AllQuotations{ $node/@n }" />
				)
			}
		</div>
	else
	...
	<div class="w3-bar">
		<div class="w3-bar-item">
			<i>
				...
				{
					if ($div/parent::t:div[@type = "edition"]/@resp) then
					...
					if ($div/@corresp) then (
						...
						(
							if (starts-with($div/@subtype, "#")) then (
								...
								return (viewItem:TEI2HTML($match/t:title), <a href="{ $div/@corresp }">{ $id }</a>)
							) else (
								<a href="{ $div/@corresp }">
				...
				{
					if ($div/@corresp and $div/ancestor::t:TEI/@type = "work") then
		...
		</div>
		{
			if ($div/t:ab[descendant::text()]) then (
				<div class="ugaritcontrols w3-tooltip w3-bar-item w3-button w3-padding-small w3-gray w3-right">
					...
					>Start a translation alignment with Alpheios Alignment. You can also add morphological annotations there. See <a
							href="https://github.com/BetaMasaheft/alpheiosannotations"
						>instructions in GitHub</a>. </span>
				</div>,
				if ($div/@corresp) then
		...
		<a class="page-scroll w3-button w3-padding-small w3-right w3-bar-item w3-gray" href="#transcription">back to top</a>
	</div>
};

declare %private function viewItem:applisting($app, $p) {
	<span id="{ string($app/ancestor::t:div[@type = "textpart"][1]/@n) }appPointer{ $p }">
		<a href="#{ string($app/ancestor::t:div[@type = "textpart"][1]/@n) }appnote{ $p }">
        ...
        {
			for $r in $app/*:rdg
			return try { viewItem:rdg($r) } catch * { $err:description }
		}
...
declare %private function viewItem:DTSpartID($node) {
	if (count($node/t:ab/t:cb) ge 1) then
...
declare function viewItem:TEI2HTML($nodes) {
	...
	return typeswitch ($node)
		...
		case element(t:ab) return
			viewItem:ab($node)
		case element(t:abbr) return
			viewItem:abbr($node)
		case element(t:accMat) return
			viewItem:accMat($node)
		case element(t:acquisition) return
			viewItem:acquisition($node)
		case element(t:add) return
			viewItem:add($node)
		case element(t:additions) return
			viewItem:additions($node)
		case element(t:adminInfo) return
			viewItem:adminInfo($node)
		case element(t:altIdentifier) return
			viewItem:altIdentifier($node)
		case element(t:app) return
			viewItem:app($node)
		case element(t:bibl) return
			viewItem:bibliographyitem($node)
		case element(t:bindingDesc) return
			viewItem:bindingDesc($node)
		case element(t:birth) return
			viewItem:date-like($node)
		case element(t:cb) return
			viewItem:cb($node)
		case element(t:certainty) return
			viewItem:certainty($node)
		case element(t:choice) return
			viewItem:choice($node)
		case element(t:cit) return
			viewItem:cit($node)
		case element(t:citeStructure) return
			()
		case element(t:classDecl) return
			()
		case element(t:collation) return
			viewItem:VisColl($node)
		case element(t:collection) return
			viewItem:collectionelement($node)
		case element(t:colophon) return
			viewItem:colophon($node)
		case element(t:condition) return
			viewItem:condition($node)
		case element(t:country) return
			viewItem:namedEntity($node)
		case element(t:custEvent) return
			viewItem:custEvent($node)
		case element(t:date) return
			viewItem:date-like($node)
		case element(t:death) return
			viewItem:date-like($node)
		case element(t:decoDesc) return
			viewItem:decoDesc($node)
		case element(t:del) return
			viewItem:del($node)
		case element(t:desc) return
			viewItem:desc($node)
		case element(t:dimensions) return
			viewItem:dimensions($node)
		case element(t:div) return
			viewItem:div($node)
		case element(t:ex) return
			viewItem:ex($node)
		case element(t:explicit) return
			viewItem:explicit($node)
		case element(t:extent) return
			viewItem:extent($node)
		case element(t:facsimile) return
			()
		case element(t:faith) return
			viewItem:faith($node)
		case element(t:figure) return
			viewItem:figure($node)
		case element(t:filiation) return
			viewItem:filiation($node)
		case element(t:foliation) return
			viewItem:foliation($node)
		case element(t:floruit) return
			viewItem:date-like($node)
		case element(t:forename) return
			viewItem:forename($node)
		case element(t:gap) return
			viewItem:gap($node)
		case element(t:handDesc) return
			viewItem:palaeography($node)
		case element(t:handShift) return
			viewItem:handShift($node)
		case element(t:hi) return
			viewItem:hi($node)
		case element(t:history) return
			viewItem:history($node)
		case element(t:idno) return
			viewItem:idno($node)
		case element(t:incipit) return
			viewItem:incipit($node)
		case element(t:item) return
			viewItem:item($node)
		case element(t:layoutDesc) return
			viewItem:layoutDesc($node)
		case element(t:lem) return
			viewItem:lem($node)
		case element(t:list) return
			viewItem:list($node)
		case element(t:listBibl) return
			viewItem:listBibl($node)
		case element(t:label) return
			viewItem:label($node)
		case element(t:lb) return
			viewItem:lb($node)
		case element(t:lg) return
			viewItem:lg($node)
		case element(t:l) return
			viewItem:l($node)
		case element(t:locus) return
			viewItem:locus($node)
		case element(t:measure) return
			viewItem:measureelement($node)
		case element(t:metamark) return
			()
		case element(t:msContents) return
			viewItem:msContents($node)
		case element(t:msDesc) return
			viewItem:manuscriptStructure($node)
		case element(t:msFrag) return
			viewItem:codicologicalUnit($node)
		case element(t:msItem) return
			viewItem:msItem($node)
		case element(t:msPart) return
			viewItem:codicologicalUnit($node)
		case element(t:nationality) return
			viewItem:nationality($node)
		case element(t:note) return
			viewItem:note($node)
		case element(t:orig) return
			viewItem:orig($node)
		case element(t:origDate) return
			viewItem:date-like($node)
		case element(t:origin) return
			viewItem:TEI2HTML($node/node()[not(self::t:provenance)])
		case element(t:origPlace) return
			viewItem:origplace($node)
		case element(t:p) return
			<p>{ viewItem:TEI2HTML($node/node()) }</p>
		case element(t:pb) return
			viewItem:pb($node)
		case element(t:persName) return
			viewItem:namedEntity($node)
		case element(t:placeName) return
			viewItem:namedEntity($node)
		case element(t:provenance) return
			viewItem:provenance($node)
		case element(t:ptr) return
			viewItem:ptr($node)
		case element(t:q) return
			viewItem:qelement($node)
		case element(t:rdg) return
			viewItem:rdg($node)
		case element(t:region) return
			viewItem:namedEntity($node)
		case element(t:relation) return
			viewItem:relation($node)
		case element(t:ref) return
			viewItem:ref($node)
		case element(t:repository) return
			viewItem:repository($node)
		case element(t:revisionDesc) return
			viewItem:revisionDesc($node)
		case element(t:roleName) return
			viewItem:roleName($node)
		case element(t:rs) return
			viewItem:rs($node)
		case element(t:rubric) return
			viewItem:rubric($node)
		case element(t:sealDesc) return
			viewItem:sealDesc($node)
		case element(t:seg) return
			viewItem:seg($node)
		case element(t:settlement) return
			viewItem:namedEntity($node)
		case element(t:sic) return
			viewItem:sic($node)
		case element(t:signatures) return
			viewItem:signatures($node)
		case element(t:space) return
			viewItem:space($node)
		case element(t:summary) return
			viewItem:summary($node)
		case element(t:supplied) return
			viewItem:supplied($node)
		case element(t:supportDesc) return
			viewItem:supportDesc($node)
		case element(t:surname) return
			viewItem:surname($node)
		case element(t:surrogates) return
			viewItem:surrogates($node)
		case element(t:surplus) return
			viewItem:surplus($node)
		case element(t:table) return
			viewItem:table($node)
		case element(t:textLang) return
			viewItem:textLang($node)
		case element(t:term) return
			viewItem:term($node)
		case element(t:title) return
			viewItem:namedEntity($node)
		case element(t:unclear) return
			viewItem:unclear($node)
		case element(t:witness) return
			viewItem:witness($node)
		case element(t:watermark) return
...
declare %private function viewItem:tokenize-text($node) {
	...
	return if (contains($node, "፡") and not($lang = "am") and not($lang = "ti")) then
...
declare %private function viewItem:standards($item) {
	...
	(: :if ($item//t:editionStmt) then
        ...
        (),: :)
	<button class="w3-button w3-gray w3-bar-item" onclick="openAccordion('pub')">Publication Statement</button>,
	<ul class="w3-bar-item w3-hide" id="pub">{ viewItem:publicationStmt($item//t:publicationStmt) }</ul>,
	<button class="w3-button w3-gray w3-bar-item" onclick="openAccordion('enc')">Encoding Description</button>,
	...
	<div class="w3-hide">
		{
			...
			return <span id="{ $r }Name">{ $viewItem:editors//t:item[@xml:id = $r]/text() }</span>
		}
	</div>
};
...
declare %private function viewItem:relsinfoblock($rels, $id) {
	<p>
		{
			let $notFormrly := $rels[not(@name = "betmas:formerlyAlsoListedAs")][not(
				@name = "betmas:isAuthorOfEthiopicTranslation"
			)][@name = "saws:isAttributedToAuthor"][@name = "dc:creator"]
	...
	<p class="w3-tiny">For a table of all relations from and to this record,
        please go to the <a
			class="w3-tag w3-gray"
			href="{ $config:appUrl }/{ switch2:col(switch2:switchPrefix($id)) }/{ $id }/analytic"
		>Relations</a> view.
...
declare %private function viewItem:divofperson($item, $element) {
	if ($element = "persName") then (
		...
		return if (count($this) ge 1) then
			<div class="w3-container" id="{ $element }">
				<h4>
					{
...
declare %private function viewItem:divofmanuscript($msDesc, $element, $label) {
	...
	return if (count($this) ge 1) then
		<div class="w3-container w3-margin-bottom" id="{ $this/@xml:id }{ $label }">{ viewItem:TEI2HTML($this) }</div>
	else (
	)
};
...
declare %private function viewItem:divofmanuscriptpath($msDesc, $path, $label) {
	...
	return if (count($this) ge 1) then
		<div class="w3-container w3-margin-bottom" id="{ $this/@xml:id }{ $label }">
			{
				if (ends-with($path, "msContents")) then
					viewItem:msContents($this)
				else
	...
	if ($path = "//t:placeName") then (
		...
		return if (count($this) ge 1) then (
			if ($level = 2) then
				...
				<h3>{ $label }</h3>,
			<p class="w3-container w3-margin-bottom">{ viewItem:TEI2HTML($this) }</p>
		) else (
		)
};
...
declare %private function viewItem:manuscriptStructure($msDesc) {
	...
	(
		<div class="w3-twothird well" id="textualcontents{ $msDesc/@xml:id }">
			<div class="w3-half">{ viewItem:divofmanuscript($msDesc, "history", "history") }</div>
			<div class="w3-half">{ viewItem:partofmanuscript($msDesc, "/t:msContents/t:summary") }</div>
		...
		<div class="w3-third w3-border-left" id="codicologicalInformation{ $msDesc/@xml:id }">
			...
			{
				viewItem:divofmanuscriptpath($msDesc, "/t:physDesc//t:objectDesc/t:layoutDesc", "layout")
			(: dimensions again! :) }
			{ viewItem:divofmanuscriptpath($msDesc, "/t:physDesc/t:handDesc", "hands") }
			{
				...
				return if ($TEI//t:persName[@role]) then
					if ($isPart) then
...
declare function viewItem:persWithRole($ms) {
	<div class="w3-panel w3-red w3-card-4 w3-margin-bottom" id="perswithrole_{ $ms/@xml:id }">
		{
			...
			return (
				<a xmlns="http://www.w3.org/1999/xhtml" class="persName" href="{ $ref }">
					{
...
declare %private function viewItem:codicologicalUnit($mspart) {
	<div class="w3-container w3-margin-bottom" id="{ $mspart/@xml:id }">
		<div class="w3-container w3-margin-bottom"><h2>Codicological Unit { substring($mspart/@xml:id, 1) }</h2></div>
		<div class="w3-twothird" id="textualcontents{ $mspart/@xml:id }">
			<div class="w3-panel w3-card-2 w3-margin-right">{ viewItem:TEI2HTML($mspart/t:msIdentifier) }</div>
		</div>
...
declare %private function viewItem:calendartables($item) {
	...
	let $id := generate-id($date)
	return <div class="w3-hide">
		<div class="w3-responsive w3-hide popuptext" id="dateInfo{ $id }">
			<table class="w3-table w3-hoverable">
				<thead><tr><th>info</th><th>value</th></tr></thead>
				<tbody>
					<tr><td>Standard date</td><td>{ viewItem:dates($date) }</td></tr>
					<tr><td>Date in current calendar</td><td>{ viewItem:dates-custom($date) }</td></tr>
					<tr><td>Calendar</td><td>{ string($date/@calendar) }</td></tr>
					{
						if ($date/@dur) then
							<tr><td>Duration</td><td>{ string($date/@dur) }</td></tr>
						else (
						)
					}
					{
						if ($date/@type) then
							<tr><td>Date type</td><td>{ string($date/@type) }</td></tr>
						else (
						)
					}
					{
						if ($date/@evidence) then
							<tr><td>Evidence</td><td>{ string($date/@evidence) }</td></tr>
						else (
						)
					}
					{
						if ($date/@cert) then
							<tr><td>Certainty</td><td>{ string($date/@cert) }</td></tr>
						else (
						)
					}
					{
						if ($date/@resp) then
							<tr><td>Attribution</td><td>{ viewItem:tipResp($date/@resp, "attributed by") }</td></tr>
						else (
						)
					}
				</tbody>
...
declare function viewItem:q($q) {
	(: replaces q.xsl :)
	<div class="w3-container w3-gray">{ viewItem:TEI2HTML($q) }</div>,
	for $n in $q//t:note
	return <div class="w3-container">{ viewItem:TEI2HTML($n) }</div>
};
...
declare function viewItem:dates-custom($date) {
	...
	) else if ($date/@notAfter-custom and not($date/@notBefore-custom)) then (
		"Before " || viewItem:date($date/@notAfter-custom)
	) else if (not($date/@notAfter-custom) and $date/@notBefore-custom) then (
		"After " || viewItem:date($date/@notBefore-custom)
	) else if ($date/@when-custom) then
...
declare function viewItem:dates($date) {
	...
	let $dates := if ($date/@when) then
		...
		) else if ($date/@from and not($date/@to)) then (
			"Before " || viewItem:date($date/@to)
		) else if (not($date/@from) and $date/@to) then (
		...
		) else if ($date/@notAfter and not($date/@notBefore)) then (
			"Before " || viewItem:date($date/@notAfter)
		) else if (not($date/@notAfter) and $date/@notBefore) then (
	...
	let $formatortext := if (count($date/node()) gt 1) then
		...
		$dates
	return ($formatortext, $evidence, $cert, $resp)
};
...
return :)
	<div>
		<div id="transcription{ $frag/descendant-or-self::*[@xml:id][1]/@xml:id }" style="word-break: keep-all;">
			{
			...
			<div class="w3-modal" id="textHelp{ $frag/descendant-or-self::*[@xml:id][1]/@xml:id }">
				<div class="w3-modal-content">
					...
					>
                        ...
                        <p
						>In the text navigation bar:</p>
						<ul class="nodot">
							<li
							>References are relative to the current level of the view. If you want to see further navigation levels, please click the arrow to open in another page.</li>
							<li
							>Each reference available for the current view can be clicked to scroll to that point. alternatively you can view the section clicking on the arrow.</li>
							<li
							>Using a hyphen between references, like LIT3122Galaw.1-2 you can get a view of these two sections only</li>
							<li
		...
		<div class="w3-container" id="roleAttestations{ $frag/descendant-or-self::*[@xml:id][1]/@xml:id }" />
		<div class="w3-hide">
			{
				for $r in distinct-values($frag//@resp)
				return <span id="{ $r }Name">{ viewItem:editorName($r) }</span>
			}
		</div>
	</div>
};
	...
	if (string-length($ref) != 2) then (
		...
		try {
			if ($viewItem:editors//t:item[@xml:id = $ref]) then
				...
				string($ref)
		} catch * { util:log("info", concat("failed parsing with viewItem:editorName ", $ref)) }
	)
};

declare function viewItem:textfragmentbibl($this, $id) {
	(: replaces textfragmentbibl.xsl :)
	if ($this//t:listBibl) then
		<div class="w3-container" id="bibliographyText{ $id }">{ viewItem:TEI2HTML($this//t:listBibl) }</div>
	else (
	),
	...
	(
		...
		if ($item//t:placeName) then
			<a
				...
				href="{ $config:appUrl }/IndexPlaces?entity={ string($item/@xml:id) }"
			>Places Index</a>
		else (
		),
		if ($item//t:persName) then
			<a
				class="w3-bar-item page-scroll"
				href="{ $config:appUrl }/IndexPersons?entity={ string($item/@xml:id) }"
			>Persons Index</a>
		else (
		),
		if ($item//t:body[t:div[@type = "edition"][t:ab or t:div[@type = "textpart"]]]) then
			<a class="w3-bar-item page-scroll w3-red" href="{ $config:appUrl }/works/{ $item/@xml:id }/text">Text</a>
		else (
		),
		if ($item//t:body[t:div[@type = "translation"][t:ab or t:div[@type = "textpart"]]]) then
			<a class="w3-bar-item page-scroll w3-red" href="{ $config:appUrl }/works/{ $item/@xml:id }/text">Translation</a>
		else (
		),
...
declare function viewItem:personnav($item) {
	(
		<a
			class="w3-bar-item page-scroll"
			href="{ $config:appUrl }/IndexPersons?pointer={ string($item/@xml:id) }"
		>Persons Index</a>,
...
declare function viewItem:placenav($item) {
	(
		<a
			class="w3-bar-item page-scroll"
			href="{ $config:appUrl }/IndexPlaces?pointer={ string($item/@xml:id) }"
		>Places Index</a>,
		<a class="w3-bar-item page-scroll" href="#general">General</a>,
		<a class="w3-bar-item page-scroll" href="#description">Description</a>,
...
declare function viewItem:manuscriptnav($item) {
	(
            ...
            <ul>
					{
						...
						return <li>
							<a class="page-scroll" href="#{ $part/@xml:id }">Codicological unit { substring($part/@xml:id, 1) }</a>
						</li>
					}
				</ul>
			</div>
		...
		<ul class="w3-bar-item w3-hide" id="NavByIds">
			{
				for $node at $p in
					$item//t:*[not(self::t:TEI)][not(self::t:category)][not(self::t:editor)][not(self::t:calendar)][@xml:id]
...
declare function viewItem:switchsubids($anchor, $node) {
	if ($anchor = "ms") then
		...
		" Item " || substring-after($anchor, "_i")
	(: || string-join(viewItem:headercontext($node)) ,
                                            if(count($node/t:title) ge 1) then viewItem:TEI2HTML($node/t:title[1]) else () :) ) else if (
		starts-with($anchor, "q") and matches($anchor, "\w\d+")
	) then
		" Quire " || substring($anchor, 1) || string-join(viewItem:headercontext($node))
	else if (matches($anchor, "^h\d+$")) then
		" Hand " || substring($anchor, 1) || string-join(viewItem:headercontext($node))
	else
		$node/name()
};
...
declare function viewItem:capitalize-first($arg as xs:string?) as xs:string? {
	concat(upper-case(substring(string($arg), 1, 1)), substring($arg, 2))
};
...
declare %private function number:RomanToInteger($romannumber, $followingvalue) {
	if (ends-with($romannumber, "CM")) then
		...
		(
			...
			number:RomanToInteger(substring($romannumber, 1, string-length($romannumber) - 1), 10)
	else if (ends-with($romannumber, "IV")) then
		4 + number:RomanToInteger(substring($romannumber, 1, string-length($romannumber) - 2), 4)
	else if (ends-with($romannumber, "V")) then
		5 + number:RomanToInteger(substring($romannumber, 1, string-length($romannumber) - 1), 5)
	else if (ends-with($romannumber, "I")) then
		(
			...
			number:RomanToInteger(substring($romannumber, 1, string-length($romannumber) - 1), 1)
	else
		0
};
...
declare %private function viewItem:analyseMeasure($measure) {
	...
	return if (matches($measure, $regex1)) then
		let $analyse := analyze-string($measure, $regex1)
		return (
			for $m in $analyse/s:match
			return (
				<beginning>{ $m/s:group[@nr = "1"] }</beginning>,
				<text>{ $m/s:group[@nr = "2"] }</text>,
				<end>{ $m/s:group[@nr = "3"] }</end>
			),
			for $n in $analyse/s:not-match
			return $n
		)
	else if (matches($measure, $regex2)) then
		let $analyse := analyze-string($measure, $regex2)
		return (
			for $m in $analyse/s:match
			return let $values := <vals>
					<val>{ $m/s:group[@nr = "1"]/text() }</val>
					<val>{ $m/s:group[@nr = "2"]/text() }</val>
				</vals>
				let $max := max($values//*:val)
				let $min := min($values//*:val)
				return (
					if ($min = xs:integer($m/s:group[@nr = "1"]/text())) then
						<beginning>{ $min }</beginning>
					else (
					),
					<text>{ $max }</text>,
	...
	else if (matches($measure, $regex5)) then
		let $analyse := analyze-string($measure, $regex5)
		return (
			for $m in $analyse/s:match
			return (
				<text>{ $m/s:group[@nr = "1"]/text() }</text>,
				<end>{ format-number(number:RomanToInteger($m/s:group[@nr = "2"]/text(), 0), "####") }</end>
			),
			for $n in $analyse/s:not-match
			return $n
		)
	else if (matches($measure, $regex6)) then
		let $analyse := analyze-string($measure, $regex6)
		return (
			for $m in $analyse/s:match
			return viewItem:analyseMeasure($m/s:group[@nr = "1"]/text())
		)
	else
		$measure
};
...
declare %private function viewItem:measure($measure) {
	...
	let $totalprotectives := if ($parsedMeasure//*:beginning and $parsedMeasure//*:end) then
		xs:integer($parsedMeasure//*:beginning/data()) + xs:integer($parsedMeasure//*:end/data())
	else if ($parsedMeasure//*:beginning and not($parsedMeasure//*:end)) then
		xs:integer($parsedMeasure//*:beginning/data())
	else if ($parsedMeasure//*:end and not($parsedMeasure//*:beginning)) then
		xs:integer($parsedMeasure//*:end/data())
	else (
	)
	let $seq := (
		if ($parsedMeasure//*:beginning) then
		...
		if ($parsedMeasure//*:end) then (
			"+",
			if ((xs:integer($parsedMeasure//*:end/data()) gt 1) and $parsedMeasure//*:beginning) then
...
declare function viewItem:n2roman($num as xs:integer) as xs:string {
	...
	let $values := (
		...
		<value char="IX" num="9" />,
		<value char="X" num="10" />,
		<value char="XL" num="40" />,
		<value char="L" num="50" />,
		<value char="XC" num="90" />,
		<value char="C" num="100" />,
		<value char="CD" num="400" />,
		<value char="D" num="500" />,
		<value char="CM" num="900" />,
		<value char="M" num="1000" />
	)
	return (: as long as we have a number, keep going :) if ($num) then
		(: reduce by the largest number that has a string value :)

		for $val in $values[@num <= $num][fn:last()]
		return (: using the highest value :) fn:concat($val/@char, viewItem:n2roman($num - xs:integer($val/@num)))
	(: nothing left :)
	else
		""
};
...
declare %private function viewItem:fulllang($lang) {
	try { $viewItem:lang//t:item[@xml:id = $lang]/text() } catch * { util:log("INFO", $lang) }
};

declare %private function viewItem:keywordtag($x) {
	<span class="w3-tag w3-gray">
		<a href="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;termkey={ $x }" target="_blank">
			{ exptit:printTitleID($x) }
		</a>
	</span>
};
...
declare %private function viewItem:keywordgroupmulti($keys) {
	if ($keys) then
		<div class="w3-row">
			{
				if (contains($keys, " ")) then
					for $x in tokenize($keys, " ")
					return viewItem:keywordtag($x)
				else
					let $type := $keys
					return viewItem:keywordtag($type)
			}
		</div>
	else (
	)
...
declare function viewItem:keywords($file, $collection) {
	...
	let $classes :=
		for $class in $file//t:term/@key
		return $config:appUrl || "/" || $class
	let $options := switch ($collection)
		...
		default return
			viewItem:keywordgroup($file//t:term/@key)
	return if (count($options) gt 1) then
		<div class="w3-container" id="keywordslist"><h3>Keywords</h3>{ $options }</div>
	else (
	)
};
...
declare function viewItem:zotero($file) {
	let $id := string($file/@xml:id)
	return <div class="w3-container" id="zoterolink">
			...
			<a
				href="{
					concat(
						"https://api.zotero.org/groups/358366/items?tag=",
						$id,
						"&amp;format=bib&amp;style=hiob-ludolf-centre-for-ethiopian-studies-with-url-doi&amp;linkwrap=1"
					)
				}  target='_blank'"
			>Click on this link to view bibliography mentioning this record</a>
		</div>
	</div>
};
...
declare function viewItem:osm($item) {
	...
	return if ($item//t:location/t:geo) then
		<div class="w3-container w3-padding float">
			<iframe
				height="350"
				src="https://www.openstreetmap.org/export/embed.html?bbox=33.68408203125001%2C11.646856393732364%2C42.857666015625%2C16.883403464236842&amp;layer=mapnik&amp;marker={
					$geo1
				}%2C{ $geo2 }"
			...
			<small>
				<a
					href="https://www.openstreetmap.org/?mlat={ $geo1 }&amp;mlon={ $geo2 }#map=8/{ $geo1 }/{ $geo2 }"
				>Show a larger map</a>
			</small>
		</div>
	else (
