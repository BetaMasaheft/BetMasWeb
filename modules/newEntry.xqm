...
declare function new:newentry($node as node()*, $model as map(*)) {
	...
	let $option := switch ($app:collection)
		...
		default return
			"last part of the"
	return <form action="edit/save-new-entity.xql" id="createnew">
		<p>Great { sm:id()//sm:username/text() }! So you are creating a new record in <span class="w3-tag w3-red w3-large">
				{ $app:collection }
			</span>, that is great news!</p>
		<fieldset class="w3-container">
			<legend>General data</legend>
			<div class="w3-container w3-margin-bottom">
				<input class="w3-input" name="collection" type="hidden" value="{ $app:collection }" />
			</div>
			<div class="w3-container w3-margin-bottom">
				<label>ID</label>
				<br />
			...
			<div class="w3-container w3-margin-bottom">
				<label>Title</label>
				<br />
				<input class="w3-input" id="title" name="title" required="required" />
				<br />
			...
			</div>
			{
				if ($app:collection = "persons" or $app:collection = "places") then (
					<div class="w3-container w3-margin-bottom">
						<label>WikiData (with wd: prefix)</label>
						<br />
						<input
							class="w3-input"
							id="Wdata"
							list="WDhits"
							name="WD"
							placeholder="enter the Q item id in the form wd:Q12345"
							type="text" />
					</div>
				) else (
				)
			}
			{
				if ($app:collection = "authority-files") then (
					...
					if ($app:collection = "persons") then (
						...
						<div class="w3-container w3-margin-bottom">
							...
							<div class="w3-container" id="formKeywords">
								{
									let $categories := switch ($app:collection)
										case "narratives" return
											$taxonomy/t:category[t:desc = "Subjects"]/t:category
										case "works" return
											$taxonomy/t:category[t:desc = "Subjects"]/t:category
										case "studies" return
											$taxonomy/t:category[t:desc = "Subjects"]/t:category
										case "manuscripts" return
											$taxonomy/t:category[t:desc = "Subjects"]/t:category
										case "places" return
											$taxonomy/t:category[t:desc = "Place types"]/t:category
										case "institutions" return
											$taxonomy/t:category[t:desc = "Place types"]/t:category
										default return
											$taxonomy/t:category/t:category
									for $k in $categories
									order by $k/@xml:id
									return <span class="w3-tag w3-gray w3-margin">
										<input class="w3-check" name="keywords" type="checkbox" value="{ data($k/@xml:id) }" />
					...
					<div class="w3-container w3-margin-bottom">
						<label>Note</label>
						<br />
						<textarea class="w3-input" id="note" name="note" />
						<br />
					...
					<div class="w3-container w3-margin-bottom">
						...
						>Any relation needed for sure? select as many as you want. You will have to complete these in your file after download with passive attribute values</small>
					</div>
				)
			}
		</fieldset>
        ...
        Group
					...
					<div class="w3-container w3-margin-bottom">
						<label>Birth</label>
						<br />
						<input class="w3-input" id="birth" name="birth" type="text" />
						<br />
						<small
							class="form-text text-muted"
						>Please, enter a well formatted date or a range and remember to update attributes </small>
					</div>,
					<div class="w3-container w3-margin-bottom">
						<label>Death</label>
						<br />
						<input class="w3-input" id="death" name="death" type="text" />
						<br />
						<small
							class="form-text text-muted"
						>Please, enter a well formatted date or a range and remember to update attributes </small>
					...
					<div class="w3-container w3-margin-bottom">
						...
						<br />
						<input class="w3-input" id="floruit" name="floruit" type="text" />
						<br />
						<small
							class="form-text text-muted"
						>Please, enter a well formatted date or a range and remember to update attributes </small>
        ...
        Title
						...
						<small
							class="form-text text-muted"
						>Do you know where is this entity attested? Check the type of input you are giving then start typing an ID or a title to get a list.</small>
					</div>,
					<div class="w3-container w3-margin-bottom">
						<label>Gender</label>
						<div id="gender">
							<label><input class="w3-check" id="male" name="gender" type="radio" value="1" />
        Male
      </label>
							<label class="form-check-label">
								<input class="w3-check" id="female" name="gender" type="radio" value="2" />
        Female
      </label>
						</div>
					</div>
					...
					<div class="w3-container w3-margin-bottom">
						...
						<br />
						<select class="w3-select" id="institution" name="institution">
							{
								for $i in doc("/db/apps/lists/institutions.xml")//t:item
								let $title := $i/string()
								order by $title
								return <option value="{ string($i/@xml:id) }">{ $title }</option>
							}
						</select>
						<br />
						<small
							class="form-text text-muted"
						>select institution where manuscript is stored. If this is a new institution, <a
								href="/newentry.html?collection=institutions"
								target="_blank"
							>please first create the institution record</a>
						</small>
					</div>,
					<div class="w3-container w3-margin-bottom">
						<label>Identifier</label>
						<br />
						<input class="w3-input" id="idno" name="idno" required="required" />
						<br />
						<small class="form-text text-muted">type here the canonical identifier for this manuscript</small>
					</div>,
					<div class="w3-container w3-margin-bottom">
						<label>Number of msParts needed</label>
						<br />
						<input class="w3-input" id="msParts" name="msParts" type="number" />
						<br />
						<small
							class="form-text text-muted"
						>You might specify here how many msParts you think you will need. Default is none.</small>
					</div>
        ...
        Title
						...
						<input
							...
							list="gotohits"
							placeholder="select..." />
						<select class="form-control" id="gotohits" multiple="multiple" name="attested">
							<option value="">Nothing yet to be selected</option>
						</select>
						<br />
						<small
