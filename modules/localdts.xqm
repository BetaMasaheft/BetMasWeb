...
declare function localdts:Collection($id as xs:string*, $page as xs:integer*, $nav as xs:string*) as map(*) {
	if (
		...
		return if (matches($parsedURN//s:group[@nr = 2], "(textualunits|narrativeunits|transcriptions|studies)")) then (
			...
			return localdts:CollMember($id, $edition, $specificID, $page, $nav, "")
		) else
			localdts:Coll($id, $page, $nav, "")
	else (
	)
...
declare function localdts:Navigation(
	...
	let $witnesses :=
		for $witness in config:distinct-values($allwits)
...
declare function localdts:Annotations(
	$coll as xs:string*,
	$BMid as xs:string*,
	$begin as xs:string*,
	$page as xs:string*,
