...
declare variable $q:name := request:get-parameter("name", ());

declare variable $q:searchType := request:get-parameter("searchType", "text");

declare variable $q:mode := request:get-parameter("mode", "none");

declare variable $q:defop := request:get-parameter("defaultoperator", ());

declare variable $q:sort := if (request:get-parameter("sort", ())) then
...
declare variable $q:TEIrangeFields := doc(
	"/db/system/config/db/apps/expanded/collection.xconf"
)//xconf:range/xconf:create/xconf:field/@name;
...
declare function q:querytype($node as node(), $model as map(*)) {
	...
	return <select class="w3-select" id="SType" name="searchType">
		<option value="text">
			{
...
declare function q:textquerydefaultoperator($node as node(), $model as map(*)) {
	...
	return <select class="w3-select" name="defaultoperator" style="padding:0px 0px;">
		<option value="OR">
			{
...
declare function q:homophonecheckbox($node as node()*, $model as map(*)) {
	...
	return <input name="homophones" type="checkbox">
		{
...
declare function q:ranking($node as node()*, $model as map(*)) {
	...
	return <input name="sort" type="checkbox">
		{
			if ($h = "on") then
				attribute checked { "checked" }
			else (
...
declare function q:translitcheckbox($node as node()*, $model as map(*)) {
	...
	return <input name="translit" type="checkbox">
		{
			if ($h = "on") then
				attribute checked { "checked" }
			else (
...
declare function q:query($node as node()*, $model as map(*), $query as xs:string*) {
	...
	let $t := if (request:get-parameter("searchType", ())) then
		...
		"undefined"
	return if ((string-length($query) lt 1) and ($t != "fields") and (count($q:params) = 0)) then (
		util:log("info", ("doing nothing there is no query and no fields, stated parameters names: ", count($q:params)))
	) else if (
		count($q:params) = 3 and
			$q:params = "searchType" and
			$q:params = "mode" and
			$q:params = "query" and
			(string-length($query) lt 1)
	) then (
		util:log(
			"info",
			(
				"doing nothing there is no query and no fields, stated parameters names are only the necessary ones: ",
				count($q:params),
				": ",
				string-join($q:params, ","),
				" should simply return the empty form."
			)
		)
	) else
		...
		return map {"runtime": $runtime-ms, "type": $t, "query": $query, "hits": $hits("tei"), "qs": $hits("qs")}
};
...
like the facet, simple and advanced searches :)
		...
		case "clavis" return
			q:clavis($q)
		case "otherclavis" return
			q:otherclavis($q)
		case "placeSearch" return
			q:placeSearch($q)
		case "bmid" return
			q:bmid($q)
		case "linkeddata" return
			q:linkeddata($q)
	...
	<div>
		{
			if ($model("type") = "bibliography") then
				<p>There are <span xmlns="http://www.w3.org/1999/xhtml" class="w3-tag w3-gray" id="hit-count">
						{ count($model("hits")) }
					</span>
                    distinct bibliographical references</p>
			else if ($model("type") = "matches") then
				<p>You found <span class="w3-tag w3-gray">
						{ q:create-field-query($model("query"), $q:mode) }
					</span> in
                        <span
						xmlns="http://www.w3.org/1999/xhtml"
						class="w3-tag w3-gray"
						id="hit-count"
					>{ count($model("hits")) }</span> results</p>
			else (
				<p> There are <span xmlns="http://www.w3.org/1999/xhtml" class="w3-tag w3-gray" id="hit-count">
						{
                        ...
                        <span class="w3-label w3-gray">
						{ $q:searchType }
					</span>
					<span class="w3-tooltip" style="word-break:break-all"> query for "{
							$model("query")
						}" with the parameters shown on the right.
                            <span class="w3-text"> (searched: <em>
								{
									if ($q:searchType != "sparql") then
			...
			<span>{ "Search time: " }<span class="w3-badge">{ $model("runtime") div 1000 }</span>{ " seconds." }</span>
		}
	</div>
};
...
declare function q:placeSearch($place) {
	if (not(starts-with($place, "LOC"))) then (
		...
		map {"qs": "sorry, this is not a valid place identifier, it should start with LOC"}
	) else
		...
		let $allrepositories :=
			for $repo in
				(
					$q:col//t:settlement[@ref eq $placeuri],
			...
			return string($repo/ancestor::t:TEI/@xml:id)
		...
		let $selected := if (count($repositoriesIDS) ge 1) then
			let $fullrepoids :=
				for $r in $repositoriesIDS
				return ($config:appUrl || "/" || $r)

			let $t3 := util:log("info", string-join($fullrepoids, " - "))
			return $q:col//t:repository[@ref eq $fullrepoids]
		else (
		)
...
declare function q:xpath($q, $params) {
	let $q :=
		for $query in normalize-space($q)
		return if ($q = "") then (
		) else
			$q
...
declare function q:sparql($q) {
	...
	return map {"tei": $results, "qs": $allquery}
};
...
declare function q:ListQueryParam-rest($parameter, $context, $mode, $function) {
	...
	return if ($function = "list") then
		...
		return "[" || string-join($all, " and ") || "]"

	else if ($function = "id") then
		let $all :=
			for $k in $keys
			return "contains(descendant::" || $context || " , '" || $k || "')"
...
declare %private function q:par-clavis($clavisID, $clavisType) {
	if (
		(empty($clavisID) or $clavisID = "" or not(matches($clavisID, "\d+"))) and
			(empty($clavisType) or $clavisType = "" or not(matches($clavisType, "\w+")))
	) then (
	) else if (
		(empty($clavisID) or $clavisID = "" or not(matches($clavisID, "\d+"))) and (matches($clavisType, "\w+"))
	) then
		"[descendant::t:bibl[@type eq '" || $clavisType || "']]"
	else
...
declare %private function q:par-date-range($element, $dateRange) {
	...
	return if (($fromYear = "0001" and $toYear = "2000") or empty($dateRange) or $dateRange = "") then (
	) else
		"[
...
declare function q:parameters2arguments($params) {
	...
	let $args :=
		...
		return if (ends-with($p, "-field")) then (
		) else if (ends-with($p, "-facet")) then
			let $facetkey := substring-before($p, "-facet")
			return if ($facetkey = "keywords") then
				q:par-keywords($r)
			else (
			)
		else if ((count($r) = 0) or ($r = "") or (empty($r)) or (string-length(string-join($r)) = 0)) then (
			(: util:log('info', concat(' not passing ', string-join($r))) :) ) else
...
names are those of the indexes where the filter is built directly from there, otherwise assigned in the form constructors.
 :)
				...
				case "CAeID" return
					q:par-CAeID($r)
				case "ClavisIDs" return
					q:par-ClavisIDs($r, "clavistype")
				case "clavistype" return
...
declare function q:text($q, $params) {
	...
	let $ftquery := if (exists($qs)) then
		if ($mode eq "phrase") then
			"[ft:query(., '""" ||
				translate($q, '"', "") ||
				"""', <options><default-operator>and</default-operator></options>)]"
		else
			"[ft:query(., $qs, $q:allopts)]"
	else (
	)
...
{  ?subject rdfs:label '" ||
  ...
  FILTER (lang(?translit) = 'gez') }
	...
	let $query := try { fusekisparql:query("traces", $sparql) } catch * { console:log($err:description) }
	return try { $query//sr:binding/sr:literal/text() } catch * { util:log("info", $err:description) }
};
...
declare function q:gettranslit($sequenceoftokens) {
   ...
   which will be in none of the official formats, because this is made meaningless by the homophons substitutions :)
	return if (count($traces) ge 1) then
		$traces
	else
...
declare function q:translitquery($query-string) {
	...
	(: traces contains only singe token annotations so the query must be repeated for each word. It may return for one part only of the query :)(: how to join this, which will then be passed to the substitutions and then to the query builder, depends on the mode and the operator
...
we need alternate phrases, so the results of the query for each term need to be joined among them before being grouped
 :)
	let $modejoin := if (request:get-parameter("mode", ()) = "phrase") then
		(: 'source1 source2' OR 'translit1 translit2' :)
		<query>
			<phrase>{ string-join($trytraces, " ") }</phrase>
			<phrase>{ string-join($tokenizedquery, " ") }</phrase>
		</query>

	else if ($q:defop = "AND") then
...
(source1 OR translit1) (source2 OR translit2)    :)
		let $toks :=
			for $tok at $p in $tokenizedquery
			return <bool>
				{
					attribute occur {
						if ($q:defop = "AND") then
...
declare function q:prepareformorphoparser($q) {
	...
	return (: if the string was in transliteration, I already have what I want :) if (
		not(matches($q, "\p{IsEthiopic}"))
	) then
		$cleanup
	else
...
declare function q:querystring($query-string, $mode as xs:string*) {
	...
	let $fields :=
		for $parm in $q:params[ends-with(., "-field")][not(contains(., "-operator-"))]
		return request:get-parameter($parm, ())
	(: let $t := util:log('info', count($fields[.!=''])) :)
	return if (count($fields[. != ""]) != 0) then
		...
		let $translit-query-string := if ($translitparam = "on") then
			...
			$query-string
		(: translit will return a query already. but if that did not go through the input may be a string :)(: here a temporary query is stored, which is either a parsing of a string or an improvement of the existing one it calls the creat-query function :)
		let $modequery := <querytemp>
			{
				if ($mode = "none") then (
					if (
						...
						$translit-query-string
				) else if ($mode = "regex" or $mode = "wildcard" or $mode = "fuzzy") then
...
declare function q:querycleanup($xmlquery) {
	...
	return typeswitch ($node)
		...
		case element(term) return
			if (
				(count($node/descendant::bool) ge 1) or
					($node/ancestor::fuzzy) or
					($node/ancestor::wildcard) or
					($node/ancestor::regex)
			) then
				q:querycleanup($node)
			else
		...
		case element(bool) return
			if ($node/@occur = "must") then
				element {$node/name()} { ($node/@*, q:querycleanup($node)) }
			else if ($node/bool[@occur][count(term[@occur]) eq 1]) then
				for $term in $node/bool[@occur][count(term[@occur]) eq 1]
				group by $occur := $term/@occur
				return <bool>{ $occur }{ q:querycleanup($term) }</bool>
			else if ($node/ancestor::near or $node/ancestor::fuzzy) then
				q:querycleanup($node)
			else
				element {$node/name()} { ($node/@*, q:querycleanup($node)) }
		case element() return
...
declare function q:groupsubst($query, $homophones) {
	...
	(: complex query :)
		...
		return q:loopqueryxml($lucene2xml/query, $homophones)
	else
		(: only text :)
...
declare function q:create-field-query($query-string, $mode) {
	let $fields :=
		for $field in $q:params[ends-with(., "-field")][not(contains(., "-operator"))]
		let $fieldpar := request:get-parameter($field, ())
		return if ($fieldpar = "") then (
		) else
			let $operator := replace($field, "-field", "-operator-field")
			return request:get-parameter($operator, ()) ||
				" " ||
				substring-before($field, "-field") ||
				":(" ||
				$fieldpar ||
				")"
...
declare function q:showFacets($node as node()*, $model as map(*)) {
	if (
		$q:searchType = "clavis" or
			$q:searchType = "otherclavis" or
			$q:searchType = "xpath" or
			$q:searchType = "sparql" or
			$q:searchType = "linkeddata"
	) then
		...
		return <form action="" class="w3-container w3-center" id="facetsSearch">
			<div class="w3-row w3-left-align">
				...
				</button>
				{
					...
					return if ((count($p) ge 1) and ($p != "") and (string-join($p, ",") != "0,2000")) then
						for $par in $p
						return <input hidden="hidden" name="{ $notfacet }" value="{ $par }" />
					else (
					)
				}
			</div>
			...
			<div class="w3-padding w3-hide" id="{ string($f) }-facet-list">
				{
					...
					return if ($f = "keywords") then (
						...
						group by $taxonomy
						order by $taxonomy
						return <div class="w3-row w3-left-align">
							<button
								class="w3-button w3-block w3-left-align w3-gray"
						...
						return <div class="w3-row w3-left-align">
							...
							<div class="w3-padding w3-hide" id="{ string($f) }-{ $first }-facet-sublist">
								{
									for $i in $input
									let $sortkey := q:sortingkey($i)
									order by $sortkey
...
declare function q:facetName($f) {
	switch ($f)
		...
		case "languages" return
			"Languages"
		case "changeWho" return
			"Author of changes"
		case "changeWhen" return
			"Date of changes"
		case "script" return
			"Script"
		...
		case "donor" return
			"Donor"
		case "msItemsCount" return
			"N. of content units"
		case "msPartsCount" return
			"N. of Codicological Units"
		case "handsCount" return
			"N. of Hands"
		case "sealCount" return
			"N. of Seals"
		case "QuireCount" return
			"N. of Quires"
		case "AdditionsCount" return
			"N. of Additions"
		case "AdditionsType" return
			"Types of Addition"
		case "titleRef" return
			"Contents"
		case "titleType" return
			"Complete/Incomplete contents"
		case "ExtraCount" return
			"N. of Extras"
		case "ExtraType" return
			"Types of Extra"
		case "leafs" return
			"N. of leaves"
		case "origDateNotBefore" return
			"Date of production (not before)"
		case "origDateNotAfter" return
		...
		case "collection" return
			"Collection"
		case "rulingpattern" return
			"Ruling Pattern"
		case "artThemes" return
			"Art Themes (in decorations)"
		case "artkeywords" return
			"Art Keywords (in decorations)"
		case "bindingkeywords" return
			"Keywords (in binding)"
		case "rubricationkeywords" return
			"Keywords (in rubrication)"
		case "decoType" return
			"Type of Decoration"
		case "calendarType" return
			"Type of calendar used"
		case "images" return
			"Images Availability"
		case "writtenLines" return
			"Written Lines"
		case "columns" return
			"Columns"
		case "authors" return
			"Authors"
		case "textDivs" return
			"Text parts"
		case "sawsVersionOf" return
			"Versions"
		case "sex" return
			"Gender"
		case "name" return
		...
		case "witness" return
			"Witnesses"
		case "thereistext" return
			"Text"
		case "thereistranscription" return
			"Transcription"
		case "reltype" return
			"Relation Names"
		case "bindingMaterial" return
			"Binding Material"
		case "tabot" return
			"Tābots"
		case "occupation" return
			"Occupation Type"
		case "persDateNotBefore" return
			"Date Not Before"
		case "persDateNotAfter" return
			"Date Not After"
		case "persDateWhen" return
			"Date point"
		case "eth" return
			"Ethnic group"
		case "presenceOfPunctuation" return
...
declare function q:create-query($query-string as xs:string?, $mode as xs:string) {
	...
	let $query := (: this filters queries to fields so that they are not passed as xml fragment :) if (
		$mode = "none" or contains($query-string, ":")
	) then
		$query-string
...
declare %private function q:lucene2xml($node as item(), $mode as xs:string) {
	typeswitch ($node)
		case element(query) return
			element {node-name($node)} { element bool { $node/node()/q:lucene2xml(., $mode) } }
			...
			if ($node/parent::*[self::query or self::bool]) then
				...
				return element {$el-name} {
					...
					if (matches($tok, "((^|[^\\])[.?*+()\[\]\\^|{}#@&amp;<>~]|\$$)")) then
						...
						attribute boost { lower-case(replace($tok, "(.*?)(\^(\d+))(\W|$)", "$3")) }
					else (
					),
					(: regex searches have to be lower-cased :)
...
declare %templates:wrap function q:charts($node as node()*, $model as map(*)) {
	if ($q:searchType = "clavis" or $q:searchType = "linkeddata") then (
	) else if (count($model("hits")) = 0) then (
	) else
		...
		return if (count($mss) = 0) then (
			...
			<div class="w3-row">
				{ "There are " }
				<span class="w3-tag w3-gray">{ count($mss) }</span>
				{ " manuscripts among the results of this search." }
			</div>,
...
declare %templates:wrap function q:compare($node as node()*, $model as map(*)) {
	if ($q:searchType = "clavis" or $q:searchType = "linkeddata") then (
	) else
		...
		return if (count($matchingmss) = 0) then (
			...
			<p class="w3-panel w3-card-2">They are <span class="w3-tag w3-gray">
					{ count($matchingmss) }
										...
										>
											{ string($msitem/@xml:id) }
											{
												...
												if ($msitem/t:title[not(@ref)]/text()) then (
													...
													<span>
														<a class="itemtitle" data-value="{ $title }" href="{ $config:appUrl }/{ $title }">
															{
																if ($title = "") then
																	<span class="w3-tag w3-pale-red">{ "no ref in title" }</span>
																else
										...
										return <li>
											{ string($additem/@xml:id) }
											{
												...
												return if ($additem/t:title[not(@ref)]/text()) then (
													...
													<span>
														<a class="itemtitle" data-value="{ $t }" href="{ $config:appUrl }/{ $t }">
															{
																if ($t = "") then
																	<span class="w3-tag w3-pale-red">{ "no ref in title" }</span>
																else
...
declare %templates:wrap function q:geobrowser($node as node()*, $model as map(*)) {
	if ($q:searchType = "clavis" or $q:searchType = "linkeddata") then (
	) else
		let $worksid := $model("hits")[@type = "work"]
		return if (count($worksid) = 0) then
			<div
				class="w3-row"
			>
			...
			let $kmlparam :=
				for $work at $p in $worksid/@xml:id
				return "kml" || $p || "=https://betamasaheft.eu/workmap/KML/" || string($work) || "?type=repo"
			let $worktitles :=
				for $work in $worksid/@xml:id
				return <a href="{ $config:appUrl }/{ string($work) }" target="_blank">{ exptit:printTitleID($work) }, </a>
			return (
				<div class="w3-container">
					<div class="w3-container alert alert-info">You can download the KML file visualized below in the <a
					...
					<div
						class="w3-panel w3-card-2 w3-red"
					>You do not find all the information you would like to
                            have? <a
							href="https://betamasaheft.eu/Guidelines/?id=howto"
						>Help improve the data and contribute to the project editing the files!</a>
					</div>
				</div>
			)
};

declare %templates:wrap function q:voyant($node as node()*, $model as map(*)) {
	let $texts := $model("hits")[@type = "work"]
	let $ids :=
		for $hit in $texts
		return "input=https://betamasaheft.eu/works/" || string($hit/@xml:id) || ".xml"
	let $urls := string-join($ids, "&amp;")
	return <a href="{ concat("http://voyant-tools.org/?", $urls) }" target="_blank">
		<span class="w3-tooltip">Voyant
                <span
				class="w3-text"
			> limited to textual units and manuscripts </span>
		</span>
	</a>
};

declare %templates:wrap %templates:default("start", 1) %templates:default("per-page", 40) function q:results(
	$node as node()*,
	$model as map(*),
	$start as xs:integer,
...
first here is the header of the results table :)
	...
	return if (($model("type") = "text") and ($model("qs") = "")) then
		q:resultswithoutmatch($hit, $p)
	else if ($model("type") = "text") then (
		...
		$model("type") = "xpath" or
			$model("type") = "bmid" or
			$model("type") = "linkeddata" or
			$model("type") = "clavis" or
			$model("type") = "placeSearch" or
			$model("type") = "otherclavis"
	) then
		q:resultswithoutmatch($hit, $p)
	else (
	)
...
declare function q:resultsTableHeader($model) {
	...
	) (: the sparql results are passed to an XSLT which produces the header as well :) else if (
		$model("type") = "clavis" or
			$model("type") = "linkeddata" or
			$model("type") = "xpath" or
			$model("type") = "otherclavis"
...
declare %templates:wrap function q:sparqlRes($hit, $p) {
...
declare function q:enrichScore($text) {
	...
	let $elementtypematch := if (
		(
			(($countelnames gt $countelplace) and ($countelnames gt $counttext) and ($countelnames gt $countelmsItem)) and
				string($text/ancestor-or-self::t:TEI/@type) = "pers"
		) or
			(
				(($countelmsItem gt $countelplace) and ($countelmsItem gt $counttext) and ($countelmsItem gt $countelnames)) and
					string($text/ancestor-or-self::t:TEI/@type) = "mss"
			)
	) then
		2
	else
	...
	let $scores :=
		...
		return if ($match/parent::t:idno) then
			$score * 5
		else if ($match/parent::t:title[parent::t:titleStmt]) then
			$score * 5
		else if ($match/parent::t:*[self::t:persName or self::t:placeName]) then
			$score * 4
		else
			$score
	...
	return format-number($enrichedScore, "0000")
};
...
declare function q:resultswithmatch($text, $p) {
	...
	return <div class="w3-row w3-border-bottom w3-margin-bottom">
		<div class="w3-third">
			<div class="w3-row">
				<div class="w3-col" style="width:70%">{ q:resultitemlinks($collection, $item, $id, $root, $text) }</div>
				<div class="w3-col" style="width:30%">
			...
			<div class="w3-row">{ q:summary($text) }</div>
		</div>
		<div class="w3-rest">
			<div class="w3-rest">
				{
					...
					return let $matchref := replace(q:refname($match), ".$", "")
						...
						return <div class="w3-row w3-padding">
							...
							<div class="w3-third w3-padding" style="word-break: break-all;">
								<a
		...
		<div class="w3-row">
				...
				<div class="w3-col" style="width:10%">
					{
						if ($q:searchType = "clavis" and $text("type") = "deleted") then
							<span class="w3-tag w3-black">DELETED</span>
						else (
						)
					}
				</div>
			...
			<div class="w3-twothird">
				<div class="w3-container">
					{
						if ($q:searchType = "clavis" and $text("type") = "deleted") then
							<a href="{ $config:appUrl }/deleted.html">to see when {
									$text("hit")/text()
								} was deleted and why, click here for the list of deleted files</a>
						else (
						)
					(: q:resultslinkstoviews($t, $id, $collection) :)
					}
...
declare function q:summary($item) {
	let $id := $item/@xml:id
	let $dates := (
		$item//t:date[not(parent::t:publicationStmt)][not(parent::t:bibl)],
		$item//t:origDate,
		$item//t:birth,
		$item//t:floruit,
		$item//t:death
	)
	return <div class="w3-container" style="max-height:200px;resize: both;overflow:auto">
		{
			switch ($item/@type)
				case "work" return
...
declare function q:summaryPers($item, $id) {
	<div class="w3-container">
		...
		}
		{
			if ($item//t:person/@sameAs) then
				<a href="{ $config:appUrl }/{ $item//t:person/@sameAs }"><span class="icon-large icon-vcard" /></a>
			else (
			)
		}
	</div>,
		...
		<ul class="nodot">
			{
				...
				return <li>
					{ $n//text() }
					<sup>{ string($n/@xml:lang) }</sup>
					{
						if ($item//t:persName[@corresp]) then (
							let $corrsNs :=
								for $corrN in $item//t:persName[@corresp]
								let $corrNcorr := substring-after($corrN/@corresp, "#")
								return if ($corrNcorr = $Nid) then (
	...
	if ($item//t:floruit/@* or $item//t:birth/@* or $item//t:death/@*) then
		<div class="w3-container">
			<h5>Dates</h5>
			<ul class="nodot">
				{
	...
	<div class="w3-container">
		{
			if ($item//t:place/@sameAs) then
				...
				return <a href="{ ("https://www.wikidata.org/wiki/" || $wd) }" target="_blank">{ $wd }</a>
			else (
			)
		}
		{
			if ($item//t:geo) then
				<a href="{ $config:appUrl }/{ ($id) }.json" target="_blank"><span class="glyphicon glyphicon-map-marker" /></a>
			else (
			)
		}
	</div>
...
declare function q:summaryIns($item, $id) {
	...
	return if (count($mss) ge 1) then
		<div class="w3-container">
                There are { count($mss) }
			<a
				href="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;reporef={ $id }"
			> manuscripts at this repository</a>.
                <ul class="w3-ul">
				{
					for $m in $mss
					return <li>
		...
		if ($item//t:titleStmt/t:author or $creator or $attributed) then
			<div class="w3-container">
				...
				<ul class="nodot">
					...
					{
						let $attributions :=
							for $r in ($creator, $attributed)
							let $rpass := $r/@passive
							return if (contains($rpass, " ")) then
								tokenize($rpass, " ")
							else
								$rpass
						...
						return <li>
							<a href="{ $config:appUrl }/{ $author }">
								{ try { exptit:printTitleID($id) } catch * { $author//t:titleStmt/t:title[1]/text() } }
							</a>
						</li>
					}
				</ul>
				...
				<ul class="nodot">
					{
						...
						return <li><a href="{ $config:appUrl }/{ $corr }">{ exptit:printTitleID($id) }</a></li>
					}
				</ul>
				<ul class="nodot">
					{
						...
						return <li><a href="{ $config:appUrl }/{ $p }">{ $id }</a></li>
					}
				</ul>
			</div>
		else (
...
declare function q:summaryMss($item, $id) {
	<div class="w3-container">
		...
		{
			let $idnos :=
				for $shelfmark in $item//t:msIdentifier//t:idno
				return $shelfmark/text()
			return string-join($idnos, ", ")
		}
	</div>,
	<div class="w3-container">
        ...
        is composed of {
			$item//t:extent/t:measure[@unit = "leaf"][not(@type = "blank")]
		} leaves.
        It has { count($item//t:msItem[not(t:msItem)]) } main content units in {
        ...
        {
			if ($item//t:origDate or $item//t:date[@evidence eq "internal-date"]) then
				"Available dates of origin in the description: " ||
					(
						...
						let $formatdates :=
							for $d in $alldates
							return try { viewItem:dates($d) } catch * { util:log("info", $err:description) }
			...
			if ($item//t:handNote) then
				...
				return if ($hands gt 1) then
					...
					"There is " ||
						...
						" described with " ||
						string-join(config:distinct-values(data($item//@script)), ", ") ||
						" script attested. "
			else (
			)
		}
...
declare function q:resultslinkstoviews($t, $id, $collection) {
	<div class="w3-third">
		...
		<a
			...
			role="button"
		>relations</a>
		<div class="w3-container" style="max-height:200px;resize: both;overflow:auto">
			{
				let $item := $q:col/id($id)[self::t:TEI]
				let $log := if (count($item) gt 1) then
					for $i in $item
					return util:log("INFO", base-uri($i))
				else (
				)
				return switch ($t)
					case "pers" return
						for $i in $item
						return item2:RestPersRole($i, $collection)
					case "work" return
...
declare function q:formatOption($rangeindexname, $key, $count) {
	...
	return if ($rangeindexname = "ident") then
		<option value="{ $key }">{ $q:languages//id($key)/text() } ({ $count[2] })</option>
	else if ($rangeindexname = "changewho") then
		<option value="{ $key }">{ editors:editorKey(substring-after($key, "#")) } ({ $count[2] })</option>

	else if (starts-with($key, $config:BMurl)) then
		let $id := replace($key, $config:BMurl, "")
		let $title := ($q:lists//t:item[@xml:id = $id] | $q:lists//t:item[@corresp = $id])/text()
		let $titlesel := if ($title) then
			$title
		else
			try { exptit:printTitleID($id) } catch * { util:log("INFO", $err:description) }
		return <option value="{ $key }">{ normalize-space($titlesel) } ({ $count[2] })</option>
	else if (starts-with($key, "#")) then
		let $id := replace($key, "#", "")
		let $t := $q:lists//t:item[@xml:id = $id]
		let $title := string-join($t/text())
		return <option value="{ $key }">{ normalize-space($title) } ({ $count[2] })</option>
	else if ($rangeindexname = $keywordsindexes) then
		let $k := string($key)
		let $cat := $q:tax/id($k)[self::t:category]
		return <option value="{ $key }">
			{ $cat/t:catDesc/text() }
                                    ({ $count[2] })</option>
	else if ($rangeindexname = "sex") then
		<option value="{ $key }">
			{
				switch ($key)
					case "1" return
...
declare function q:MssPersRoles($node as node(), $model as map(*)) {
	...
	return <div class="w3-container">
		<label for="{ $role }">{ $label }<span class="w3-badge">{ count($keywords) }</span></label>
		<input class="w3-input w3-border" id="{ $role }" list="{ $role }-list" name="{ $role }" />
		<datalist id="{ $role }-list" style="width:100%">{ q:selectors($role, $keywords, "rels") }</datalist>
	</div>
};
...
declare function q:WorkAuthors($node as node(), $model as map(*)) {
	let $attributions :=
		for $rel in
			($q:col//t:relation[@name eq "saws:isAttributedToAuthor"], $q:col//t:relation[@name eq "dcterms:creator"])
		let $r := $rel/@passive
	...
	return q:formcontrol("author", $keywords, "false", "rels")
};
...
declare function q:PersonsRangeIndexesFilters($node as node(), $model as map(*)) {
	...
	(: ('TEIpersOcc', 'occupationtext', 'faithtext', 'faithtype', 'persrole') :) return q:datalist($indexnames)
};
...
declare function q:PlacesRangeIndexesFilters($node as node(), $model as map(*)) {
	...
	(: ('TEIplName', 'placetype') :) return q:datalist($indexnames)
};
...
declare function q:tabot($node as node(), $model as map(*)) {
	...
	return q:formcontrol("tabot", $alltabots, "false", "rels")
};
...
declare function q:datalist($indexnames) {
	...
	return <div class="w3-container">
		<label for="{ $nodeName }">{ $label }<span class="w3-badge">{ count($lookup) }</span></label>
		<input class="w3-input w3-border" id="{ $nodeName }" list="{ $nodeName }-list" name="{ $nodeName }" />
		<datalist id="{ $nodeName }-list" style="width:100%">
			{
...
declare function q:formcontrol($nodeName as xs:string, $path, $group, $type) {
	if ($group = "true") then (
		...
		return <div class="w3-container">
			<label for="{ $nodeName }">
				{
					if ($nodeName = "tabot") then
						"Tābots"
					else (
						$nodeName || "s"
					)
				}
				<span class="w3-badge">{ count($nodes[. != ""][. != " "]) }</span>
			</label>
		...
		let $nodes :=
			...
			return $node
		return <div class="w3-container">
			<label for="{ $nodeName }">{ $nodeName }s <span class="w3-badge">{ count($path) }</span></label>
			{ q:selectors($nodeName, $nodes, $type) }
		</div>
	)
};
	...
	<select class="w3-select w3-border" id="{ $nodeName }" multiple="multiple" name="{ $nodeName }">
		{
			if ($type = "keywords") then (
				...
				return for $n in $group//t:catDesc
					...
					return <option value="{ $id }">{ $title[1] }</option>
				...
				order by $sortkey
				return <option value="{ $n }">{ $title }</option>
...
(\:                                        this has to stay because optgroup requires label and this cannot be computed from the javascript as in other places:\)
				...
				return if (count($n) = 1) then
					<option value="{ $work }">{ $titlework }</option>
				else (
