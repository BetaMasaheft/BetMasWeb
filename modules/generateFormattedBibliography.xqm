...
declare function gfb:shortCit($c) {
	...
	let $replaced := replace($parseedZoteroApiResponse?*?citation, "&lt;", "<") => replace("&gt;", ">")
	return $replaced
};
...
declare function gfb:updateBib($context) {
	...
	return if (count($match) ge 1) then ( (: update the entry :)
		...
		let $update := if ($entry/b:citation[not(text())] or $entry/b:reference[not(text())]) then
			util:log("WARN", $entry)
		else
			update insert $entry into $bibliography/b:entries
		let $updatetotal := update value $bibliography/b:total with count($cleancitations)
		(: add the entry :) return util:log("INFO", ("added entry for " || $rawc))
};
...
declare function gfb:makeBib($collection) {
	...
	let $cleancitations :=
		for $c in $citations
		return replace($c, "\s+", "")
	let $bibliography := <bibliography xmlns="betmas.biblio">
		<total>{ count($citations) }</total>
		<entries>
			{
				for $rawc in $cleancitations
				...
				group by $shortCit
				order by $shortCit
				return if ($shortCit = "" or $shortCit = " ") then
					for $sameCit in $rawc
					let $cit := "EMPTY! WRONG POINTER!"
					return gfb:entry($cit, $sameCit)
				else if (count($c) gt 1) then
					...
					for $sameCit in $rawc
					return gfb:entry($shortCit, $sameCit)
			}
		</entries>
	</bibliography>
