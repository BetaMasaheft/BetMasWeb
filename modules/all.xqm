...
declare function all:repl($query, $match, $sub) {
	...
	return if ($x = $match) then
		let $index := index-of($seq, $x)
		return (: loop each occurrence of that character to do the substitutions one by one in case it matches, 0 and 3 for the example :) for $i in
				$index

			return (: substitute only that occurence by removing it and adding the substitute in its place, so in the first loop, remove a and then add d before position 0 :) let $rem :=
				remove($seq, $i)
				...
				return ($newstring, all:repl($newstring, $match, $sub))
...
declare function all:subs($query, $homophones, $mode) {
	let $all :=
		for $b in $homophones
		return for $q in $query
			return if (contains($q, $b)) then
				let $options :=
					for $s in $homophones[. != $b]
					return (
						let $values := all:repl($q, $b, $s)
						return all:distinct-values($values),
						if ($mode = "ws") then (
							replace($q, $b, "")
						) else (
						)
...
function all:substitutionsInQuery($query as xs:string*) {
	...
	return (: this statements limits the input string on which to do the substitutions to 15 characters :) (: if($stringlength gt 15) then ($query-string) else :) let $emphaticS :=
		("s", "s", "ḍ")
		...
		return all:limitoptions($query-string)
};
...
declare function all:limitoptions($options) {
	...
	let $reductionFactor := if ($stringLength gt 10000) then
		10
	else if ($stringLength gt 5000) then
		5
	else
		1
	return (: we have set 4000 to be the maximum length of the query, if the query is longer, reduce the options and try again if not, the candidate query can be passed on :) if (
		$stringLength gt 4000
	) then
