...
declare function r:roman-numeral-to-integer($input as xs:string) as xs:integer {
	...
	let $character-to-integer := function ($character as xs:string) {
		switch ($character)
			case "I" return
				1
			case "V" return
			...
			default return
				error(
					xs:QName("roman-numeral-error"),
					concat(
						"Invalid input: ",
						$input,
						". Valid Roman numeral characters are I, V, X, L, C, D, and M. This function is case insensitive."
					)
				)
	}
	...
	let $values :=
		...
		return if ($number < $numbers[position() = $n + 1]) then (
			0 - $number
		) (: Handles subtractive principle of Roman numerals. :) else
			$number
	return sum($values)
};
