...
pointers in expanded. Both backend resolvers run independently even when the
backend names match. The runner fails only when `unreviewedCount` is nonzero.
...
unreviewed and therefore fails the gate. A future catalog backend adds its
...
`CATALOG_BACKEND_B`; the unreviewed-count assertion is the BetMasWeb PR gate.
