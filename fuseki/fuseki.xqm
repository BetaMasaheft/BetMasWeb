...
declare variable $fusekisparql:port := config:service-url("FUSEKI_URL", "http://localhost:3030/");
...
declare function fusekisparql:query($dataset, $query) {
	let $format := if (contains($query, "CONSTRUCT")) then
		"format=xml&amp;"
	else (
	)
	...
	let $sparqlupdate := $config:sparqlPrefixes || $InsertOrDelete || " DATA
{
  " || $triples || "
	...
	let $req := <http:request href="{ xs:anyURI($url) }" http-version="1.1" method="POST">
