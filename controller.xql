...
if ($exist:path eq "") then
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist"><redirect url="{ local:get-uri() }/" /></dispatch>
...
(: Requests for javascript libraries are resolved to the file system :)
	...
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<forward url="{ $exist:controller }/vocabularies/{ substring-after($exist:path, "vocabularies/") }" />
	</dispatch>

(: Requests for javascript libraries are resolved to the file system :)
else if (contains($exist:path, "build/mirador")) then
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<forward url="{ $exist:controller }/resources/mirador/{ substring-after($exist:path, "mirador/") }" />
	</dispatch>
(: the xql files :)
...
(: another backward compatibility redirect :)
	...
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<redirect
			absolute="yes"
			url="{ $config:appUrl }/newSearch.html?searchType=placeSearch&amp;mode=any&amp;query={
				request:get-parameter("place", ())
			}" />
	</dispatch>
else if ($exist:path = "/manuscripts/list") then
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<redirect absolute="yes" url="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=mss" />
	</dispatch>
else if ($exist:path = "/persons/list") then
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<redirect absolute="yes" url="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=pers" />
	</dispatch>
else if ($exist:path = "/institutions/list") then
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<redirect absolute="yes" url="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=ins" />
	</dispatch>
else if ($exist:path = "/ethnic/list") then
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<redirect absolute="yes" url="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=eth" />
	</dispatch>
	...
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<redirect absolute="yes" url="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=nar" />
	</dispatch>
else if ($exist:path = "/works/list") then
	<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
		<redirect absolute="yes" url="{ $config:appUrl }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=work" />
	</dispatch>
...
(: ALSO INTERNAL CALLS of XSLT ARE THUS PROCESSED! :)
	...
	if ($exist:path eq "/") then
		<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
			...
			<view>
				<forward url="{ $exist:controller }/modules/view.xql">
					<add-parameter name="uri" value="{ ("/db/apps/BetMasWeb/index.html") }" />
				</forward>
			</view>
			<error-handler>
				<forward method="get" url="{ $exist:controller }/error/error-page.html" />
...
function apisparql:constructURIsubid() is called to construct a graph of that resource.
 :)
		...
		return if (contains(request:get-header("Accept"), "rdf")) then
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
				<forward absolute="yes" url="{ concat("/restxq/BetMasWeb", $exist:path, "/rdf") }">
					{ login:set-user($domain, (), false()) }
					<set-header name="Cache-Control" value="no-cache" />
				</forward>
			</dispatch>
			...
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
				<redirect url="{ $config:appUrl }/{ $switchCollection }/{ $tokenizePath[2] }/main#{ $tokenizePath[last()] }" />
			</dispatch>
...
construct the annotation graph if application/rdf+xml is specified
 :)
	...
	(: https://betamasaheft.eu/bond/snap:GrandfatherOf-PRS1854Amdase :)
		...
		return if (contains(request:get-header("Accept"), "rdf")) then
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
				<forward absolute="yes" url="{ concat("/restxq/BetMasWeb/bond/", encode-for-uri($tokenizePath[3]), "/rdf") }">
					{ login:set-user($domain, (), false()) }
					<set-header name="Cache-Control" value="no-cache" />
				</forward>
			</dispatch>
...
manuscript followed by a passage reference, then go to the text view of that
 ...
 : these will actually go to the api
  :)
		...
		return <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
			<redirect url="{ $config:appUrl }/{ $switchCollection }/{ $id }/text{ $parms }" />
		</dispatch>
...
https://betamasaheft.eu/manuscript/BNFet32/main
 :) if (contains(request:get-header("Accept"), "rdf")) then
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
				<forward absolute="yes" url="{ concat("/restxq/BetMasWeb/", $exist:resource, "/rdf") }">
					{ login:set-user($domain, (), false()) }
					<set-header name="Cache-Control" value="no-cache" />
				</forward>
			</dispatch>
			...
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
				<redirect url="{ $config:appUrl }/{ $switchCollection }/{ $exist:resource }/main" />
			</dispatch>
 ...
 :)
		...
		if (contains($exist:path, "/authority-files/")) then
			if (contains(request:get-header("Accept"), "rdf")) then
				...
				<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
					<redirect url="{ $config:appUrl }/authority-files/{ $exist:resource }/main" />
				</dispatch>
		else (: if it is a special page then go there :) if (starts-with($exist:path, "/decorations")) then
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
			...
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist"><redirect url="{ $exist:path }list" /></dispatch>
		else if (
			$exist:path eq "/manuscripts" or
				$exist:path eq "/works" or
				$exist:path eq "/narratives" or
			...
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist"><redirect url="{ $exist:path }/list" /></dispatch>
                                                                                                                                                                                                   ...
                                                                                                                                                                                                   let $c2 := console:log($taxonomy)
                                                                                                                                                                                                    return
                                        :) (: Inline the canonical loading, it takes a lot of time :) if (
			doc("/db/apps/lists/canonicaltaxonomy.xml")//t:category/@xml:id = $exist:resource
		) then
			<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
				<redirect url="{ $config:appUrl }/authority-files/{ $exist:resource }/main" />
			</dispatch>
		...
		<dispatch xmlns="http://exist.sourceforge.net/NS/exist">
			<error-handler>
				<redirect method="get" url="{ $exist:controller }/error/error-page.html" />
				<forward url="{ $exist:controller }/modules/view.xql" />
			</error-handler>
		</dispatch>
