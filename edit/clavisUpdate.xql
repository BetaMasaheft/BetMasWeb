...
if (contains(sm:get-user-groups(sm:id()//sm:real/sm:username/string()), "Editors")) then
	...
	let $cc := if ($cmlc = "null" or $cmlc = "") then (
	) else
		<bibl xmlns="http://www.tei-c.org/ns/1.0" type="CC">
			<ptr target="bm:CC" />
			<citedRange unit="item">{ substring-after($cmlc, "cc") }</citedRange>
		</bibl>
	let $biblcpg := if ($cpg = "null" or $cpg = "") then (
	) else
		<bibl xmlns="http://www.tei-c.org/ns/1.0" type="CPG">
			<ptr target="bm:CPG1" />
			<citedRange unit="item">{ $cpg }</citedRange>
		</bibl>
	let $biblbho := if ($bho = "null" or $bho = "") then (
	) else
		<bibl xmlns="http://www.tei-c.org/ns/1.0" type="BHO">
			<ptr target="bm:BHO" />
			<citedRange unit="item">{ $bho }</citedRange>
		</bibl>
	let $biblbhl := if ($bhl = "null" or $bhl = "") then (
	) else
		<bibl xmlns="http://www.tei-c.org/ns/1.0" type="BHL">
			<ptr target="bm:BHL" />
			<citedRange unit="item">{ $bhl }</citedRange>
		</bibl>
	let $biblbhg := if ($bhg = "null" or $bhg = "") then (
	) else
		<bibl xmlns="http://www.tei-c.org/ns/1.0" type="BHG">
			<ptr target="bm:BHG" />
			<citedRange unit="item">{ $bhg }</citedRange>
		</bibl>
	let $biblcant := if ($cant = "null" or $cant = "") then (
	) else
		<bibl xmlns="http://www.tei-c.org/ns/1.0" type="CANT">
			<ptr target="bm:CANT1" />
			<citedRange unit="item">{ $cant }</citedRange>
		</bibl>
	let $biblcavt := if ($cavt = "null" or $cavt = "") then (
	) else
		<bibl xmlns="http://www.tei-c.org/ns/1.0" type="CAVT">
			<ptr target="bm:CAVT2" />
			<citedRange unit="item">{ $cavt }</citedRange>
		</bibl>

	let $allbibl := ($cc, $biblcpg, $biblbho, $biblbhl, $biblbhg, $biblcant, $biblcavt)
	let $clavisListBibl := <p xmlns="http://www.tei-c.org/ns/1.0"><listBibl type="clavis">{ $allbibl }</listBibl></p>
	...
	let $updateClavis := if (exists($clavis)) then (
		...
		update insert $clavisListBibl into $sourceDesc
	let $updatechanges := update insert <change
		xmlns="http://www.tei-c.org/ns/1.0"
		when="{ format-date(current-date(), "[Y0001]-[M01]-[D01]") }"
		who="{ $editor }"
	>Added ids matched from the PATHs project via API.</change> into $changes
		...
		<body>
			<div class="container" id="confirmation">
				<div class="jumbotron">
					...
					<p><span class="lead">{ $WorkID }</span> has been updated!</p>
					<p>But <span class="label label-warning confirmationwarning">WAIT!</span> you are <span
						...
						>not yet done</span>...</p>
					<p>Download the file in your BetMas project's <span class="lead">WORKS</span> folder under <span
							class="lead"
			...
			<div class="container" id="confirmation">
				<div class="jumbotron">
					<p class="lead">Sorry {
						...
						}! You have no sufficient rights to use this module.</p>
				</div>
