...
if (sm:is-authenticated() and sm:is-dba(sm:id()//sm:real/sm:username/string())) then
	try {
		...
		let $secondaryGroup := if (empty($secondarygroup)) then (
		) else
			sm:add-group-member($secondarygroup, $user)
			...
			(
				let $contributorMessage := <mail>
					...
					<message>
						<xhtml>
							<html>
								<head><title>Your account data</title></head>
								<body>
									<p>Dear { $user }, Welcome to Beta Maṣāḥǝft!</p>
									<p>Your password has just been set to: <b>{ $pw }</b>.</p>
									<p>You can change your password by going to <a
											href="{ $config:appUrl || "/user/" || $user }"
										>your user page</a> where you also find all relevant information about your account.</p>
									{
										for $x in sm:get-account-metadata-keys($user)
										return <p>
											<b>
												{
													switch ($x)
														case "http://exist-db.org/security/description" return
															"Role: "
														case "http://axschema.org/namePerson" return
															"Full name: "
														case "http://axschema.org/contact/email" return
															"E-mail: "
														default return
															()
												}
											</b>
											{ sm:get-account-metadata($user, $x) }
										</p>
									}
						...
						<xhtml>
							<html>
								...
								<body>
									...
									{
										for $x in sm:get-account-metadata-keys($user)
										return <p><b>{ $x }</b>:{ sm:get-account-metadata($user, $x) }</p>
									}
								</body>
							</html>
						</xhtml>
			...
			(
				let $adminMessage := <mail>
					...
					<message>
						<xhtml>
							<html>
								<head><title>Failure to set new account</title></head>
								<body>
				...
				<html>
					...
					<body>
						<div id="confirmation">
							<p class="lead">Sorry, the system could not create this account.</p>
							<p>{ concat($err:code, ": ", $err:description) }</p>
						</div>
					</body>
				</html>
