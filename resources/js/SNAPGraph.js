...
?bondwithSubject rdf:type ?relwithSubject; \
                       snap:bond-with ?person2 . \
?person2 crm:P48_has_preferred_identifier '" +
		id +
		"' . \
?person1 snap:hasBond ?bondName . \
?bondName rdf:type ?relName; \
...
?partName bm:wifeOf ?resourceName . \
?resourceName crm:P48_has_preferred_identifier '" +
		id +
		"' . \
BIND(bm:wifeOf as ?relName) \
}UNION { \
...
BIND(bm:husbandOf as ?relName) \
	...
	$.getJSON(apicall, function (data) {
		...
		if (results == null) {
			$("#SNAPGraph").text("the sparql query returned no results");
		} else {
			var reslength = "";
			if (results.length) {
	...
	var rdf =
		"SELECT DISTINCT ?name ?attestation ?citation ?role ?roleType \
WHERE { \
