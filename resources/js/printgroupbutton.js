$(document).on("ready", function () {
	...
	if (url.includes("works/list")) {
		$("#optionsList").append(
			'<button class="w3-button w3-gray  mapgroup">Manuscripts Map</button><button class="w3-button w3-gray  titlesgroup">Titles</button>',
		);
		var checkBoxes = $("input.mapSelected");
		checkBoxes.change();
	} else {
		$("#optionsList").append(
			'<button class="w3-button w3-gray  comparegroup">Compare</button><button class="w3-button w3-gray  titlesgroup">Titles</button>',
		);
