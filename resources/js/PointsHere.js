$("span[id$='relations']").on("click", function () {
	...
	if ($(this).children("div").hasClass("popuptext")) {
		...
		$.getJSON(callsparql, function (data) {
            ...
            <a href="/Dillmann?mode=fuzzy&q=' +
						popText +
						'" target="_blank">in the Online Lexicon</a><br/>\
                <ul xmlns="http://www.w3.org/1999/xhtml" class="nodot"><head xmlns="http://www.w3.org/1999/xhtml" >This record, with ID: ' +
						trimmedID +
						" is linked to </head></div>",
				);
				var length = data.length;

				$(data).each(function (i) {
					$(content).append(
						'<li><a target="_blank" href="' + this.id + '">' + this.title + "</a> by " + this.relation + "</li>",
					);
				});
