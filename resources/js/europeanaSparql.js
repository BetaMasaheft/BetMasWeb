...
$(document).on("ready", function () {
	...
	$(Europeana)
        ...
        $(iconclass).append(relatedItem)
			...
			var sparqlQuery =
				"SELECT ?item ?commonsCat ?itemLabel \n" +
				"WHERE \n" +
				"{\n" +
				"  ?item wdt:P1256 '" +
				iconclassNumber +
			...
			makeSPARQLQuery(endpointUrl, sparqlQuery, function (data) {
				...
				$(bds).each(function (linkshere) {
					var relatedItem =
						'<li>Link to Wikidata entity <a href="' + this.item.value + '">' + this.itemLabel.value + "</a></li>";
					...
					$(cc).each(function () {
						$("#EuropeanaMatches")
							...
							.children("ul")
							.append(
								'<li>Wikimedia Commons Category <a href="https://commons.wikimedia.org/wiki/Category:' +
									this.value +
									'">' +
									this.value +
									"</a></li>",
