$(document).on("ready", function () {
	...
	$.getJSON("/api/count", function (data) {
		...
		$(count).append(
			"<p><b  class='lead'>" + tp + "</b> are Records about people, groups, ethnic or linguistic groups. </p>",
		);
		$(count).append(
			"<p>The other " + diff + " records are Authority files and places which are not repositories. </p>",
		);
	});
});
