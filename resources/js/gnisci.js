	...
	$.getJSON(restcall, function (data) {
		...
		if (data.total >= 1) {
			...
			$("#Chojnacki").append(
				"<h3>" +
					data.total +
					' related items from <a href="https://digi.vatlib.it/stp/Chojnacki" target="_blank">Chojnacki Archive at the Vatican Library</a></h3>',
			);
			...
			$(data.ChojnackItems).each(function (Choj) {
				...
				$.getJSON(manifest, function (man) {
					...
					var relatedItem =
						'<p class="w3-padding"><a target="_blank" href="' +
						record.link +
						'">' +
						record.name +
						'</a>, \
                    (<a target="_blank" href="https://digi.vatlib.it/view/STP_' +
						record.segnatura +
						'">' +
						record.segnatura +
						'</a>)</p>\
