...
$(document).on("ready", function () {
	...
	var pelagioscall =
		"https://peripleo.pelagios.org/peripleo/search?places=http%3A%2F%2Fbetamasaheft.eu%2Fplaces%2F" + id;
	$.getJSON(pelagioscall, function (data) {
		//console.log(data)
		if (data.total >= 1) {
			$("#pelagiosrelateditems").append(
				"<h3>" +
					data.total +
					' related items from <a href="https://commons.pelagios.org/" target="_blank">Pelagios</a></h3>',
			);

			$(data.items).each(function (linkshere) {
				var relatedItem =
					'<div class="w3-container"><p class="w3-large"><a href="' +
					this.homepage +
					'">' +
					this.title +
					"</a> in " +
					this.dataset_path["0"].title +
					"</p></div>";
...
$(".pelagios").on("click", function () {
	...
	var content = $(
		'<div id="' +
			placeID +
			'relations-content" class="popuptext w3-hide w3-tiny w3-padding" style="width: 260px;background-color: black;\
	...
	if ($(this).children("div").hasClass("popuptext")) {
		...
		$.getJSON(pelagioscall, function (data) {
			...
			if (data.total >= 1) {
				$(listtitle).append(
					data.total +
						' items from <a href="https://commons.pelagios.org/" target="_blank">Pelagios</a> related to <a href="' +
						href +
						'" target="_blank">this place</a>',
				);

				$(data.items).each(function (linkshere) {
					var relatedItem =
						'<li class="nodot"><a href="' +
						this.homepage +
						'">' +
						this.title +
						"</a> in " +
