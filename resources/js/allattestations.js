...
function results(res) {
	...
	var resultdiv =
		'<div class="w3-panel w3-card-4 attestationresult "><div class="w3-quarter"><a href="/' +
		resid +
		'">' +
		restitle +
...
function entry(entry) {
	...
	var thisentry =
		'<div class="w3-row singleattestation"><div class="w3-quarter">' +
		entry.position +
		") " +
		entrytitles +
		"<b>" +
...
function occurrences(occ) {
	...
	var alloccs =
		'<div class="w3-row w3-margin  w3-card-4"><p class="w3-padding">In the same context of this attestation also the following ' +
		type +
		' occur</p><div class="w3-responsive"><table class="w3-table w3-hoverable"><thead><tr><th>name</th><th>type</th></tr></thead><tbody>' +
		allnames +
		"</tbody></table></div></div>";
	return alloccs;
}
	...
	$.getJSON(apiCall, function (data) {
		if (data.msitems.length === 0) {
			if (start > 1 && $(msContainer).find(".no-more-items-msg").length === 0) {
				$(msContainer).append(
					'<div class="w3-text-grey no-more-items-msg" style="margin:0.5em 0;">No more items.</div>',
				);
			}
			return;
		}
