...
$(document).on("ready", function () {
	...
	var query =
		"SELECT DISTINCT ?locusFrom ?locusTo ?locusTarget ?type ?name \
	...
	$.getJSON(apicall, function (sparqlresult) {
		...
		$("#SdCTable tbody tr").each(function (row) {
			if (row > 0) {
				var thisrow = $(this);
				var previousrow = $(this).prev();
				if (
					thisrow.children("td").eq(0).text() === previousrow.children("td").eq(0).text() &&
					thisrow.children("td").eq(2).text() === previousrow.children("td").eq(2).text() &&
					thisrow.children("td").eq(3).text() === previousrow.children("td").eq(3).text() &&
					thisrow.children("td").eq(4).text() === previousrow.children("td").eq(4).text() &&
					thisrow.children("td").eq(5).text() === previousrow.children("td").eq(5).text() &&
					thisrow.children("td").eq(6).text() === previousrow.children("td").eq(6).text() &&
					thisrow.children("td").eq(7).text() === previousrow.children("td").eq(7).text() &&
					thisrow.children("td").eq(8).text() === previousrow.children("td").eq(8).text() &&
					thisrow.children("td").eq(9).text() === previousrow.children("td").eq(9).text() &&
					thisrow.children("td").eq(10).text() === previousrow.children("td").eq(10).text() &&
					thisrow.children("td").eq(11).text() === previousrow.children("td").eq(11).text() &&
					thisrow.children("td").eq(12).text() === previousrow.children("td").eq(12).text()
				) {
		...
		$("#SdCTable tbody tr td").each(function (tdindex) {
			...
			if ($(this).attr("style") === "display:none") {
				...
				if (txt === "") {
					...
					// console.log(name)
					var newtext =
						'<a class="enrichable"  data-anchor="' +
						finalid +
						'" target="_blank" href="/manuscripts/' +
						id +
						"/main#" +
						finalid +
						'">' +
...
$("#enrichTable").on("click", function () {
	...
	$(".enrichable").each(function () {
		...
		$.getJSON(apicallenrich, function (data) {
			/*    console.log(data)*/
			var enrichment = $('<table class="w3-table enriched"><tbody></tbody></table>');
