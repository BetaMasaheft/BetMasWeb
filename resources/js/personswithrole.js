$("#persRole").change(function (showthefilters) {
	...
	$.getJSON(apiurl + role, function (data) {
		...
		if (data.total == 0) {
			heading =
				"<h4>There are no persons with role <span class='w3-tag w3-gray w3-round'>" + data.role + "</span></h4>";
			$("#persWithRoleResults").empty();
			$(heading).appendTo("#persWithRoleResults");
		} else if (data.total > 0) {
			heading =
				"<h4>There are <span class='w3-tag w3-red w3-round'>" +
				data.total +
				"</span> persons with a role <span class='w3-tag w3-gray w3-round'>" +
				data.role +
				"</span></h4>";
			for (var i = 0; i < data.hits.length; i++) {
				...
				items.push(
					"<div class='w3-card-4 w3-padding w3-margin w3-third'><div id='" +
						id +
						"' class='w3-display-container'><header class='w3-container'><a href='/" +
						id +
						"'>" +
						title +
						"</a></header><div class='w3-container'> is mentioned as " +
						role +
						" " +
						match.hits +
