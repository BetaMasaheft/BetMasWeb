...
function convertDate(calendar, year, month, date, era) {
	...
	// western to ethiopic
	else if (calendar == "western" || calendar == "gregorian" || calendar == "julian") {
		const origDate = {
			year: year,
			month: month,
		};
		...
		return (
			(typeof date == "number" ? convDate.date + " " : "") +
			ethiopianMonths[convDate.month] +
			" " +
			convDate.year +
			" (" +
			(convDate.year + 5500) +
...
function ethiopicToJdn(date) {
	...
	if (date.month < 1 || date.month > 13) {
		...
		if (date.year % 4 == 3 && date.date > 6) {
			...
			return false;
		} else if (date.year % 4 != 3 && date.date > 5) {
...
function westernToJdn(date) {
	...
	if (date.month < 1 || date.month > 12) {
		...
		return false;
	} else if ((date.month == 4 || date.month == 6 || date.month == 9 || date.month == 11) && date.date > 30) {
		console.log("For date.month in [4,6,9,11] date.date must be <= 30.");
		return false;
	} else if (date.month == 2) {
		if (date.year % 4 == 0 && (date.year <= 1582 || date.year % 100 != 0 || date.year % 400 == 0) && date.date > 29) {
...
*/
	...
	if (date.year < 8 || (date.year == 8 && date.month < 8) || (date.year == 8 && date.month == 8 && date.date < 29)) {
