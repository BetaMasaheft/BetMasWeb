...
function printbibl() {
	...
	$(".Zotero").each(function () {
		...
		var linkBack =
			'<a href="/bibliography?pointer=' + tag + '"> <span class="fa fa-hand-o-left"  style="display:inline;"/></a>';
		...
		$.getJSON(apiurl + tag, function (d) {
			...
			if (thisurl == "") {
				...
				url +=
					' <a target="_blank" href="https://www.zotero.org/groups/358366/ethiostudies/tags/' +
					tag +
					'/library"><span class="fa fa-share" style="display:inline;"/></a>';
			} else {
			...
			if (d.length === 0) {
				...
				alert(
					"Something is wrong with bibliographic reference to " +
						tag +
						".  Perhaps it does not exist or the id contains a typo. Please, go check the source file and the Zotero Group library to fix this.  The element giving this error is contained in " +
						P[0].nodeName.toLowerCase() +
						"#" +
						P[0].id,
				);
			} else {
