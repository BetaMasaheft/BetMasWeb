$("a[id^='quotation']").on("click", function () {
	...
	$.getJSON(getquotations, function (d) {
		console.log(d);
		if (d.total >= 1) {
			$("#AllQuotations" + unit).append(
				'There are <span class="label label-warning">' + d.total + "</span> quotations of this passage",
			);
			...
			for (var i = 0; i < d.total; i++) {
				...
				$("#AllQuotations" + unit).append(
					'<div id="quotation' +
						q.source.id +
						'" class="row alert quote">Quotation of ' +
						reference +
						" in " +
						" " +
						q.source.title +
			...
			$("#AllQuotations" + unit).text(d.info);
		}
	});
});
